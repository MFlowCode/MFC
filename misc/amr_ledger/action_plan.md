...
## 2026-09-11 (128) — THE CURVE ON THE RUNG DECK, np8 -> np32: 1.23x then 1.32x per doubling, 1.62x over the span = 1.28x per doubling, against a MATCHED cross-node AMReX bar of 1.23x -- MFC is 3 % off the reference slope, not adrift from it; and the growth is NOT the O(P) regrid family the campaign expected but the base-grid advance and the base halo

**Registration.** GOAL v7 item 5, queued 2026-09-09, asked for np32 and np64 40/240 pairs at one pin with AMReX at the same sizes, judging weak scaling np8 -> np64; GOAL v8 item 1 re-tasked it to np8 -> np48 plus ledger 133's np64 closure. An empty `l128_prereg.md` placeholder exists and carries no registration, so those two texts are the registration. **Three departures, stated rather than glossed:** np64 is unplaceable; np48 is deferred to an addendum, which GOAL v8 asked to be in this entry; and the AMReX arms are pre-existing runs from 2026-08-25, not fresh ones taken alongside these.

**np64 is closed, by ledger 133 and not by this entry.** Ten nodes, two down, and two unusable, k004-003 with a faulting GPU and k004-006 with a dead IB port, leaves **six healthy nodes**, so np48 on six is the ceiling and np64 on eight is impossible. GOAL.md's statement-1 block still says "8 usable" at line 40 while line 32 of the same section says six; line 40 is the error and this corrects it. The np8 -> np48 span is a factor of six, two and a half doublings' worth, enough to separate a 1.2x slope from a 1.4x one, 1.60x against 2.39x.

**The curve must be pin-matched, and most of the rung history is not.** Every np16 rung in the logs was taken on the binary of the increment it tested, so ledger 133's doublings, 1.23x fix / 1.25x fold / 1.26x interleave / 1.265x seam-post union / 1.33x halo merge, are valid A/B readings and invalid curve points: a curve compares ranks, not commits. Grouping by the ``PIN`` file's binary **sha** rather than its ``head`` line leaves exactly two np8/np16 pairs on the binary these rungs use (a6dd813c, sha 19593fbc). Pin-matching is necessary and **not sufficient**: the lower rungs ran 2026-09-09 on k004-001+005 and the upper rung today on a different set, so this curve is one binary across two days and two node sets.

| pair | np8 | np16 | np32 | np48 | doubling |
|---|---|---|---|---|---|
| job 411779, healthy pair | 4.040 | 4.969 | | | **1.230x** |
| job 411122 | 3.791 | 5.798 | | | 1.529x, discarded |
| job 415070 | | | **6.563** | | **1.321x** |
| job 413469 | | | | pending | pending |

Job 411122 is discarded for the reason **ledger 119** recorded, not the one two live documents still give: its arms ran on **k004-003, whose GPU faults**, so ledger 119 voided its np16 wall and discarded its np8 arm. It has nothing to do with the k004-009 stall, which began three hours after that job ran; GOAL.md line 38 and notes/l128_curve_so_far.txt line 8 both carry the wrong reason and need the same fix.

**Reading.** The np32 rung ran 18:46-19:18 on **k004-[002,004-005,009]**, confirmed by sacct NodeList with State COMPLETED 0:0; the live ExcNodeList is no longer queryable, so sacct is the check that survives. **Zero NaN** across all 67 files of the run, the job log, both run logs, all 64 per-rank batch logs and pre.log. 40-step wall 175.794 s, 240-step 1488.330 s, differenced **6.563 s/step**, rc=0, zero truncation warnings, three and five rebuilds as at every other rung, ``[amr-cad]`` escaped 0 on both arms. Ledger 133 voided the earlier np32/np48 attempts on an unexplained NaN and ledger 134 diagnosed it as a chemistry-variant ``pre_process`` writing a 16-variable initial condition; this run's ``indices.dat`` lists **6** conservative variables, which is the direct refutation of that failure mode here and a stronger check than the rebuild count.

Weak scaling is genuinely fixed-work-per-rank: 8.0M base cells per rank at every rung, and the ``[amr-cad]`` L1 tag counts give 6,380,751 / 6,380,786 / 6,380,803 tags per rank, flat to eight parts in a million.

The curve is **np8 4.040, np16 4.969, np32 6.563 s/step**: 1.230x then **1.321x**, i.e. 1.624x over the 4x span = **1.275x per doubling**. The slope steepens with rank.

**The rerun's own node set is flagged, and its straggler detector FIRES.** Three of its four nodes carry standing warnings: k004-009 has intermittent GPU stalls that inflate 240-step arms while leaving 40-step arms flat, k004-002 reads about 20 % slow at np8, and k004-004 is on the cross-node-sick list. Ledger 133 trusts only k004-001 and k004-005 for timing, and only k004-005 of these four qualifies. Running the prescribed per-rank detector on the 240-step arm: **rank 31, on k004-009, sums 465.6 s against a 380.0 s mean, +22.5 %**, sustained in every 20-step block from step 40 on, with cv 0.0574 and max/min 1.306. Per-node means are k004-009 392.8, k004-004 381.8, k004-002 377.0, k004-005 368.6. **So this number is biased upward, in the direction the k004-009 stall is known to bias a differenced step.**

That also reframes the comparison with the void attempt. Job 413468 read 6.424, 2.1 % lower, and the earlier finding that k004-003 showed no slow-rank signature there is confirmed by the same detector: its eight ranks are the *fastest* group, node mean 362.5 against a 374.9 global mean, and its slowest rank is at the mean. The void was procedural. But the rerun has its own straggler, so the 2.1 % gap is not evidence about k004-003 in either direction.

**The bar exists, it is matched, and MFC is close to it.** An earlier draft of this entry said the AMReX arms at these sizes were never run. That was wrong. Jobs 384936 and 385522 ran AMReX at **amr.regrid_int 20**, max_level 2, no subcycling, reflux on, at n_cell 400³, 800x400x400 and 800x800x400 — exactly the MFC rung grids. Their cross-node doublings are **1.192x** from np8 to np16 and **1.278x** from np16 to np32, chaining to 1.523x over the span = **1.234x per doubling**, against MFC's 1.624x and **1.275x**. **MFC's slope is 3.3 % steeper than AMReX's on the same grids, the same refinement cadence and the same node boundaries.** Caveats that keep this honest: the two AMReX jobs disagree by 18 % on their shared np16 arm, so only each job's internal ratio is sound; AMReX still runs max_grid_size 32 against MFC's 64; the AMReX arms are 40-step total walls, not 240-40 differenced; it is scalar advection against six conservative variables; and both ran on nodes now unusable. The older intra-node S0 bar, 1.20x from np2 to np4 and 1.15x from np4 to np8 at regrid_int 2 and max_grid_size 32, remains the reference for the single-node regime and has not been re-run at the changed operating point.

**What the phase rows say about the mechanism, which is not what the campaign expected.** Differenced across the pin-matched rungs, with np32 recomputed from the clean rerun rather than the void attempt:

| row | np8 | np16 | np32 | 8->16 | 16->32 |
|---|---|---|---|---|---|
| rhs | 1.706 | 1.686 | 1.724 | 0.99 | 1.02 |
| coarse | 0.465 | 0.741 | 1.478 | **1.59** | **2.00** |
| b:halo | 0.310 | 0.581 | 1.299 | **1.87** | **2.24** |
| halo | 0.146 | 0.306 | 0.520 | 2.10 | 1.70 |
| gather | 0.212 | 0.401 | 0.489 | 1.89 | 1.22 |
| regrid | 0.236 | 0.283 | 0.335 | 1.20 | **1.18** |

**The regrid family is the slowest-growing row after rhs**, at 1.19x per doubling, and its share of the step *falls* with rank, 5.83 / 5.70 / 5.11 %. The 2026-09-02 O(P) reading and the box-gather and regrid-collective inventories predicted that family as the driver of the slope. At these rank counts it is not. The growth is the base-grid coarse advance, which doubles on the upper hop, and the base halo, which more than doubles — which is what ledgers 113 and 114 said and what GOAL.md already quotes. Statement 1 requires the regrid row to be read with the wall, and it is read here.

**Statement 1 restated.** Weak scaling on the rung deck at pin a6dd813c reads **1.28x per doubling from np8 to np32**, against a matched cross-node AMReX bar of 1.23x on the same grids. **This entry restates statement 1 over np8 -> np32 only**; the np48 rung (413469, six nodes, queued) closes the full span in a short addendum. GOAL v8 recorded a restatement target of 1.26-1.35x, but that band was derived from the *void* 6.424 and is not an independent prediction this measurement can fall inside; on the clean rerun the same np8 sensitivity gives **1.28-1.37x**, the band moving up by about 0.02. One rep per rung, and the np32 point is biased upward by its own straggler.

| np8 used | span np8 -> np32 | per doubling |
|---|---|---|
| 4.040, this ledger | 1.624x | 1.275x |
| 3.791, the other same-pin pair | 1.731x | 1.316x |
| 3.518, the low end of the same-node hour range | 1.866x | 1.366x |

**Caveats.** (1) Pin a6dd813c predates ledger 131's EOS fix. On the rung deck that regression is worth **+0.67 s/step at np8**, ledger 131's 3.13 -> 3.80, of which about 0.48 s is the fine RHS row, and about 0.8 s/step at np16 by ledger 133. The 0.212 and 0.155 figures quoted elsewhere are **ledger 138's** and belong to the WENO5 primary deck, which GOAL v8's deck split forbids carrying here. (2) **That the doublings transfer across the fix is an assumption, not a property, and it is directionally suspect**: ledger 133 measured the doubling moving by up to 0.10 across five pins on one node pair, and the fix cuts compute while leaving exchange bytes unchanged, which raises the comm share and should make the doubling worse. (3) One rep per rung; these are multi-node jobs and scarce, and the np8 level varies by hour on one node by more than it varies by pin. (4) Multi-node runs use the UCX recipe with ``UCX_NET_DEVICES`` unset. Two cross-node MPI_Init failure pairs are documented, k004-002 with k004-008 from 2026-08-31 and **k004-004 with k004-009 from 2026-09-10**; this rung ran on both of the second pair and completed, so the hang did not recur, but the pair is documented and an earlier draft wrongly said otherwise. (5) The upper rung builds its initial condition in-job from an **unpinned** ``pre_process`` and the ``PIN`` file shas only ``simulation``. Ledger 134 verified that binary by np8 initial-condition byte-identity; **the np32 initial condition has no such reference** and is verified only indirectly, by indices.dat carrying six variables, the rebuild counts and the absence of NaN. (6) The ``[phase]`` budget does not close at these rank counts, RESIDUAL -85.6 % in the np32 240-step arm, so individual rows are usable and the budget is not. (7) The rung recipe is labelled i8; ledger 119 calls the same family i7, and the i8 script carries the ``wait_vram`` guard added 2026-09-09. (8) The voided attempt emitted 190 UCX flush errors at teardown, 108 in its 40-step arm and 82 in its 240-step arm, after the step-loop wall with rc=0; **the rerun emits none**.

## 2026-09-11 (141) — GOAL v8 ITEM 2 IS NOT WORTH BUILDING, AND THE REASON IS MECHANICAL: a `nowait` target region encountered outside a parallel region is NOT deferred on this stack, it runs INLINE -- the launch loop takes 102 % of the kernel time to return -- so it overlaps only 11-14 % of a concurrent exchange when measured against the synchronous control, and the item is not built on either of the two scorings the data admits

**Pre-registration (notes/ledger_drafts/l141_prereg_asyncmpi.md, mtime 2026-09-11 16:32:13, 44 s before the first job was submitted at 16:32:57; the header line inside the file says 16:35 and is simply wrong).** GOAL v8 item 2 assumes an asynchronous kernel lets the host post and progress an exchange during it. Three arms on two ranks, same kernel and same message: (A) synchronous kernel then exchange, (B) `nowait` kernel, exchange, `taskwait`, (C) `nowait` kernel with non-blocking exchange and `MPI_Waitall`. P1: overlap >= 0.30 on B or C, build it in MFC. P2: overlap < 0.10 on both, the lever is dead. P3: between, rescale item 2's -0.06 to -0.12 s/step prediction by the measured fraction and drop the item if it falls below the 0.03 MDE. Void condition: either side below 50 ms.

**Two runs were discarded before the reported one, and both are reported rather than reinterpreted.** Job 415386 was VOID by the pre-registered floor: the exchange measured 14-23 ms, and three of its six overlap fractions came out above 1.0, which is impossible. Job 415388 was rescaled and cleared the floor, but an independent review of this ledger found it **fatally flawed and it is discarded**: the binary was compiled `--offload-arch=gfx942` while the partition is gfx90a (MI250X), and with `OMP_TARGET_OFFLOAD` unset the default is a silent host fallback, so the kernels may never have reached the GPU. Its symptom is visible in hindsight: its kernel-alone time swung 0.48 / 0.55 / 0.88 s across three repetitions of identical work, and two of its own overlap fractions again exceeded 1.0.

**The sound run: job 415410, k004-002, gfx90a, three repetitions.** Rebuilt `--offload-arch=gfx90a` with `OMP_TARGET_OFFLOAD=MANDATORY`, and the build line now lives inside amr-bench/async_mpi.sbatch so the run reproduces. Two probes were added that the earlier runs lacked: the program prints `omp_is_initial_device()` from inside a target region, and it times how long the `nowait` launch loop takes to RETURN. Device residency is proven, `kernels on the HOST? F` on every repetition, and the kernel-alone time is now rock steady at 0.395 / 0.394 / 0.395 s, which is itself the retrospective proof that the discarded run was executing on the host.

| quantity | mean | sd |
|---|---|---|
| K, kernel alone | 0.3947 s | 0.0006 |
| M, exchange alone | 0.2433 s | 0.0006 |
| A, synchronous kernel then exchange | 0.6913 s | 0.0050 |
| B, `nowait` kernel, exchange, `taskwait` | 0.6650 s | 0.0072 |
| C, `nowait` kernel, non-blocking exchange | 0.6577 s | 0.0040 |
| **B's launch loop, time to return** | **0.4030 s** | 0.0026 |

**The launch-loop timing is the finding.** The `nowait` launch loop takes 0.403 s to return against a kernel time of 0.395 s, a ratio of **1.021**. The loop does not return early; it returns after the work is done. **The deferred target task is not deferred at all: libomp executes it inline**, which is consistent with what libomp does for a task encountered by the initial thread outside any `parallel` region, though this test does not probe that path directly. So the absence of overlap is not the runtime refusing to progress MPI during a device kernel. It is the task never becoming asynchronous in the first place.

**What overlap there is, which prediction fires, and a declared change of statistic.** By the pre-registered formula, (K + M - t) / min(K, M), the overlap fractions are **-0.11 for B and -0.08 for C**, both below 0.10, so **P2 fires exactly as written** and the item is dead as designed. That formula is contaminated, however: the synchronous control itself exceeds the additive K + M = 0.638 s by 0.053 s, so an additive baseline charges a fixed per-region overhead against every arm. Measured instead against the synchronous control, which is the honest baseline here, B saves 0.026 s and C saves 0.034 s, or **10.8 % and 13.8 % of the exchange**, which lands in the P3 band. Both deltas are positive in every repetition and are resolved at n = 3 (paired t = 4.8 and 6.4, p about 0.04 and 0.02). **This change of statistic was made after seeing the data and is declared as such**, because it is the one substitution a pre-registration exists to prevent. The conclusion does not depend on it: P2 closes the item outright, and P3's own rescaling of -0.06 to -0.12 s/step by 13.8 % gives **0.008 to 0.017 s/step** against a 0.03 s/step detection threshold. **The item is not built on either route.** Nor is there concurrency available to explain the 11-14 %: if the launch loop returns only after the work is done, reduced per-region synchronization is the only mechanism left, and `nowait` recovers about half of the control's 0.053 s of it.

**What the profile says the ceiling would have been anyway.** Ledger 136 already published the relevant comparison and deserves the credit: the main thread's device-completion waits are about 25 s against 22.8 s of device kernel time, and its top stacks already route that wait through `targetDataEnd`. Re-measured here across all eight rank files, `InterruptSignal::WaitAcquire` carries 25.50 s per rank of sampled self time, of which 16.44 s is reached through the map-exit path, 7.79 s through `__tgt_target_kernel` with no map frame, and 1.27 s elsewhere, 0.70 s of that being the begin-of-region map; map-routed wait is therefore 17.14 s, 67 %. The map frames themselves carry almost nothing: `targetDataEnd` 0.037 s, H2D copy 0.030, D2H copy 0.009, `__tgt_target_kernel` 0.008, `submitData` 0.006, `retrieveData` 0.000. Two thirds of the block is therefore *reached through* a map, so stack shape alone does not distinguish "waiting for the kernel" from "waiting for the copy". What distinguishes them is the arithmetic: 25.50 s of waiting against 22.8 s of measured kernel time leaves almost nothing for the copies, so the wait is the kernel. An earlier draft of this ledger inferred from frame COUNTS that map traffic was the serializer and proposed a device-pointer rewrite; that inference is withdrawn here, before it was ever landed, and its prize would in any case be bounded by the 25.50 - 22.8 = **2.7 s per rank** residual, about 7 % of the 38.2 s loop, not the 0.09 s that draft quoted.

**What this changes.**
1. **GOAL v8 item 2 is closed, NOT BUILT, by its own pre-registered rescaling rule.** The cost was three micro-test runs and two discarded ones, against the four to six days the item budgeted.
2. **Ledger 136's mechanism stands and is credited.** The launches are synchronous and the host blocks on them. What this ledger adds is why the obvious fix does not work: the OpenMP form that should make them asynchronous does not, on this stack, in this calling context.
3. **One untried variant is named but not proposed:** encountering the `nowait` regions inside a `parallel` region, or from an explicit task, might let libomp actually defer them. That is a different experiment and a different pre-registration, and it would still be capped by the exchange time it could hide.
4. **The runtime candidate is closed at the stack level.** `ompi_info` on Open MPI 4.1.8 reports `Thread support: posix (MPI_THREAD_MULTIPLE: yes, OPAL support: yes, OMPI progress: no, ORTE progress: yes, Event lib: yes)`. The OMPI-layer progress thread is not built in; the only matching parameters in the whole build are `btl_tcp_progress_thread`, `ess_tool_async_progress` and `orte_progress_thread_debug`, which belong to the TCP BTL and to ORTE, neither of which progresses the UCX PML this machine uses. MFC owning its own progress thread would contend for the same core and is not proposed.

**Caveats.** (1) The micro-test's message buffers are never mapped to the device and travel over shared memory with `vader`` single-copy disabled, a two-copy host path, whereas MFC's halo is device-resident over UCX with RDMA. The gap is transport, not only size and frequency, and small eager messages may complete without progress calls at all. (2) The 600 ``nowait` regions carry no `depend`` clauses while all writing the same array, so they are formally unordered and serialized in practice only by the plugin's single stream. (3) Three time windows appear above and are not interchangeable: 25.50 s and 22.8 s are whole-run per rank, the 38.2 s loop is a loop wall, and the in-loop MPI figure below is whole-run. (4) The whole-run MPI is 8.4 s by the MPI call table but the sampler puts ``opal_progress` alone at 11.0 s per rank, so the two instruments disagree by about 2.6 s on the same quantity; treat 8.4 s as approximate, and note that much of a blocking wait is peer skew that freeing the host would not recover. (5) The negative result is specific to this stack, amdflang with Open MPI 4.1.8 and UCX on MI250X, and the test is cheap enough to repeat wherever MFC is ported. (6) A withdrawn pre-registration also numbered 141 exists at notes/ledger_drafts/WITHDRAWN_prereg_precision.md; the number belongs to this entry and that file is retired.

## 2026-09-11 (140) — LEDGER 120's PRESCRIPTION EXECUTED AND ITS POINT ESTIMATE CONFIRMED TO WITHIN 5 %: growing both uniform arms to the AMR arm's own 200-step window cuts the uniform arm's rep-to-rep spread 7x and 3.3x and its share of the excess variance from 96 % to 52 %, and shows the short window was BIASED as well as noisy -- the excess on the primary deck reads 0.649 (sd 0.022) and 1.75x AMReX on one node and one window

**Pre-registration (notes/ledger_drafts/l140_prereg.md, mtime 2026-09-11 14:18:06, sixteen minutes before job 415125 was submitted at 14:34:03).** The file carries one mtime, identical to twocode_u4.sh's to 17 ns, so it records when the prereg and the fixed script were saved together and no earlier revision survives. Stated honestly: 14:18 is after the first attempt (job 415051) started at 14:05 and after its first 240-step AMR arm had finished, so **P3's band was registered with one AMR arm already in hand**; P1 is fully independent of 415051, which had produced no uniform arm at all; P2's band contains the AMR term, so it shares P3's caveat in small part. P1: the uniform arm's rep-to-rep spread falls below 0.015, with three branches for 0.015-0.025 and above. P2: the excess mean lands inside 0.60-0.75; outside means the short window was biased, not merely noisy. P3: the AMR step is unchanged at 1.56-1.60 s/step, the control.

**The change is to the instrument only.** amr-bench/twocode_u4.sh differences the MFC uniform arm over 240-40 (200 steps) in place of 60-20 (40 steps); reader amr-bench/twocode_table_u4.py. Diffed against their parents, exactly two functional lines change in the script and one in the reader; the AMR arms, the AMReX arms, the cell ratios and the phase parsing are untouched. Same pin, and the binary sha (9b5f6f06d3a8b378) is identical to the two earlier runs', which pins it harder than the commit hash does. Job 415125, k004-001, 14:34-15:18; `sacct` confirms no other job of mine on that node in the window. The uniform arms now run 11.3-11.7 s and 57.1-58.7 s in place of 6.3-7.4 s and 15.2-16.2 s, a measured **+47 s per rep and +2.4 minutes on the run**.

**Reading.** All values recomputed from the RESULT lines, not from rounded tables.

| quantity | 60-20, k004-004 (414188) | 60-20, k004-002 (414158) | **240-40, k004-001 (415125)** |
|---|---|---|---|
| MFC uniform, per rep | 0.206 / 0.220 / 0.248 | 0.217 / 0.222 / 0.237 | **0.235 / 0.234 / 0.229** |
| MFC uniform, mean s/step | 0.2246 | 0.2255 | **0.2326** |
| MFC uniform, spread s/step | 0.042 | 0.020 | **0.006** |
| MFC AMR step, s/step | 1.561 / 1.583 / 1.564 | 1.563 / 1.583 / 1.604 | 1.564 / 1.545 / 1.568 |
| **MFC excess, s/step** | 0.691 (sd 0.086) | 0.701 (sd 0.023) | **0.649 (sd 0.022)** |
| AMReX excess, s/step | 0.358 (sd 0.039) | 0.365 (sd 0.003) | 0.372 (sd 0.017) |
| **ratio, dimensionless** | 1.93x | 1.92x | **1.75x** |

**P1 MET on its tightest branch:** the uniform spread is 0.006, below the 0.015 threshold, so the rep-to-rep variation was amplified short-window noise and not a physical rep-to-rep effect. The variance decomposition is the sharper statement and is the run's strongest result: the uniform step's sd is now **0.0033**, the amplified ideal term's sd 0.0128 against the AMR step's 0.0122, so **the uniform pair now contributes 52 % of the PROPAGATED excess variance, down from the 96 % ledger 120 measured**, which is the same propagation ledger 120's figure came from and so is like for like. The observed three-rep excess sd, 0.0219, runs above the propagated 0.0177, meaning the two arms came out anti-correlated across three reps; at n = 3 that is not interpretable, and ledger 139 recorded the same axis at +0.96 and -0.08 on its two nodes. Ledger 120 predicted this instrument almost exactly, forecasting a uniform-step sd of 0.003-0.008 and an excess sd near 0.023; measured, 0.0033 and 0.0219.

**P2 MET, and its bias branch did NOT fire:** 0.649 is inside the registered 0.60-0.75 band. A separate comparison, post hoc and labelled as such, nonetheless shows the short window was biased. The 60-20 difference read the uniform step at 0.2246 and 0.2255; the 200-step difference reads 0.2326, i.e. **biased low by 0.0076 s/step, which inflates the excess by 0.030**. That is 64 % of the 0.047 gap between this excess and the mean of the two earlier ones, and the remaining 0.017 decomposes exactly onto the AMR arm's own node-and-window term, whose step reads 1.559 here against 1.569 and 1.583.

**P3 MET to rounding but not literally:** the reps are 1.564 / 1.545 / 1.568 with mean 1.559, just under the registered 1.56 floor. The protocol's own pre-read checks pass: the AMR 240-step walls are flat and not monotone at 349.5 / 344.1 / 346.6, climb -0.8 %; ``fine_work`` is 186,311,808 in all six MFC AMR arms and ``cells_last_step`` 344,145,920 in all six AMReX arms, so every arm ran the same mesh; and ``nan=0`` on every MFC arm. The AMR arm is untouched by this change, so that shortfall is the same 0.017 window term the decomposition above already accounts for.

**What this changes.**
1. **Statement 2 on the primary deck reads 0.649 vs AMReX 0.372 = 1.75x**, on k004-001, pin 2cad1019, 240-40 on both arms. It supersedes ledger 139's 1.92x, which was right for its instrument, and ledger 132's 1.30x, which was right for its pin. The supersession is on instrument grounds and not on weight of evidence: ledger 139 read six reps on two nodes and this run reads three on one. It is one node and one window, and this very run measures a 0.017 node-and-window term on the AMR arm alone, so **0.649 carries an unreplicated window component and a second node is owed.** Every future excess is quoted with its deck, its pin and its uniform window.
2. **The protocol's floor moves from ledger 120's 0.050 to 0.022 s/step**, a 2.3x improvement, inside the 1.6x-to-5x range ledger 139 projected and short of its 4x point estimate. The two figures use different conventions and the conservative one is quoted: 0.050 was propagated from ledger 120's arm sds, while 0.022 is this run's direct three-rep sample sd of the excess; propagated on ledger 120's own basis this run gives 0.018, i.e. 2.8x. Like for like, two standard errors of a difference of two three-rep means, the campaign's own convention, goes from 0.082 to **0.036 s/step**. GOAL v8's quoted 0.10 was ledger 139's deliberately conservative all-in figure, not this formula; a power-based threshold at four degrees of freedom would be roughly twice 0.036.
3. **GOAL v8's metric rule stands unchanged.** The excess floor is now within striking distance of the AMR step's, but this run does not license scoring an increment on the excess: its arms are BLOCKED, not interleaved as the rule requires, which is exactly the design ledger 139 warned understates the bias; the run's own headline is that the excess moved 0.047 between windows on an unchanged binary, which is 1.3x the 0.036 threshold; and 0.022 is a three-point sample sd whose 95 % interval spans roughly 0.011 to 0.14. Revisit after a second, interleaved read.
4. twocode_u4.sh and twocode_table_u4.py replace twocode_fix3.sh and twocode_table.py for every future two-code read; the old pair is kept only to re-read historical logs.

**Phase rows, for the record.** rhs 0.795, coarse 0.213, restr 0.121, gather 0.111, reflux 0.097; the exchange families' MPI wait is 0.234 of a 0.338 total wait table, the same family definition ledger 139 used for its 0.242 of 0.341. These agree with 414188's mean-rank rows to within 0.011 on every row, worst at reflux, which is a node-to-node reproducibility check on the AMR arms and **not** a check on the instrument change: the two readers parse phases with byte-identical code, and the AMR arms were never touched. On the max-rank column the two runs agree only to 0.031, at rhs.

**Caveats.** (1) One node, one window, three reps; the 0.022 floor is a three-point estimate and firms up only as reads accumulate. (2) The AMReX arms were already differenced over 240-40 and are untouched here. Its excess reads 0.358 / 0.365 / 0.372 across the three runs with sds of 0.039 / 0.003 / 0.017, so those differences sit inside their own bands and no part of the ratio change should be attributed to AMReX with confidence. Decomposing the ratio move is order- and baseline-dependent: against k004-002 the 0.176 drop splits 0.141 numerator and 0.035 denominator, against k004-004 the 0.184 drop splits 0.116 and 0.068. (3) The uniform arm's start-up cost is now amortised over 200 steps rather than 40, which is why the level shifts; the transient itself is real and is diluted, not removed. (4) The first attempt, job 415051, reported COMPLETED because its wrapper exited zero, but its script died at line 45 with a bash syntax error after the MFC AMR arms: twocode_u4.sh's mtime falls inside that job's window, which is the proof that the script was edited while it executed. Its three AMR arms were clean at 1.577 / 1.561 / 1.586, mean 1.575, agreeing with this run to 0.016 s/step, which is the size of this metric's own window term.

## 2026-09-11 (139) — THE FIRST PIN-MATCHED TWO-CODE READS OF THE CURRENT CODE, ON BOTH HEALTHY NODES, REPRODUCE LEDGER 117's k004-004 READING (0.69-0.70 s/step, 1.92-1.93x) AND DISAGREE WITH LEDGER 132's k004-002 READING: the whole 0.583-to-0.696 gap is a level shift in the UNIFORM arm -- the same term ledger 120 measured as 96 % of this protocol's within-window variance, and about half of the 1.30x-to-1.92x move is AMReX's own excess falling on an unchanged binary
...
## 2026-09-15 (166) — SUBCYCLING PRICED AGAINST THE BATCHED DEFAULT: per-block subcycle is 1.20x SLOWER than batched lock-step at matched physical time and mesh; batching alone is worth 1.29x on this deck; a batched subcycled advance projects to 0.94x of the default (~6 %), not built

**Design** (`amr-bench/notes/prereg_subcycle_ab_0916.md`, pre-registered; run on the session allocation 420590, k004-009,
8 GCDs, pinned binary `inc/8f2e4a51` for every arm after the tip aborted on this deck, see the amendment). Campaign deck
(400^3, 2 levels, ratio 2, WENO5/HLLC), L = shipped batched lock-step (240/40 fine steps), S = `amr_subcycle = T` on the
per-block advance with dt = 4 dt_F, 60/10 coarse steps and `amr_regrid_int = 5` (same physical time and regrid cadence), order
L S L S L S with 120 s cool-downs; metric = wall per fine-step-equivalent, differenced. `fine_work` identical on every arm.

| arm | rep 1 | rep 2 | rep 3 | mean |
|---|---|---|---|---|
| L batched lock-step (s / fine dt) | 1.032 | 1.016 | 1.007 | 1.018 |
| S per-block subcycled | 1.229 | 1.243 | 1.209 | 1.227 |
| R = S / L | 1.190 | 1.224 | 1.201 | **1.205 (sd 0.017)** |
| P per-block lock-step (one rep, added after) | | | | 1.312 |

**Reading.** Subcycling on the unbatched path is 20 % slower than the shipped default, in the pre-registered band that
says "price the machinery before deciding". The extra arm prices it: batching is worth P/L = 1.29x on this deck, and
subcycling on the same per-block path is worth S/P = 0.94x (ledger 46's parity, 1.07x, now inside its own spread). If the
batching gain transferred entirely to the subcycled fine substeps, a batched subcycled advance would run at S/1.29 =
0.95 s per fine dt, **6 % under the default**. The arithmetic bound is low because this deck is fine-dominated (the base
grid is a quarter of the cell-updates); the machinery that subcycling amortises (gather, restrict, reflux once per coarse
step) is what keeps S/P under 1 despite the four-times-more fine substeps and their time-interpolated ghost exchanges.
Decision: not built; the item reopens for deep hierarchies, where the coarse share and hence the ceiling grow.

**A "tip regression" that was the harness.** The current tip (18313360) aborted at the first offloading construct on this
deck (all 8 ranks, `omptarget fatal ... Failure to copy data from host to device ... size = 0`), as did every commit back to
90b81be8, while the pinned 8f2e4a51 ran. The pinned binaries are case-optimized and the harness strips the four baked-in keys
(`num_fluids`, `weno_order`, `mapped_weno`, `mp_weno`) from the namelist because such a binary refuses them; tonight's bisect
binaries were generic, read those keys from the namelist, found none, and sized every field for zero fluids. Restoring the
keys removes the zero-size copies; the deck then stops with exit 22 (a further namelist mismatch: the deck's `simulation.inp`
was generated by an older toolchain). Rule for the next run: either pin a case-optimized build of the tip through `inc.sh`,
or regenerate the deck's namelists with the current toolchain before using a generic binary. No code regression is
established; the A/B above is unaffected (one binary for all arms).

## 167. 2026-09-16: the per-block advance leaves the branch; the deck's "exit 22" was a swallowed prohibit

**Decision.** The user moved every usable-but-slower AMR path off this branch: subcycling (6ac213f6) and the
per-block fine advance with the physics only it served - cylindrical and stretched grids, QBMM pb/mv side-state,
Euler bubbles, phase change, moving particle clouds (88793197). `amr_subcycle` and `amr_batched_advance` are gone;
the validator rejects those physics under `amr`. Net -6,900 lines. The removed paths live on
`sbryngelson/MFC:amr-per-block` = this branch + reverts of the two commits, for a follow-on pull request.

**Gate.** CPU: all 62 AMR/L0/active-box goldens unchanged (the batched advance was already every remaining case's
path, so byte-identical was the prediction and the result); 7 non-AMR smoke goldens pass. GPU (amdflang OpenMP
offload, k004-005): 62/62, three chemistry cases after their variant binary was built. Of the 18 subcycle goldens
eight became lock-step cases with regenerated goldens; the two that had lock-step siblings (6eq, moving IBM circle)
reproduced those siblings byte-for-byte.

**The "exit 22".** Ledger 166 read a code-22 abort with no message from generic binaries on the 3D 8-rank deck and
guessed a namelist mismatch. Code 22 is `CASE_FILE_ERROR_CODE`: a `@:PROHIBIT` fired and `s_mpi_abort` flushed stdout
only when handed its own message, so the reason vanished into the srun pipe. `s_mpi_abort` now flushes on every abort
(ac58fd70). The regenerated deck namelist differs from the campaign deck only by the toolchain defaults, and the current
tip's generic GPU binary runs the deck to completion (40 steps, 33.3 s step-loop, one rep, not a measurement); the
prohibit that fired on the earlier pins is not reproducible without rebuilding them and is left unexplained.

**Harness.** `twocode_u6.sh` (no `amr_bat_pad`; strips the four case-optimization keys only when the binary is
case-optimized, detected from its namelist table; strips `amr_subcycle`/`amr_batched_advance`); 32 scripts that read
removed report lines moved to `attic/`; the 11 stashes archived as patches in `attic/stashes/`.
