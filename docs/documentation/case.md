...
| `amr_device_pack`       | Logical | Pack and unpack the per-stage coarse-patch gather (F1/F2) as one fused device kernel per family per stage, walking the plan's flat transfer list, instead of one launch per transfer. Wire bytes and floating-point values are unchanged. Requires `amr`; incompatible with `amr_subcycle`; the non-polytropic QBMM pb/mv twin keeps its per-transfer path. Default F. |
