...
| `amr_lb_beta`           | Real    | Time-feedback load balance. At each regrid a block's partition weight is scaled by `1 + amr_lb_beta * (f/mean(f) - 1)`, where `f` is its previous owner's measured fine-block compute per unit weight over the last regrid interval, so a rank that runs slow per cell (fragmented shapes, more launches) is given fewer cells. 0 = weights are cell counts only. Requires `amr = T`, `0 <= amr_lb_beta <= 1`, and `rank_time_wrt = T` when > 0 (the timer is the signal). Default 0. |
| `amr_equal_tiles`       | Logical | Before `amr_max_grid_size` tiling, shrink a box's tag padding (never closer than 2 cells to the tags) so the tiles come out equal: an odd extent otherwise tiles unevenly (43 cells into 22 + 21), and unequal extents batch separately. Requires `amr = T` and `amr_buf >= 3`; incompatible with `ib` (default F). |
...
| `amr_equal_tiles`       | Logical | Equal `amr_max_grid_size` tiles by shrinking tag padding, keeping 2 cells (default F); requires `amr_buf >= 3`, not with `ib` |
| `amr_lb_beta`           | Real    | Time-feedback block weights in the regrid partition, damping in [0, 1] (default 0 = off); requires `rank_time_wrt = T` |
