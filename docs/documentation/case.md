...
- `hypoelasticity` activates elastic stress calculations for fluid-solid interactions. Requires `G` to be set in `fluid_pp`, and `fd_order` to be set (1, 2, or 4). Compatible with HLL (`riemann_solver = 1`), HLLC (`riemann_solver = 2`), and HLLD (`riemann_solver = 4`). The Riemann solver choice determines how the elastic stress source term \f$\mathbf{S}^e\f$ is discretized:
  ...
  - Characteristic boundary conditions (`bc_{x,y,z}%%beg/end` between -5 and -12) are supported when only fluids touch those boundaries (solids and their stress fields stay away from them). Elastic material - nonzero `G` or nonzero stress - adjacent to a characteristic boundary is not supported: the characteristic treatment covers the acoustic fields only.
