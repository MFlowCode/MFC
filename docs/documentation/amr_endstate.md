...
| W4 | no per-CELL global collectives | **level-1 gather DELETED by S3.1** (`s_amr_union_gtag` gone; ntag_bytes 185 MB -> 0 per rank per regrid, box set bit-identical). What replaced it is NOT flat: per-node signature reductions grow **1.66x per np-doubling (P^0.73)** vs the gather's 2.00x -- better exponent AND coefficient, ~4.4 GB/rank/regrid extrapolated to 1e5 ranks. Levels >= 2 (gwin pairs) still gather. **Remaining: S3.2 as a SPLIT-COMMUNICATOR recursion targeting the level-2 FOREST (~85 percent of tree nodes), then S3.3** | tag exchange O(local + peers) |
