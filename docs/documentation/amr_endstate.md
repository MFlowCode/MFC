...
|---|---|---|---|
| W1 | per-rank step cost = f(local cells, peers) | seam all-pairs scan FIXED (5ec798ed); the regrid pass-2 loops (parents x global tagged cells) remain, and are retired by S3.3 rather than needing their own increment | f(local, peers) |
...
| W4 | no per-CELL global collectives | **level-1 gather DELETED by S3.1** (`s_amr_union_gtag` gone; ntag_bytes 185 MB -> 0 per rank per regrid, box set bit-identical). What replaced it is NOT flat and NOT a better exponent: across np8/16/32 the reductions grow 1.660x then 1.827x, the exponent rising toward 1.0 because the forest term scales exactly with P while the cap-fixed level-1 term is constant. **Asymptotically O(P) -- a 20-27x COEFFICIENT cut only.** (A two-point 'P^0.73' reading was refuted by the third point.) Levels >= 2 (gwin pairs) still gather. **Remaining: S3.2 as a SPLIT-COMMUNICATOR recursion, proven on the level-1 path FIRST (the forest does zero collectives today, so converting it first would add ~22-28k collectives with nothing to scope them), then S3.3.** Note `rbytes` prices both paths and is a projection of the finished design, not today's wire | tag exchange O(local + peers) |
