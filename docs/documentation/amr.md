...
**Batched advance.** Within a rank, owned blocks advance either one at a time (the
per-block advance swaps one block into a single working slot: global grid state plus a
shared coarse-patch scratch buffer) or in batches (`amr_batched_advance`): blocks of
equal level and extent are stacked along the last active dimension and served by one
RHS call. The toolchain turns `amr_batched_advance` on whenever the case admits it and
...
the path for subcycling, stretched or cylindrical grids, MHD, IGR, bubbles,
hypoelasticity, chemistry, phase relaxation, surface tension, relativity, continuum
damage and the 6-equation model. In both modes per-rank wall time scales with the *sum*
...
| `amr_batched_advance` | Logical | F | Advance owned fine blocks of equal level and extent in batches, stacked in one RHS call (lock-step, Cartesian, uniform grid only; requires `amr_max_grid_size > 0`). The toolchain turns it on whenever the case admits it and leaves it unset; `F` forces the per-block advance |
