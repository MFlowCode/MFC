...
DESCRIPTIONS = {
    ...
    "jwl_reactive": "Enable progressive (reactive) JWL burn: transport a reaction-progress variable lambda with a Lee-Tarver Ignition & Growth source",
    "jwl_unr_A": "Reactive JWL: unreacted-explosive JWL pressure constant A",
    "jwl_unr_B": "Reactive JWL: unreacted-explosive JWL pressure constant B",
    "jwl_unr_R1": "Reactive JWL: unreacted-explosive JWL exponential coefficient R1",
    "jwl_unr_R2": "Reactive JWL: unreacted-explosive JWL exponential coefficient R2",
    "jwl_unr_omega": "Reactive JWL: unreacted-explosive JWL Gruneisen coefficient omega",
    "jwl_unr_rho0": "Reactive JWL: unreacted-explosive reference density",
    "jwl_unr_E0": "Reactive JWL: unreacted-explosive reference energy",
    "jwl_lt_I": "Lee-Tarver ignition rate coefficient I",
    "jwl_lt_b": "Lee-Tarver ignition exponent b on (1-lambda)",
    "jwl_lt_a": "Lee-Tarver ignition compression offset a",
    "jwl_lt_x": "Lee-Tarver ignition exponent x on compression",
    "jwl_lt_G1": "Lee-Tarver growth rate coefficient G1",
    "jwl_lt_c": "Lee-Tarver growth exponent c on (1-lambda)",
    "jwl_lt_d": "Lee-Tarver growth exponent d on lambda",
    "jwl_lt_y": "Lee-Tarver growth pressure exponent y",
    "jwl_lt_G2": "Lee-Tarver completion rate coefficient G2",
    "jwl_lt_e": "Lee-Tarver completion exponent e on (1-lambda)",
    "jwl_lt_g": "Lee-Tarver completion exponent g on lambda",
    "jwl_lt_z": "Lee-Tarver completion pressure exponent z",
    "jwl_lt_figmax": "Lee-Tarver max lambda for the ignition term",
    "jwl_lt_fg1max": "Lee-Tarver max lambda for the growth term",
    "jwl_lt_fg2min": "Lee-Tarver min lambda for the completion term",
    # WENO reconstruction
...
PATTERNS = [
    ...
    # fluid_pp patterns
    (r"fluid_pp\((\d+)\)%eos", "Equation of state selector for fluid {0}: 1=stiffened gas, 2=JWL"),
    ...
    (r"fluid_pp\((\d+)\)%jwl_A", "JWL pressure constant A for fluid {0}"),
    (r"fluid_pp\((\d+)\)%jwl_B", "JWL pressure constant B for fluid {0}"),
    (r"fluid_pp\((\d+)\)%jwl_R1", "JWL exponential coefficient R1 for fluid {0}"),
    (r"fluid_pp\((\d+)\)%jwl_R2", "JWL exponential coefficient R2 for fluid {0}"),
    (r"fluid_pp\((\d+)\)%jwl_omega", "JWL Gruneisen coefficient omega for fluid {0}"),
    (r"fluid_pp\((\d+)\)%jwl_rho0", "JWL reference density for fluid {0}"),
    (r"fluid_pp\((\d+)\)%jwl_E0", "JWL reference explosive energy for fluid {0}"),
    (r"fluid_pp\((\d+)\)%jwl_air_e0", "JWL air/reference specific internal energy for fluid {0}"),
    (r"fluid_pp\((\d+)\)%jwl_air_rho0", "JWL air/reference density for fluid {0}"),
    (r"fluid_pp\((\d+)\)%jwl_air_gamma", "JWL air/reference gamma-minus-one for fluid {0}"),
