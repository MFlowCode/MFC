...
DESCRIPTIONS = {
    ...
    "amr_equal_tiles": "Shrink a regrid box's tag padding (keeping 2 cells) so max_grid_size tiling cuts it into equal tiles: fewer distinct block extents, fewer batched launches (default F)",
    "amr_lb_beta": "Time-feedback load balance: each regrid scales a block's weight by 1 + beta*(owner's measured compute rate / mean - 1); 0 = cells only; needs rank_time_wrt",
