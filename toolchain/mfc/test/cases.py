...
def list_cases() -> typing.List[TestCaseBuilder]:
    ...
    def amr_golden_tests():
        ...
        # (r2) the same pinned-cap multi-level deck with amr_equal_tiles: cap 16 tiles a 31-wide region 16 + 15 (see (r)), which the
        # flag must equalise wherever the tags leave padding beyond 2 cells. Kept a SEPARATE golden so (r)'s brand-new-region path
        # stays exercised exactly as before.
        stack.push(
            "AMR -> 2D -> pinned max_grid_size multi-level equal tiles np=2",
            {
                **amr_2d_base,
                "m": 127,
                "n": 63,
