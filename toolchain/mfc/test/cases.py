    ...
    def alter_acoustic_src(dimInfo):
        ...
        if len(dimInfo[0]) == 1:
            for pulse_type in ['Sine', 'Square']:
                ...
                cases.append(define_case_d(stack, 'Frequency', {'acoustic(1)%frequency': 100}))
            ...
            stack.push('Gaussian', {'acoustic(1)%pulse': 2, 'acoustic(1)%delay': 0.05})
            cases.append(define_case_d(stack, 'Sigma Time', {'acoustic(1)%gauss_sigma_time': 0.01}))
            cases.append(define_case_d(stack, 'Sigma Dist', {'acoustic(1)%gauss_sigma_dist': 0.25}))
            stack.pop()
            ...
            for support in [5, 6]:
                ...
                cases.append(define_case_d(stack, 'Gaussian', {'acoustic(1)%pulse': 2, 'acoustic(1)%delay': 0.05, 'acoustic(1)%gauss_sigma_dist': 0.25}))
            ...
            stack.push('', {'acoustic(1)%loc(2)': 0.5, 'acoustic(1)%loc(3)': 0.5, 'acoustic(1)%wavelength': 0.25})
