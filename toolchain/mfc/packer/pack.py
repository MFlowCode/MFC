...
def compile(casepath: str) -> typing.Tuple[Pack, str]:
    ...
    for filepath in list(Path(D_dir).rglob("*.dat")):
        ...
        try:
            if "lag_bubble" in short_filepath:
            ...
            elif "probe" in short_filepath or "integral" in short_filepath or "ib_forces" in short_filepath:
                ...
                # retained. None of them carry a header line: ib_forces.dat keeps its column
                # names in a sibling .hdr, because a header inside the file would shift every
                # byte offset the writer computes.
                doubles = _extract_doubles(content)
            else:
