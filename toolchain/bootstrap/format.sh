...
# the structural warnings name the construct ("unmatched FortranBlockClose"): match that, not the bare word, which
# any future diagnostic (or a path) could carry
if grep -q "unmatched Fortran" "$FFMT_ERR"; then
