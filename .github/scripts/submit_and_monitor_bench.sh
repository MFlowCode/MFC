...
# Always use the PR's submit-slurm-job.sh so both master and PR builds benefit
# from the run_monitored_slurm_job.sh SIGKILL recovery wrapper.  The bench
# script is resolved relative to the current directory (master/ or pr/) so the
# correct branch code is benchmarked.
bash "${SCRIPT_DIR}/submit-slurm-job.sh" .github/workflows/common/bench.sh "$device" "$interface" "$cluster"
