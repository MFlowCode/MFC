...
# Script to run MFC tests in space-constrained environments
# This script uses chunked testing to avoid running out of space
...
CHUNK_SIZE=15  # Reduced chunk size for better space management
MEMORY="16G"   # Reduced memory allocation to avoid space issues  
TMPFS_SIZE="32G"  # Reduced tmpfs size for better space management

# Create external test directory to avoid container space issues
EXTERNAL_TEST_DIR="/tmp/mfc-external-tests"
mkdir -p "$EXTERNAL_TEST_DIR"
chmod 755 "$EXTERNAL_TEST_DIR"

# Enhanced cleanup function with more aggressive space management
cleanup_between_chunks() {
    echo "Performing enhanced cleanup for low-space environment..."
    
    # Stop any lingering apptainer processes
    pkill -f apptainer 2>/dev/null || true
    sleep 2
    
    # Clean up any test output directories with extended patterns
    ...
    find /tmp -name "*chunk*" -type d -exec rm -rf {} + 2>/dev/null || true
    ...
    # Clean up external test directory completely
    rm -rf "$EXTERNAL_TEST_DIR"/* 2>/dev/null || true
    rm -rf "$EXTERNAL_TEST_DIR"/.* 2>/dev/null || true
    
    # Clean up cache directories more aggressively
...
# Enhanced space checking with stricter thresholds for low-space environment
check_space() {
    ...
    if [ $available_mb -lt 3000 ]; then
        ...
        if [ $available_mb -lt 1500 ]; then
            echo "Error: Still insufficient space after cleanup (${available_mb}MB < 1500MB). Aborting."
            return 1
        fi
    fi
}

# Function to run a chunk of tests with enhanced space management
run_test_chunk() {
    local start_idx=$1
    local end_idx=$2
...
main() {
    ...
    echo "Starting chunked test execution with space optimization..."
