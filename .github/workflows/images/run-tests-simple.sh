...
# Script to run MFC tests with simple space optimization
# This version focuses on internal space management without external binding
...
CHUNK_SIZE=5   # Ultra-small chunk size to prevent space issues
MEMORY="4G"    # Minimal memory allocation to avoid tmpfs overflow

# Enhanced cleanup function with more aggressive space management
cleanup_between_chunks() {
    ...
    # Force system sync and memory cleanup
    sync
    echo "Cleanup completed"
}

# Enhanced space checking with more aggressive thresholds
check_space() {
    ...
    if [ $available_mb -lt 2000 ]; then
        echo "Warning: Low space detected (${available_mb}MB), performing aggressive cleanup..."
        cleanup_between_chunks
        
        # Check again after cleanup
        available_space=$(df /tmp | awk 'NR==2 {print $4}')
        available_mb=$((available_space / 1024))
        echo "Space after cleanup: ${available_mb}MB"
        
        if [ $available_mb -lt 1000 ]; then
            echo "Error: Still insufficient space after cleanup. Aborting."
            return 1
        fi
    fi
}

# Enhanced function to run a chunk of tests with maximum space optimization
run_test_chunk() {
    local start_idx=$1
    local end_idx=$2
    ...
    if ! check_space; then
        echo "Insufficient space, skipping this chunk"
        return 1
    fi
    
    # Create a unique temporary working directory for this chunk
    local chunk_work_dir="/tmp/mfc-chunk-$$-$(date +%s)"
    mkdir -p "$chunk_work_dir"/{cache,tests,output}
    chmod 755 "$chunk_work_dir" "$chunk_work_dir"/{cache,tests,output}
    
    # Set up environment variables to redirect all temporary files
    export APPTAINER_CACHEDIR="$chunk_work_dir/cache"
    export SINGULARITY_CACHEDIR="$chunk_work_dir/cache"
    export TMPDIR="$chunk_work_dir"
    export TEMP="$chunk_work_dir"
    export TMP="$chunk_work_dir"
    
    echo "Working directory: $chunk_work_dir"
    
    # Create external test directory to avoid container internal space issues
    local external_test_dir="/tmp/mfc-external-$$"
    mkdir -p "$external_test_dir"/{tests,cache,output}
    chmod 755 "$external_test_dir" "$external_test_dir"/{tests,cache,output}
    
    apptainer run \
        --no-home \
        --tmpdir "$chunk_work_dir" \
        --bind "$external_test_dir/tests:/opt/MFC/tests" \
        --bind "$external_test_dir/cache:/tmp/cache" \
        --bind "$external_test_dir/output:/tmp/output" \
        --env TMPDIR="/tmp/output" \
        --env TEMP="/tmp/output" \
        --env TMP="/tmp/output" \
        ...
        echo "Test chunk $start_idx-$end_idx failed, cleaning up..."
        rm -rf "$chunk_work_dir" 2>/dev/null || true
        cleanup_between_chunks
        return 1
    }
    
    echo "Test chunk completed successfully"
    
    # Immediate cleanup of chunk working directory and external directory
    rm -rf "$chunk_work_dir" 2>/dev/null || true
...
main() {
    ...
    echo "Starting chunked test execution with simple space optimization..."
