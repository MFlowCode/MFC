...
- Still manual: derived-type `TYPE` member definitions in `src/common/m_derived_types.fpp`;
  ...
  `G`); enum constants for compound registry keys (e.g. `fluid_pp(:)%eos`), hand-written in
  `m_constants.fpp` because `{param}_{name}` is not a valid Fortran identifier for a
  `fluid_pp(i)%...` key — `generate_constants_fpp` skips any key containing `%` or `(`; and the
