...
mfc_plots = [[], []]
for y in Y_MAJORS: mfc_plots[0].append(case.plot_time(f"Y_{y}", ax=axes[0], label=f"${y}$"))
for y in Y_MINORS: mfc_plots[1].append(case.plot_time(f"Y_{y}", ax=axes[1], label=f"${y}$"))
...
axes[0].add_artist(axes[0].legend([
    ...
    title="Induction Times", loc='lower right'))
