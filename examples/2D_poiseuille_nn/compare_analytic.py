#!/usr/bin/env python3
"""
Compare the steady MFC velocity profile of the 2D power-law Poiseuille channel
(examples/2D_poiseuille_nn/case.py) against the closed-form analytic solution.
...
This script reads the raw restart binaries (restart_data/lustre_*.dat) written by
the run. They store the conservative variables in Fortran order with x as the
fastest index, so the axis mapping is unambiguous (unlike the post-processed silo,
whose array is transposed). It extracts the x-averaged u(y) from the last save,
computes the analytic profile at the same cell centers, and reports the relative
L2 error

    err = sqrt( sum((u_num - u_ana)^2) / sum(u_ana^2) ).

The local momentum balance K*|du/dy|^n = rho*g*(H - y) is also checked in the
near-wall region: it must hold pointwise regardless of the regularization the
solver applies to the (singular) shear-free centerline, so it is the cleanest
shear-dependent-viscosity correctness test. Steady state is confirmed by comparing
the last two saves.

Run:  ./build/venv/bin/python3 examples/2D_poiseuille_nn/compare_analytic.py
"""
...
def main():
    ...
    if not dats:
        raise SystemExit(f"No restart files in {RESTART_DIR}. " "Run: ./mfc.sh run examples/2D_poiseuille_nn/case.py -n 2")
    ...
    # blunter, theory (n+1)/(2n+1).
    print(f"mean/peak (bluntness): {u_last.mean()/u_last.max():.3f}  " f"(parabola 0.667; n={NN} theory {(NN + 1) / (2 * NN + 1):.3f})")

    # Local momentum balance in the near-wall region (away from the singular
    # centerline): K*|du/dy|^n must equal rho*g*(H - y).
