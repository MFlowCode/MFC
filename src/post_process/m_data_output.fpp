...
contains
    ...
    impure subroutine s_write_intf_data_file(q_prim_vf)
        ...
        do k = 0, p
            do j = 0, n
                do i = 0, m
                    if (q_prim_vf(sys_idx%E + 2)%sf(i, j, k) > maxalph_loc) then
                        maxalph_loc = q_prim_vf(sys_idx%E + 2)%sf(i, j, k)
                    end if
                end do
            end do
        end do
        ...
        do k = 0, n
            do j = 0, m
                axp = q_prim_vf(sys_idx%E + 2)%sf(j + 1, k, cent)
                axm = q_prim_vf(sys_idx%E + 2)%sf(j, k, cent)
                ayp = q_prim_vf(sys_idx%E + 2)%sf(j, k + 1, cent)
                aym = q_prim_vf(sys_idx%E + 2)%sf(j, k, cent)
        ...
        do k = 0, p
            do j = 0, n
                do i = 0, m
                    ...
                    Egint = Egint + q_prim_vf(sys_idx%E + 2)%sf(i, j, k)*(gammas(2)*pres)*dV
                    do s = 1, num_vels
                        vel(s) = q_prim_vf(num_fluids + s)%sf(i, j, k)
                        Egk = Egk + 0.5_wp*q_prim_vf(sys_idx%E + 2)%sf(i, j, k)*q_prim_vf(2)%sf(i, j, k)*vel(s)*vel(s)*dV
                        Elk = Elk + 0.5_wp*q_prim_vf(sys_idx%E + 1)%sf(i, j, k)*q_prim_vf(1)%sf(i, j, k)*vel(s)*vel(s)*dV
                    ...
                    Et = Et + q_cons_vf(sys_idx%E)%sf(i, j, k)*dV
                end do
            end do
        end do
