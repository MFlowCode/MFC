...
contains
    ...
    impure subroutine s_lapack_example_eigenvalues()
        ...
        real(wp), dimension(n) :: wr, wi  ! Eigenvalues
        real(wp), dimension(n, n) :: vl, vr
        ...
        ! Call appropriate LAPACK routine based on precision
#ifdef MFC_SINGLE_PRECISION
        call cgeev( 'N',  & ! do not compute left eigenvectors
                    'V',  & ! compute right eigenvectors
                    n,    & ! order of input matrix (3x3)
                    A,    & ! input matrix
                    n,    & ! leading dimension of input matrix
                    wr,   & ! real part of eigenvalues (out)
                    wi,   & ! imaginary part of eigenvalues (out)
                    vl,   & ! left eigenvectors (out)
                    n,    & ! leading dimension of vl (in)
                    vr,   & ! right eigenvectors (out)
                    n,    & ! leading dimension of vr (in)
                    work, & ! work array (out)
                    lwork,   & ! dimension of work array (>=4n recommended)
                    info)   ! return state
        print *, "Using LAPACK (CGEEV)"
#else
        call dgeev('N','V',3,A,3,wr,wi,vl,ldvl,vr,ldvr,work,12,info)
        print *, "Using LAPACK (DGEEV)"
#endif

        ! Check for success
        if (info == 0) then
            print *, "Eigenvalues computed successfully!"
            print *, "wr(1): [", wr(1), ", ", wr(2), ", ", wr(3), "]"
            print *, "wi(1): [", wi(1), ", ", wi(2), ", ", wi(3), "]"
            print *, "vr: "
        else if (info < 0) then
            print *, "LAPACK error: argument ", -info, " had an illegal value"
        else
