        ...
        do l = 0, p
            do k = 0, n
                do j = 0, m
                    ...
                    q_cons_vf(sys_idx%alf)%sf(j, k, l) = (4._wp*pi*nR3bar)/(3._wp*q_cons_vf(sys_idx%n)%sf(j, k, l)**2._wp)
                end do
            end do
        end do
    ...
    impure subroutine s_compute_bubble_EE_source(q_cons_vf, q_prim_vf, rhs_vf, divu_in)
            ...
            do k = 0, n
                do j = 0, m
                    if (adv_n) then
                        ...
                        nbub = (3._wp/(4._wp*pi))*q_prim_vf(sys_idx%alf)%sf(j, k, l)/R3
                    end if
                    ...
                    do q = 1, nb
                        ...
                        myP = q_prim_vf(sys_idx%E)%sf(j, k, l)
                        alf = q_prim_vf(sys_idx%alf)%sf(j, k, l)
        ...
        if (.not. adap_dt) then
            ...
            do l = 0, p
                do q = 0, n
                    do i = 0, m
                        rhs_vf(sys_idx%alf)%sf(i, q, l) = rhs_vf(sys_idx%alf)%sf(i, q, l) + bub_adv_src(i, q, l)
