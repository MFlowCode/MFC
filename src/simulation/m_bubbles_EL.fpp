...
contains
    ...
    impure subroutine s_add_bubbles(inputBubble, q_cons_vf, bub_id)
        ...
        pliq = (q_cons_vf(sys_idx%E)%sf(cell(1), cell(2), cell(3)) - dynP - pi_inf)/gamma
        if (pliq < 0) print *, "Negative pressure", proc_rank, q_cons_vf(sys_idx%E)%sf(cell(1), cell(2), cell(3)), pi_inf, gamma, &
            & pliq, cell, dynP
    ...
    subroutine s_compute_bubbles_EL_source(q_cons_vf, q_prim_vf, rhs_vf)
        ...
        if (lag_params%solver_approach == 2) then
            ...
            do l = 1, num_dims
                call s_gradient_dir(q_prim_vf(sys_idx%E)%sf, q_beta(3)%sf, l)
                ...
                do k = idwbuff(3)%beg, idwbuff(3)%end
                    do j = idwbuff(2)%beg, idwbuff(2)%end
                        do i = idwbuff(1)%beg, idwbuff(1)%end
                            q_beta(3)%sf(i, j, k) = q_prim_vf(sys_idx%E)%sf(i, j, k)*q_prim_vf(contxe + l)%sf(i, j, k)
                        end do
                    end do
                end do
                $:END_GPU_PARALLEL_LOOP()
                ...
                do k = 0, p
                    do j = 0, n
                        do i = 0, m
                            if (q_beta(1)%sf(i, j, k) > (1._wp - lag_params%valmaxvoid)) then
                                rhs_vf(sys_idx%E)%sf(i, j, k) = rhs_vf(sys_idx%E)%sf(i, j, k) - q_beta(4)%sf(i, j, &
    ...
    subroutine s_get_pinf(bub_id, q_prim_vf, ptype, f_pinfl, cell, preterm1, term2, Romega)
        ...
        if ((lag_params%cluster_type == 1)) then
            ...
            if (p == 0) then  ! 2D
                f_pinfl = q_prim_vf(sys_idx%E)%sf(cell(1), cell(2), cell(3))*(1._wp - psi(1))*(1._wp - psi(2))
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1) + 1, cell(2), cell(3))*psi(1)*(1._wp - psi(2))
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1) + 1, cell(2) + 1, cell(3))*psi(1)*psi(2)
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1), cell(2) + 1, cell(3))*(1._wp - psi(1))*psi(2)
            else  ! 3D
                f_pinfl = q_prim_vf(sys_idx%E)%sf(cell(1), cell(2), cell(3))*(1._wp - psi(1))*(1._wp - psi(2))*(1._wp - psi(3))
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1) + 1, cell(2), cell(3))*psi(1)*(1._wp - psi(2))*(1._wp - psi(3))
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1) + 1, cell(2) + 1, cell(3))*psi(1)*psi(2)*(1._wp - psi(3))
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1), cell(2) + 1, cell(3))*(1._wp - psi(1))*psi(2)*(1._wp - psi(3))
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1), cell(2), cell(3) + 1)*(1._wp - psi(1))*(1._wp - psi(2))*psi(3)
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1) + 1, cell(2), cell(3) + 1)*psi(1)*(1._wp - psi(2))*psi(3)
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1) + 1, cell(2) + 1, cell(3) + 1)*psi(1)*psi(2)*psi(3)
                f_pinfl = f_pinfl + q_prim_vf(sys_idx%E)%sf(cell(1), cell(2) + 1, cell(3) + 1)*(1._wp - psi(1))*psi(2)*psi(3)
            end if
                ...
                do j = 1, smearGrid
                    ...
                    do k = 1, smearGridz
                        ...
                        if (.not. celloutside) then
                            ...
                            !> Update values
                            charvol = charvol + vol
                            charpres = charpres + q_prim_vf(sys_idx%E)%sf(cellaux(1), cellaux(2), cellaux(3))*vol
                            charvol2 = charvol2 + vol*q_beta(1)%sf(cellaux(1), cellaux(2), cellaux(3))
                            charpres2 = charpres2 + q_prim_vf(sys_idx%E)%sf(cellaux(1), cellaux(2), &
