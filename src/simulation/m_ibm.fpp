    ...
    subroutine s_ibm_correct_state(q_cons_vf, q_prim_vf, pb_in, mv_in)
        ...
        if (num_gps > 0) then
            $:GPU_PARALLEL_LOOP(private='[i, physical_loc, dyn_pres, alpha_rho_IP, alpha_IP, pres_IP, pres_GP, vel_IP, vel_g, &
                                ...
                                & norm, buf, radial_vector, j, k, l, q, qv_K, c_IP, nbub, patch_id, Ys_IP, W_species, T_IP, &
                                & mw_IP, e_IP, Ys_g, Ys_s, T_s, T_g, mw_s, mw_g, rho_s, mdot_s, v_stefan, d, surface_converged, &
                                & vel_sum_g, E_ghost, alpha_q, alpha_rho_q, e_q]', reduction='[[n_not_converged, n_ill_posed]]', reductionOp='[+]')
            do i = 1, num_gps
                gp = ghost_points(i)
                j = gp%loc(1)
                k = gp%loc(2)
