...
contains
    ...
    subroutine s_ibm_correct_state(q_cons_vf, q_prim_vf, pb_in, mv_in)
        ...
        if (num_gps > 0) then
            ...
            & r_IP, v_IP, pb_IP, mv_IP, nmom_IP, presb_IP, massv_IP, rho, gamma, pi_inf, Re_K, G_K, Gs, gp, innerp, norm, buf, &
                & radial_vector, rotation_velocity, j, k, l, q, qv_K, c_IP, nbub, patch_id]')
            do i = 1, num_gps
                gp = ghost_points(i)
                j = gp%loc(1)
                k = gp%loc(2)
                ...
                if (patch_ib(patch_id)%moving_ibm <= 1) then
                    ...
                    do q = 1, num_fluids
                        ...
                        q_prim_vf(E_idx)%sf(j, k, l) = q_prim_vf(E_idx)%sf(j, k, &
                            & l) + pres_IP/(1._wp - 2._wp*abs(gp%levelset*alpha_rho_IP(q)/pres_IP)*dot_product(patch_ib(patch_id) &
                            & %force/patch_ib(patch_id)%mass, gp%levelset_norm))
                    end do
                end if
    ...
    subroutine s_compute_ib_forces(q_prim_vf, fluid_pp)
        ...
        & local_torque_contribution, dynamic_viscosity, viscous_stress_div, viscous_stress_div_1, viscous_stress_div_2, dx, dy, &
            & dz]', copy='[forces, torques]', copyin='[ib_markers, patch_ib, dynamic_viscosities]', collapse=3)
        do i = 0, m
            do j = 0, n
                do k = 0, p
