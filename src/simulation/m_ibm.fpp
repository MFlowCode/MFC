...
contains
    ...
    subroutine s_ibm_correct_state(q_cons_vf, q_prim_vf, pb_in, mv_in)
        ...
        $:GPU_PARALLEL_LOOP(private='[i, j, k, patch_id, rho]', copyin='[sys_idx%E, momxb]', collapse=3)
        do l = 0, p
            do k = 0, n
                do j = 0, m
                    patch_id = ib_markers%sf(j, k, l)
                    if (patch_id /= 0) then
                        q_prim_vf(sys_idx%E)%sf(j, k, l) = 1._wp
                        rho = 0._wp
                        do i = 1, num_fluids
        ...
        if (num_gps > 0) then
            ...
            do i = 1, num_gps
                ...
                if (surface_tension) then
                    q_prim_vf(sys_idx%c)%sf(j, k, l) = c_IP
                end if

                ! set the pressure
                if (patch_ib(patch_id)%moving_ibm <= 1) then
                    q_prim_vf(sys_idx%E)%sf(j, k, l) = pres_IP
                else
                    q_prim_vf(sys_idx%E)%sf(j, k, l) = 0._wp
                    $:GPU_LOOP(parallelism='[seq]')
                    do q = 1, num_fluids
                        ! Pressure correction for moving IB: accounts for acceleration of IB surface
                        q_prim_vf(sys_idx%E)%sf(j, k, l) = q_prim_vf(sys_idx%E)%sf(j, k, &
                ...
                ! Set color function
                if (surface_tension) then
                    q_cons_vf(sys_idx%c)%sf(j, k, l) = c_IP
                end if

                ! Set Energy
                if (bubbles_euler) then
                    q_cons_vf(sys_idx%E)%sf(j, k, l) = (1 - alpha_IP(1))*(gamma*pres_IP + pi_inf + dyn_pres)
                else
                    q_cons_vf(sys_idx%E)%sf(j, k, l) = gamma*pres_IP + pi_inf + dyn_pres
                end if
                ! Set bubble vars
                if (bubbles_euler .and. .not. qbmm) then
        ...
        do i = i1, i2
            ...
            do j = j1, j2
                ...
                do k = k1, k2
                    ...
                    pres_IP = pres_IP + coeff*q_prim_vf(sys_idx%E)%sf(i, j, k)
                    ...
                    if (surface_tension) then
                        c_IP = c_IP + coeff*q_prim_vf(sys_idx%c)%sf(i, j, k)
                    end if
                ...
                do k = 0, p
                    ...
                    if (ib_idx /= 0) then
                        ...
                        do fluid_idx = 0, num_fluids - 1
                            ...
                            local_force_contribution(1) = local_force_contribution(1) - (q_prim_vf(sys_idx%E + fluid_idx)%sf(i &
                                                     & + 1, j, k) - q_prim_vf(sys_idx%E + fluid_idx)%sf(i - 1, j, &
                                                     & k))/(2._wp*dx)  ! force is the negative pressure gradient
                            local_force_contribution(2) = local_force_contribution(2) - (q_prim_vf(sys_idx%E + fluid_idx)%sf(i, &
                                                     & j + 1, k) - q_prim_vf(sys_idx%E + fluid_idx)%sf(i, j - 1, k))/(2._wp*dy)
                            cell_volume = abs(dx*dy)
                            ! add the 3D component of the pressure gradient, if we are working in 3 dimensions
                            if (num_dims == 3) then
                                dz = z_cc(k + 1) - z_cc(k)
                                local_force_contribution(3) = local_force_contribution(3) - (q_prim_vf(sys_idx%E + fluid_idx) &
                                                         & %sf(i, j, k + 1) - q_prim_vf(sys_idx%E + fluid_idx)%sf(i, j, &
                                                         & k - 1))/(2._wp*dz)
