                    ...
                    do l = is3%beg, is3%end
                        do k = is2%beg, is2%end
                            do j = is1%beg, is1%end
                                ...
                                if (hypoelasticity) then
                                    flux_rs${XYZ}$_vf(j, k, l, contxe + dir_idx(1)) = &
                                        xi_M*(rho_L*(vel_L(dir_idx(1))*vel_L(dir_idx(1)) + &
                                                     s_M*(xi_L*s_S - vel_L(dir_idx(1)))) + pres_tot_L) &
                                        + xi_P*(rho_R*(vel_R(dir_idx(1))*vel_R(dir_idx(1)) + &
                                                       s_P*(xi_R*s_S - vel_R(dir_idx(1)))) + pres_tot_R) &
                                        + (s_M/s_L)*(s_P/s_R)*pcorr
                                    if (n > 0) then
                                        flux_rs${XYZ}$_vf(j, k, l, contxe + dir_idx(2)) = &
                                            xi_M*(rho_L*(vel_L(dir_idx(1))*u_t_L + &
                                                         s_M*(xi_L*u_t_star - u_t_L)) - tau_nt_L) &
                                            + xi_P*(rho_R*(vel_R(dir_idx(1))*u_t_R + &
                                                           s_P*(xi_R*u_t_star - u_t_R)) - tau_nt_R)
                                    end if
                                    if (p > 0) then
                                        flux_rs${XYZ}$_vf(j, k, l, contxe + dir_idx(3)) = &
                                            xi_M*(rho_L*(vel_L(dir_idx(1))*u_t2_L + &
                                                         s_M*(xi_L*u_t2_star - u_t2_L)) - tau_nt2_L) &
                                            + xi_P*(rho_R*(vel_R(dir_idx(1))*u_t2_R + &
                                                           s_P*(xi_R*u_t2_star - u_t2_R)) - tau_nt2_R)
                                    end if

                                    flux_rs${XYZ}$_vf(j, k, l, E_idx) = &
                                        xi_M*((E_L + pres_tot_L)*vel_L(dir_idx(1)) &
                                              - u_t_L*tau_nt_L - u_t2_L*tau_nt2_L + &
                                              s_M*(xi_L*(E_L + (s_S - vel_L(dir_idx(1)))* &
                                                         (rho_L*s_S + pres_tot_L/(s_L - vel_L(dir_idx(1)))) &
                                                         + (u_t_L*tau_nt_L - u_t_star*tau_nt_star) &
                                                         /(s_L - vel_L(dir_idx(1))) &
                                                         + (u_t2_L*tau_nt2_L - u_t2_star*tau_nt2_star) &
                                                         /(s_L - vel_L(dir_idx(1)))) &
                                                   - E_L)) &
                                        + xi_P*((E_R + pres_tot_R)*vel_R(dir_idx(1)) &
                                                - u_t_R*tau_nt_R - u_t2_R*tau_nt2_R + &
                                                s_P*(xi_R*(E_R + (s_S - vel_R(dir_idx(1)))* &
                                                           (rho_R*s_S + pres_tot_R/(s_R - vel_R(dir_idx(1)))) &
                                    ...
                                    if (n == 0) then
                                        ...
                                        if (dir_idx(1) == 1) then
                                            ...
                                            flux_rs${XYZ}$_vf(j, k, l, strxb + 1) = &
                                                xi_M*(rho_L*vel_L(dir_idx(1))*tau_nt_L + s_M*(rho_L*xi_L*tau_nt_star - rho_L*tau_nt_L)) &
                                                + xi_P*(rho_R*vel_R(dir_idx(1))*tau_nt_R + s_P*(rho_R*xi_R*tau_nt_star - rho_R*tau_nt_R))
                                        ...
                                        flux_rs${XYZ}$_vf(j, k, l, itnt2) = &
                                            xi_M*(rho_L*vel_L(dir_idx(1))*tau_nt2_L + s_M*(rho_L*xi_L*tau_nt2_star - rho_L*tau_nt2_L)) &
                                            + xi_P*(rho_R*vel_R(dir_idx(1))*tau_nt2_R + s_P*(rho_R*xi_R*tau_nt2_star - rho_R*tau_nt2_R))
                                    ...
                                    if (cyl_coord) then
                                        flux_rs${XYZ}$_vf(j, k, l, strxe) = &
                                            xi_M*rho_L*tau_qq_L*(vel_L(dir_idx(1)) + s_M*(xi_L - 1._wp)) &
    ...
    subroutine s_hypo_hlld_riemann_solver(qL_prim_rsx_vf, qL_prim_rsy_vf, qL_prim_rsz_vf, &
        ...
        #:for NORM_DIR, XYZ in [(1, 'x'), (2, 'y'), (3, 'z')]
            if (norm_dir == ${NORM_DIR}$) then
                ...
                $:GPU_PARALLEL_LOOP(collapse=3, private='[i,j,k,l,alpha_rho_L,alpha_rho_R,vel,alpha_L,alpha_R,rho,pres,E,H,gamma,pi_inf,qv,vel_rms,c,S_L,S_R,s_M,S_Lstar,S_Rstar,pTot_L,pTot_R,rhoL_star,rhoR_star,U_L,U_R,U_starL,U_starR,U_starstarL,U_starstarR,F_L,F_R,F_starL,F_starR,F_hlld,F_HLL,U_HLL,rho_HLL,u_n_HLL_cons,tau_nn_HLL,u_n_HLL_trace,u_t_HLL_trace,p_face_HLL,tau_qq_face_HLL,ncomp,C_NC,sqrtC_NC,A_L,A_R,denomA,fac_L,fac_R,u_n_L,u_t_L,u_n_R,u_t_R,u_t2_L,u_t2_R,tau_nn_L,tau_nt_L,tau_tt_L,tau_nn_R,tau_nt_R,tau_tt_R,tau_nt2_L,tau_nt2_R,tau_t2t2_L,tau_t2t2_R,tau_t1t2_L,tau_t1t2_R,tau_qq_L,tau_qq_R,G_L,G_R,tau_e_L,tau_e_R,alpha1_L_star,alpha1_R_star,alpha2_L_star,alpha2_R_star,u_t_star,tau_nt_star,u_t2_star,tau_nt2_star,tau_nn_L_star,tau_nn_R_star,tau_tt_L_star,tau_tt_R_star,tau_tt_L_starstar,tau_tt_R_starstar,tau_t2t2_L_star,tau_t2t2_R_star,tau_t2t2_L_starstar,tau_t2t2_R_starstar,tau_t1t2_L_star,tau_t1t2_R_star,tau_t1t2_L_starstar,tau_t1t2_R_starstar,tau_qq_L_star,tau_qq_R_star,pTot_star,E_L_star,E_R_star,E_L_starstar,E_R_starstar,p_face,tau_qq_face,u_n_face,u_t_face,G_hat,rho_hat,u_n_hat,u_t_hat,u_t2_hat,tau_nn_hat,tau_nt_hat,tau_tt_hat,tau_qq_hat,tau_nt2_hat,tau_t2t2_hat,tau_t1t2_hat,alpha_hat,alpha_rho_hat,vel_hat,tau_e_hat,pres_hat,blkmod1_hat,blkmod2_hat,K_hat,C_hat_1,C_hat_2,Sigma_L,Sigma_R,dSigma,Sigma_ref,a_L_ref,a_R_ref,a_ref,du_t,dtau_nt,du_t2,dtau_nt2,sensor_ptot,sensor_vt,sensor_tnt,sensor_combined,phi,alpha_L_sum,alpha_R_sum]')
                do l = is3%beg, is3%end
                    do k = is2%beg, is2%end
                        do j = is1%beg, is1%end
