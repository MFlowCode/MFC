...
module m_hyperelastic
 ...
 s_finalize_hyperelastic_module
    ...
    !! at the cell-interior Gaussian quadrature points.
    !! is needed to be calculated once and make the code DRY.
...
contains
    ...
    !>  The following subroutine handles the calculation of the btensor.
        !! of the btensor takes qprimvf.
        !! Primitive variables
        !! is the output
        !! grad_xi, grad_xi is a nxn tensor
        !! inverse of grad_xi to obtain F, F is a nxn tensor
        !! FFtranspose to obtain the btensor, btensor is nxn tensor
        !! symmetric, save the data space
            ...
            do k = 0, n
                do j = 0, m
                    ...
                    if (G_local > verysmall) then
                        ...
                        ! 1: computing the grad_xi tensor using finite differences
                        ! definition / organization
                        ! for the tensor 1-3:  dxix_dx, dxiy_dx, dxiz_dx
                        ! :                       dxix_dy, dxiy_dy, dxiz_dy
                        ! :                       dxix_dz, dxiy_dz, dxiz_dz
                        $:GPU_LOOP(parallelism='[seq]')
                        ...
                        ! 2a: computing the adjoint of the grad_xi tensor for the inverse
                        ...
                        ! 2b: computing the determinant of the grad_xi tensor
                        ...
                        if (tensorb(tensor_size) > verysmall) then
                            ! 2c: computing the inverse of grad_xi tensor = F
                            ! is the adjoint, tensora becomes F
                            $:GPU_LOOP(parallelism='[seq]')
                            do i = 1, tensor_size - 1
                                tensora(i) = tensorb(i)/tensorb(tensor_size)
                            end do

                            ! 2d: computing the J = det(F) = 1/det(\grad{\xi})
                            tensorb(tensor_size) = 1._wp/tensorb(tensor_size)

                            ! 3: computing F transpose F
                            ...
                            ! 4: update the btensor, this is consistent with Riemann solvers
                            #:for BIJ, TXY in [(1,1),(2,2),(3,5),(4,3),(5,6),(6,9)]
                                btensor%vf(${BIJ}$)%sf(j, k, l) = tensorb(${TXY}$)
                            #:endfor
                            ! the determinant at the last entry of the btensor
                            btensor%vf(b_size)%sf(j, k, l) = tensorb(tensor_size)
                            ! 5a: updating the Cauchy stress primitive scalar field
