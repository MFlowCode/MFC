...
contains
        ...
        do l = 0, p
            do k = 0, n
                do j = -1, m
                    ...
                    ! temporarily undo dt scaling for inviscid igr cases
                    ...
                    do j = 0, m
                        ...
                        #:for LR in ['L', 'R']
                            ...
                            rhs_vf(igr_momxb + 1)%sf(j, k + 1, l) = rhs_vf(igr_momxb + 1)%sf(j, k + 1, &
                                   & l) + real(0.5_wp*sigma_dt*F_${LR}$*(1._wp/dy(k + 1)), kind=stp)
                            $:GPU_ATOMIC(atomic='update')
                            rhs_vf(igr_E_idx)%sf(j, k + 1, l) = rhs_vf(igr_E_idx)%sf(j, k + 1, &
                                   & l) + real(0.5_wp*sigma_dt*vel_${LR}$*F_${LR}$*(1._wp/dy(k + 1)), kind=stp)
                            $:GPU_ATOMIC(atomic='update')
                            rhs_vf(igr_momxb + 1)%sf(j, k, l) = rhs_vf(igr_momxb + 1)%sf(j, k, &
                                   & l) - real(0.5_wp*sigma_dt*F_${LR}$*(1._wp/dy(k)), kind=stp)
                            $:GPU_ATOMIC(atomic='update')
                            rhs_vf(igr_E_idx)%sf(j, k, l) = rhs_vf(igr_E_idx)%sf(j, k, &
                                   & l) - real(0.5_wp*sigma_dt*vel_${LR}$*F_${LR}$*(1._wp/dy(k)), kind=stp)
                        #:endfor
                    end do
                end do
            end do
                    ...
                    do j = 0, m
                        ...
                        #:for LR in ['L', 'R']
                            ...
                            rhs_vf(igr_momxb + 2)%sf(j, k, l + 1) = rhs_vf(igr_momxb + 2)%sf(j, k, &
                                   & l + 1) + real(0.5_wp*sigma_dt*F_${LR}$*(1._wp/dz(l + 1)), kind=stp)
                            $:GPU_ATOMIC(atomic='update')
                            rhs_vf(igr_E_idx)%sf(j, k, l + 1) = rhs_vf(igr_E_idx)%sf(j, k, &
                                   & l + 1) + real(0.5_wp*sigma_dt*vel_${LR}$*F_${LR}$*(1._wp/dz(l + 1)), kind=stp)
                            $:GPU_ATOMIC(atomic='update')
                            rhs_vf(igr_momxb + 2)%sf(j, k, l) = rhs_vf(igr_momxb + 2)%sf(j, k, &
                                   & l) - real(0.5_wp*sigma_dt*F_${LR}$*(1._wp/dz(l)), kind=stp)
                            $:GPU_ATOMIC(atomic='update')
                            rhs_vf(igr_E_idx)%sf(j, k, l) = rhs_vf(igr_E_idx)%sf(j, k, &
                                   & l) - real(0.5_wp*sigma_dt*vel_${LR}$*F_${LR}$*(1._wp/dz(l)), kind=stp)
                        #:endfor
                    end do
                end do
            end do
    ...
    ! temporary subroutine that duplicates reconstruction logic in s_igr_riemann_solver
