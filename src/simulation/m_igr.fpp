            ...
            do k = 0, n
                do j = -1, m
                    ...
                    #:for LR in ['L', 'R']
                        ...
                        $:GPU_ATOMIC(atomic='update')
                        rhs_vf(sys_idx%E)%sf(j + 1, k, l) = rhs_vf(sys_idx%E)%sf(j + 1, k, &
                            ...
                            do j = -1, m
                                ...
                                do q = vidxb + 1, vidxe
                                    ...
                                    if (num_fluids > 1) then
                                        $:GPU_LOOP(parallelism='[seq]')
                                        do i = 1, num_fluids - 1
                                            alpha_L(i) = alpha_L(i) + coeff_L(q)*q_cons_vf(sys_idx%E + i)%sf(j + q, k, l)
                                        end do
                                    else
                                        alpha_L(1) = 1._wp
                                    end if
                                ...
                                do q = vidxb, vidxe - 1
                                    ...
                                    if (num_fluids > 1) then
                                        $:GPU_LOOP(parallelism='[seq]')
                                        do i = 1, num_fluids - 1
                                            alpha_R(i) = alpha_R(i) + coeff_R(q)*q_cons_vf(sys_idx%E + i)%sf(j + q, k, l)
                                        end do
                                    else
                                        alpha_R(1) = 1._wp
                                    end if
                                ...
                                $:GPU_LOOP(parallelism='[seq]')
                                do q = vidxb + 1, vidxe
                                    E_L = E_L + coeff_L(q)*q_cons_vf(sys_idx%E)%sf(j + q, k, l)
                                end do

                                $:GPU_LOOP(parallelism='[seq]')
                                do q = vidxb, vidxe - 1
                                    E_R = E_R + coeff_R(q)*q_cons_vf(sys_idx%E)%sf(j + q, k, l)
                                end do
                            ...
                            do j = 0, m
                                ...
                                do q = vidxb + 1, vidxe
                                    ...
                                    if (num_fluids > 1) then
                                        $:GPU_LOOP(parallelism='[seq]')
                                        do i = 1, num_fluids - 1
                                            alpha_L(i) = alpha_L(i) + coeff_L(q)*q_cons_vf(sys_idx%E + i)%sf(j, k + q, l)
                                        end do
                                    else
                                        alpha_L(1) = 1._wp
                                    end if
                                ...
                                do q = vidxb, vidxe - 1
                                    ...
                                    if (num_fluids > 1) then
                                        $:GPU_LOOP(parallelism='[seq]')
                                        do i = 1, num_fluids - 1
                                            alpha_R(i) = alpha_R(i) + coeff_R(q)*q_cons_vf(sys_idx%E + i)%sf(j, k + q, l)
                                        end do
                                    else
                                        alpha_R(1) = 1._wp
                                    end if
                                ...
                                if (viscous) then
                                    ...
                                    $:GPU_ATOMIC(atomic='update')
                                    rhs_vf(sys_idx%E)%sf(j, k + 1, l) = rhs_vf(sys_idx%E)%sf(j, k + 1, &
                                ...
                                $:GPU_LOOP(parallelism='[seq]')
                                do q = vidxb + 1, vidxe
                                    E_L = E_L + coeff_L(q)*q_cons_vf(sys_idx%E)%sf(j, k + q, l)
                                    F_L = F_L + coeff_L(q)*jac(j, k + q, l)
                                end do

                                $:GPU_LOOP(parallelism='[seq]')
                                do q = vidxb, vidxe - 1
                                    E_R = E_R + coeff_R(q)*q_cons_vf(sys_idx%E)%sf(j, k + q, l)
                        ...
                        do j = 0, m
                            ...
                            do q = vidxb + 1, vidxe
                                ...
                                if (num_fluids > 1) then
                                    $:GPU_LOOP(parallelism='[seq]')
                                    do i = 1, num_fluids - 1
                                        alpha_L(i) = alpha_L(i) + coeff_L(q)*q_cons_vf(sys_idx%E + i)%sf(j, k, l + q)
                                    end do
                                else
                                    alpha_L(1) = 1._wp
                                end if
                            ...
                            do q = vidxb, vidxe - 1
                                ...
                                if (num_fluids > 1) then
                                    $:GPU_LOOP(parallelism='[seq]')
                                    do i = 1, num_fluids - 1
                                        alpha_R(i) = alpha_R(i) + coeff_R(q)*q_cons_vf(sys_idx%E + i)%sf(j, k, l + q)
                                    end do
                                else
                                    alpha_R(1) = 1._wp
                                end if
                            ...
                            if (viscous) then
                                ...
                                $:GPU_ATOMIC(atomic='update')
                                rhs_vf(sys_idx%E)%sf(j, k, l + 1) = rhs_vf(sys_idx%E)%sf(j, k, &
                            ...
                            $:GPU_LOOP(parallelism='[seq]')
                            do q = vidxb + 1, vidxe
                                E_L = E_L + coeff_L(q)*q_cons_vf(sys_idx%E)%sf(j, k, l + q)
                                F_L = F_L + coeff_L(q)*jac(j, k, l + q)
                            end do

                            $:GPU_LOOP(parallelism='[seq]')
                            do q = vidxb, vidxe - 1
                                E_R = E_R + coeff_R(q)*q_cons_vf(sys_idx%E)%sf(j, k, l + q)
