...
!Coefficient array for polytropic model (pb for each R0 bin accounted for in wght_pb)
    subroutine s_coeff(pres, rho, c, coeffs)
        ...
        real(kind(0._wp)), intent(inout) :: pres, rho, c
        real(kind(0._wp)), dimension(nterms, 0:2, 0:2), intent(out) :: coeffs
    ...
    function f_quad(abscX, abscY, wght_in, q, r, s)
        !$acc routine seq
        real(kind(0._wp)), dimension(nnode, nb), intent(in) :: abscX, abscY, wght_in
    ...
    function f_quad2D(abscX, abscY, wght_in, pow)
        !$acc routine seq
        real(kind(0._wp)), dimension(nnode), intent(in) :: abscX, abscY, wght_in
