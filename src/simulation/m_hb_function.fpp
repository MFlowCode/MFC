...
!> @brief The module contains functions to compute Herschel-Bulkley non-Newtonian viscosity with Papanastasiou regularization. mu =
!! (tau0/gdot)*(1 - exp(-m*gdot)) + K*gdot^(nn-1)
...
contains
    ...
    !> Computes shear rate from strain rate tensor components. gdot = sqrt(2*D_ij*D_ij) where D_ij is the strain rate tensor. Set
    !! D_zz, D_xz, D_yz to 0 for 2D/1D cases.
