        ...
        do ai = 1, num_source
            ...
            if (.not. (sim_time < delay(ai) .and. (pulse(ai) == 1 .or. pulse(ai) == 3))) then
                ...
                do i = 1, num_points
                    ...
                    c = sqrt(small_gamma*(q_prim_vf(sys_idx%E)%sf(j, k, l) + ((small_gamma - 1._wp)/small_gamma)*B_tait)/myRho)
        ...
        do l = 0, p
            do k = 0, n
                do j = 0, m
                    ...
                    rhs_vf(sys_idx%E)%sf(j, k, l) = rhs_vf(sys_idx%E)%sf(j, k, l) + e_src(j, k, l)
                end do
            end do
        end do
        $:END_GPU_PARALLEL_LOOP()
