    ...
    impure subroutine s_acoustic_src_calculations(q_cons_vf, q_prim_vf, t_step, rhs_vf)
        ...
        do ai = 1, num_source
            ...
            if (.not. (sim_time < delay(ai) .and. (pulse(ai) == 1 .or. pulse(ai) == 3))) then
              ...
              #:call GPU_PARALLEL_LOOP(private='[myalpha,myalpha_rho, myRho, B_tait,sim_time, c,  small_gamma, frequency_local, gauss_sigma_time_local, mass_src_diff, mom_src_diff, source_temporal, j, k, l, q ]')
                  do i = 1, num_points
