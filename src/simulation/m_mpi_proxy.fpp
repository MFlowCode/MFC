...
contains
    ...
    impure subroutine s_mpi_bcast_user_inputs()
        ...
        #:for VAR in [ 'run_time_info','cyl_coord', 'mpp_lim',     &
            ...
            & 'hyper_cleaning', 'particles_lagrange' ]
    ...
    contains !f_crosses_boundary(k, 2, -1, pos, posPrev, include_ghost
        ...
        do l = 1, n_neighbors
            ...
            if (p_send_counts(i, j, k) > 0 .and. abs(i) + abs(j) + abs(k) /= 0 .and. abs(i) + abs(j) + abs(k) /= 0) then
                ...
                do q = 0, p_send_counts(i, j, k) - 1
                    ...
                    call MPI_Pack(lag_id(particle_id, 1), 1, MPI_INTEGER, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                    call MPI_Pack(particle_R0(particle_id), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                    ...
                    do r = 1, 2
                        ...
                        ! call MPI_Pack(gas_mv(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                        call MPI_Pack(rad(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                    ...
                    do r = 1, lag_num_ts
                        call MPI_Pack(drad(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                        ! call MPI_Pack(drvel(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                        ! call MPI_Pack(dgasp(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                        ! call MPI_Pack(dgasmv(particle_id, r), 1, mpi_p, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                        call MPI_Pack(dpos(particle_id, :, r), 3, mpi_p, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                        call MPI_Pack(dvel(particle_id, :, r), 3, mpi_p, p_send_buff(send_offset), p_buff_size, position, MPI_COMM_WORLD, ierr)
                    end do
                end do
        ...
        do l = 1, n_neighbors
            ...
            if (p_recv_counts(i, j, k) > 0 .and. abs(i) + abs(j) + abs(k) /= 0) then
                ...
                do q = 0, p_recv_counts(i, j, k) - 1
                    ...
                    call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, lag_id(particle_id, 1), 1, MPI_INTEGER, MPI_COMM_WORLD, ierr)
                    call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, particle_R0(particle_id), 1, mpi_p, MPI_COMM_WORLD, ierr)
                    ...
                    do r = 1, 2
                        ...
                        ! call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, gas_mv(particle_id, r), 1, mpi_p, MPI_COMM_WORLD, ierr)
                        call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, rad(particle_id, r), 1, mpi_p, MPI_COMM_WORLD, ierr)
                    ...
                    do r = 1, lag_num_ts
                        call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, drad(particle_id, r), 1, mpi_p, MPI_COMM_WORLD, ierr)
                        ! call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, drvel(particle_id, r), 1, mpi_p, MPI_COMM_WORLD, ierr)
                        ! call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, dgasp(particle_id, r), 1, mpi_p, MPI_COMM_WORLD, ierr)
                        ! call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, dgasmv(particle_id, r), 1, mpi_p, MPI_COMM_WORLD, ierr)
                        call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, dpos(particle_id, :, r), 3, mpi_p, MPI_COMM_WORLD, ierr)
                        call MPI_Unpack(p_recv_buff(recv_offset), p_recv_size, position, dvel(particle_id, :, r), 3, mpi_p, MPI_COMM_WORLD, ierr)
                    end do
    ...
    impure subroutine s_transfer_collision_forces(total_recv, force_recv_ids, force_recv_vals)
        ...
        do l = 1, n_neighbors
            ...
            recv_count = recv_count + 1
            call MPI_Irecv(force_recv_counts(partner), 1, MPI_INTEGER, partner, recv_tag, &
                           MPI_COMM_WORLD, recv_requests(recv_count), ierr)

            send_count = send_count + 1
            call MPI_Isend(force_send_counts(partner), 1, MPI_INTEGER, partner, send_tag, &
                           MPI_COMM_WORLD, send_requests(send_count), ierr)
        end do
        ...
        do l = 1, n_neighbors
            ...
            if (force_recv_counts(partner) > 0) then
                recv_count = recv_count + 1
                call MPI_Irecv(force_recv_ids(recv_displs(partner) + 1), force_recv_counts(partner), MPI_INTEGER, &
                               partner, recv_tag, MPI_COMM_WORLD, recv_requests(recv_count), ierr)
                recv_count = recv_count + 1
                call MPI_Irecv(force_recv_vals(recvdispls_vals(partner) + 1), recvcounts_vals(partner), mpi_p, &
                               partner, recv_tag + 1, MPI_COMM_WORLD, recv_requests(recv_count), ierr)
            end if

            if (force_send_counts(partner) > 0) then
                send_count = send_count + 1
                call MPI_Isend(flat_send_ids(send_displs(partner) + 1), force_send_counts(partner), MPI_INTEGER, &
                               partner, send_tag, MPI_COMM_WORLD, send_requests(send_count), ierr)
                send_count = send_count + 1
                call MPI_Isend(flat_send_vals(senddispls_vals(partner) + 1), sendcounts_vals(partner), mpi_p, &
                               partner, send_tag + 1, MPI_COMM_WORLD, send_requests(send_count), ierr)
            end if
        end do
