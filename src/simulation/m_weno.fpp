...
contains
    ...
    !> One cell of the WENO5 reconstruction in direction ${XYZ}$: the body of the s_weno kernel, moved into a device routine so that
    !! dvd/poly/beta/alpha/omega/delta are routine LOCALS. As `private` entities of the target region each of the six cost ~31 us
    !! per launch on amdflang (descriptor materialisation, measured amr-bench/nowait_probe/descr.f90: 265 vs 85 us); as locals of a
    !! called device routine they cost nothing. The arithmetic is the kernel's, statement for statement.
    #:for WENO_DIR, XYZ, STENCIL_VAR, COORDS in &
        [(1, 'x', 'j', '{STENCIL_IDX}, k, l'), (2, 'y', 'k', 'j, {STENCIL_IDX}, l'), (3, 'z', 'l', 'j, k, {STENCIL_IDX}')]
        #:set SV = STENCIL_VAR
        ...
        subroutine s_weno5_cell_${XYZ}$ (j, k, l, i, vL, vR)
            ...
            poly(0) = vp0 + poly_coef_cbL_${XYZ}$ (${SV}$, 0, 0)*dvd(1) + poly_coef_cbL_${XYZ}$ (${SV}$, 0, 1)*dvd(0)
            poly(1) = vp0 + poly_coef_cbL_${XYZ}$ (${SV}$, 1, 0)*dvd(0) + poly_coef_cbL_${XYZ}$ (${SV}$, 1, 1)*dvd(-1)
            poly(2) = vp0 + poly_coef_cbL_${XYZ}$ (${SV}$, 2, 0)*dvd(-1) + poly_coef_cbL_${XYZ}$ (${SV}$, 2, 1)*dvd(-2)

            if (uniform_grid(${WENO_DIR}$)) then
                beta(0) = 13._wp/12._wp*(dvd(1) - dvd(0))**2 + 0.25_wp*(dvd(1) - 3._wp*dvd(0))**2 + weno_eps
                beta(1) = 13._wp/12._wp*(dvd(0) - dvd(-1))**2 + 0.25_wp*(dvd(0) + dvd(-1))**2 + weno_eps
                beta(2) = 13._wp/12._wp*(dvd(-1) - dvd(-2))**2 + 0.25_wp*(3._wp*dvd(-1) - dvd(-2))**2 + weno_eps
            else
            ...
            vL = omega(0)*poly(0) + omega(1)*poly(1) + omega(2)*poly(2)

            ! reconstruct from right side

            poly(0) = vp0 + poly_coef_cbR_${XYZ}$ (${SV}$, 0, 0)*dvd(1) + poly_coef_cbR_${XYZ}$ (${SV}$, 0, 1)*dvd(0)
            poly(1) = vp0 + poly_coef_cbR_${XYZ}$ (${SV}$, 1, 0)*dvd(0) + poly_coef_cbR_${XYZ}$ (${SV}$, 1, 1)*dvd(-1)
            poly(2) = vp0 + poly_coef_cbR_${XYZ}$ (${SV}$, 2, 0)*dvd(-1) + poly_coef_cbR_${XYZ}$ (${SV}$, 2, 1)*dvd(-2)
            ...
            omega(1) = alpha(1)/(alpha(0) + alpha(1) + alpha(2))
            omega(2) = alpha(2)/(alpha(0) + alpha(1) + alpha(2))
            vR = omega(0)*poly(0) + omega(1)*poly(1) + omega(2)*poly(2)

        end subroutine s_weno5_cell_${XYZ}$
    #:endfor
