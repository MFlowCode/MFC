                ...
                if (weno_order == 3) then
                    ...
                    do i = is%beg - 1 + weno_polyn, is%end - 1 - weno_polyn
                        ...
                        poly_coef_cbR_${XYZ}$ (i + 1, 1, &
                            & 0) = ((s_cb(i - 1) - s_cb(i + 1))*(s_cb(i + 1) - s_cb(i)))/((s_cb(i - 1) - s_cb(i + 2))*(s_cb(i + 2) &
                            & - s_cb(i)))
                        poly_coef_cbR_${XYZ}$ (i + 1, 1, &
                            & 1) = ((s_cb(i) - s_cb(i + 1))*(s_cb(i + 1) - s_cb(i + 2)))/((s_cb(i - 1) - s_cb(i + 1))*(s_cb(i - 1) &
                            & - s_cb(i + 2)))
                        ...
                        poly_coef_cbL_${XYZ}$ (i + 1, 0, &
                            & 1) = ((s_cb(i) - s_cb(i + 2)) + (s_cb(i) - s_cb(i + 3)))/((s_cb(i) - s_cb(i + 2))*(s_cb(i) - s_cb(i &
                            & + 3)))*((s_cb(i + 1) - s_cb(i)))
                        poly_coef_cbL_${XYZ}$ (i + 1, 2, &
                            & 0) = ((s_cb(i - 2) - s_cb(i)) + (s_cb(i - 1) - s_cb(i + 1)))/((s_cb(i - 2) - s_cb(i + 1))*(s_cb(i &
                            & + 1) - s_cb(i - 1)))*((s_cb(i) - s_cb(i + 1)))

                        d_cbR_${XYZ}$ (0, &
                            & i + 1) = ((s_cb(i - 2) - s_cb(i + 1))*(s_cb(i + 1) - s_cb(i - 1)))/((s_cb(i - 2) - s_cb(i + 3)) &
                            & *(s_cb(i + 3) - s_cb(i - 1)))
                        d_cbR_${XYZ}$ (2, &
                            & i + 1) = ((s_cb(i + 1) - s_cb(i + 2))*(s_cb(i + 1) - s_cb(i + 3)))/((s_cb(i - 2) - s_cb(i + 2)) &
                            & *(s_cb(i - 2) - s_cb(i + 3)))
                        d_cbL_${XYZ}$ (0, &
                        ...
                        beta_coef_${XYZ}$ (i + 1, 1, &
                            & 0) = 4._wp*(s_cb(i) - s_cb(i + 1))**2._wp*(10._wp*(s_cb(i + 1) - s_cb(i))**2._wp + (s_cb(i) &
                            & - s_cb(i - 1))**2._wp + (s_cb(i) - s_cb(i - 1))*(s_cb(i + 1) - s_cb(i)))/((s_cb(i - 1) - s_cb(i + 2) &
                        ...
                        beta_coef_${XYZ}$ (i + 1, 2, &
                            & 2) = 4._wp*(s_cb(i) - s_cb(i + 1))**2._wp*(10._wp*(s_cb(i + 1) - s_cb(i))**2._wp + (s_cb(i) &
                            & - s_cb(i - 1))**2._wp + (s_cb(i) - s_cb(i - 1))*(s_cb(i + 1) - s_cb(i)))/((s_cb(i - 2) - s_cb(i)) &
                    ...
                    if (.not. teno) then
                        do i = is%beg - 1 + weno_polyn, is%end - 1 - weno_polyn
                            ...
                            ! To reduce computational cost, we leverage the fact that all polynomial coefficients in a stencil sum
                            ! to 1
                            ! and compute the polynomial coefficients (poly_coef_cb) for the cell value differences (dvd) instead of
                            ! the values themselves.
                            ! The computation of coefficients is further simplified by using grid spacing (y or w) rather than the
                            ! grid locations (s_cb) directly.
                            ! Ideal weights (d_cb) are obtained by comparing the grid location coefficients of the polynomial
                            ! coefficients.
                            ! The smoothness indicators (beta_coef) are calculated through numerical differentiation and integration
                            ! of each cross term of the polynomial coefficients,
                            ! using the cell value differences (dvd) instead of the values themselves.
                            ! While the polynomial coefficients sum to 1, the derivative of 1 is 0, which means it does not create
                            ! additional cross terms in the smoothness indicators.

                            w = s_cb(i - 3:i + 4) - s_cb(i) ! Offset using s_cb(i) to reduce floating point error
                            d_cbR_${XYZ}$ (0, &
                                & i + 1) = ((w(5) - w(6))*(w(5) - w(7))*(w(5) - w(8)))/((w(1) - w(6))*(w(1) - w(7))*(w(1) - w(8))) &
                                & !&
                            d_cbR_${XYZ}$ (1, &
                                & i + 1) = ((w(1) - w(5))*(w(5) - w(7))*(w(5) - w(8))*(w(1)*w(2) - w(1)*w(6) - w(1)*w(7) - w(2) &
                                & *w(6) - w(1)*w(8) - w(2)*w(7) - w(2)*w(8) + w(6)*w(7) + w(6)*w(8) + w(7)*w(8) + w(1)**2 + w(2) &
                                & **2))/((w(1) - w(6))*(w(1) - w(7))*(w(1) - w(8))*(w(2) - w(7))*(w(2) - w(8))) !&
                            d_cbR_${XYZ}$ (2, &
                                & i + 1) = ((w(1) - w(5))*(w(2) - w(5))*(w(5) - w(8))*(w(1)*w(2) + w(1)*w(3) + w(2)*w(3) - w(1) &
                                & *w(7) - w(1)*w(8) - w(2)*w(7) - w(2)*w(8) - w(3)*w(7) - w(3)*w(8) + w(7)*w(8) + w(7)**2 + w(8) &
                                & **2))/((w(1) - w(7))*(w(1) - w(8))*(w(2) - w(7))*(w(2) - w(8))*(w(3) - w(8))) !&
                            d_cbR_${XYZ}$ (3, &
                                & i + 1) = ((w(1) - w(5))*(w(2) - w(5))*(w(3) - w(5)))/((w(1) - w(8))*(w(2) - w(8))*(w(3) - w(8))) &
                                & !&
                            ...
                            poly_coef_cbR_${XYZ}$ (i + 1, 0, &
                                & 1) = -(y(1)*y(2)*(3*y(2)**2 + 6*y(2)*y(3) + 3*y(2)*y(4) + 2*y(1)*y(2) + 3*y(3)**2 + 3*y(3)*y(4) &
                                & + 2*y(1)*y(3) + y(4)**2 + y(1)*y(4)))/((y(2) + y(3))*(y(1) + y(2) + y(3))*(y(2) + y(3) + y(4)) &
                                & *(y(1) + y(2) + y(3) + y(4))) !&
                            poly_coef_cbR_${XYZ}$ (i + 1, 0, &
                                & 2) = (y(1)*(y(1)**2 + 3*y(1)*y(2) + 2*y(1)*y(3) + y(4)*y(1) + 3*y(2)**2 + 4*y(2)*y(3) + 2*y(4) &
                                & *y(2) + y(3)**2 + y(4)*y(3)))/((y(1) + y(2))*(y(1) + y(2) + y(3))*(y(1) + y(2) + y(3) + y(4))) !&

                            y = s_cb(i:i + 3) - s_cb(i - 1:i + 2)
                            poly_coef_cbR_${XYZ}$ (i + 1, 1, &
                                & 0) = -(y(2)*y(3)*(y(1) + y(2)))/((y(3) + y(4))*(y(2) + y(3) + y(4))*(y(1) + y(2) + y(3) + y(4))) &
                                & !&
                            poly_coef_cbR_${XYZ}$ (i + 1, 1, &
                                & 1) = (y(2)*(y(1) + y(2))*(y(2)**2 + 4*y(2)*y(3) + 2*y(2)*y(4) + y(1)*y(2) + 3*y(3)**2 + 3*y(3) &
                            ...
                            poly_coef_cbR_${XYZ}$ (i + 1, 2, &
                                & 0) = (y(3)*(y(2) + y(3))*(y(1) + y(2) + y(3)))/((y(3) + y(4))*(y(2) + y(3) + y(4))*(y(1) + y(2) &
                                & + y(3) + y(4))) !&
                            poly_coef_cbR_${XYZ}$ (i + 1, 2, &
                                ...
                                & + y(4))*(y(1) + y(2) + y(3) + y(4))) !&
                            poly_coef_cbR_${XYZ}$ (i + 1, 2, &
                                & 2) = -(y(3)*y(4)*(y(2) + y(3)))/((y(1) + y(2))*(y(1) + y(2) + y(3))*(y(1) + y(2) + y(3) + y(4))) &
                                & !&

                            y = s_cb(i - 2:i + 1) - s_cb(i - 3:i)
                            poly_coef_cbR_${XYZ}$ (i + 1, 3, &
                                & 0) = (y(4)*(y(2)**2 + 4*y(2)*y(3) + 4*y(2)*y(4) + y(1)*y(2) + 3*y(3)**2 + 6*y(3)*y(4) + 2*y(1) &
                                & *y(3) + 3*y(4)**2 + 2*y(1)*y(4)))/((y(3) + y(4))*(y(2) + y(3) + y(4))*(y(1) + y(2) + y(3) + y(4) &
                                & )) !&
                            poly_coef_cbR_${XYZ}$ (i + 1, 3, &
                            ...
                            poly_coef_cbR_${XYZ}$ (i + 1, 3, &
                                & 2) = (y(4)*(y(3) + y(4))*(y(2) + y(3) + y(4)))/((y(1) + y(2))*(y(1) + y(2) + y(3))*(y(1) + y(2) &
                                & + y(3) + y(4))) !&
                            ...
                            poly_coef_cbL_${XYZ}$ (i + 1, 3, &
                                ...
                                & *(y(1) + y(2) + y(3) + y(4))) !&
                            poly_coef_cbL_${XYZ}$ (i + 1, 3, &
                                & 0) = (y(1)*(y(1)**2 + 3*y(1)*y(2) + 2*y(1)*y(3) + y(4)*y(1) + 3*y(2)**2 + 4*y(2)*y(3) + 2*y(4) &
                                & *y(2) + y(3)**2 + y(4)*y(3)))/((y(1) + y(2))*(y(1) + y(2) + y(3))*(y(1) + y(2) + y(3) + y(4))) !&

                            y = s_cb(i + 2:i - 1:-1) - s_cb(i + 1:i - 2:-1)
                            poly_coef_cbL_${XYZ}$ (i + 1, 2, &
                                & 2) = -(y(2)*y(3)*(y(1) + y(2)))/((y(3) + y(4))*(y(2) + y(3) + y(4))*(y(1) + y(2) + y(3) + y(4))) &
                                & !&
                            poly_coef_cbL_${XYZ}$ (i + 1, 2, &
                                & 1) = (y(2)*(y(1) + y(2))*(y(2)**2 + 4*y(2)*y(3) + 2*y(2)*y(4) + y(1)*y(2) + 3*y(3)**2 + 3*y(3) &
                                & *y(4) + 2*y(1)*y(3) + y(4)**2 + y(1)*y(4)))/((y(2) + y(3))*(y(1) + y(2) + y(3))*(y(2) + y(3) &
                            ...
                            poly_coef_cbL_${XYZ}$ (i + 1, 1, &
                                & 2) = (y(3)*(y(2) + y(3))*(y(1) + y(2) + y(3)))/((y(3) + y(4))*(y(2) + y(3) + y(4))*(y(1) + y(2) &
                                & + y(3) + y(4))) !&
                            poly_coef_cbL_${XYZ}$ (i + 1, 1, &
                                & 1) = (y(3)*y(4)*(y(1)**2 + 3*y(1)*y(2) + 3*y(1)*y(3) + y(4)*y(1) + 3*y(2)**2 + 6*y(2)*y(3) &
                                & + 2*y(4)*y(2) + 3*y(3)**2 + 2*y(4)*y(3)))/((y(2) + y(3))*(y(1) + y(2) + y(3))*(y(2) + y(3) &
                                & + y(4))*(y(1) + y(2) + y(3) + y(4))) !&
                            poly_coef_cbL_${XYZ}$ (i + 1, 1, &
                                & 0) = -(y(3)*y(4)*(y(2) + y(3)))/((y(1) + y(2))*(y(1) + y(2) + y(3))*(y(1) + y(2) + y(3) + y(4))) &
                                & !&

                            y = s_cb(i + 4:i + 1:-1) - s_cb(i + 3:i:-1)
                            poly_coef_cbL_${XYZ}$ (i + 1, 0, &
                                & 2) = (y(4)*(y(2)**2 + 4*y(2)*y(3) + 4*y(2)*y(4) + y(1)*y(2) + 3*y(3)**2 + 6*y(3)*y(4) + 2*y(1) &
                                & *y(3) + 3*y(4)**2 + 2*y(1)*y(4)))/((y(3) + y(4))*(y(2) + y(3) + y(4))*(y(1) + y(2) + y(3) + y(4) &
                                & )) !&
                            poly_coef_cbL_${XYZ}$ (i + 1, 0, &
                                & 1) = -(y(4)*(y(3) + y(4))*(y(1)**2 + 3*y(1)*y(2) + 3*y(1)*y(3) + 2*y(1)*y(4) + 3*y(2)**2 &
                                ...
                                & + y(3))*(y(2) + y(3) + y(4))*(y(1) + y(2) + y(3) + y(4))) !&
                            poly_coef_cbL_${XYZ}$ (i + 1, 0, &
                                & 0) = (y(4)*(y(3) + y(4))*(y(2) + y(3) + y(4)))/((y(1) + y(2))*(y(1) + y(2) + y(3))*(y(1) + y(2) &
                                & + y(3) + y(4))) !&
                            ...
                            beta_coef_${XYZ}$ (i + 1, 3, &
                                ...
                                & + 45*y(1)*y(2)**2*y(4) + 110*y(1)*y(2)*y(3)**2 + 165*y(1)*y(2)*y(3)*y(4) + 260*y(1)*y(2)*y(4) &
                                & **2 + 60*y(1)*y(3)**3 + 135*y(1)*y(3)**2*y(4) + 400*y(1)*y(3)*y(4)**2 + 225*y(1)*y(4)**3 &
                                & + 5*y(2)**4 + 40*y(2)**3*y(3) + 30*y(2)**3*y(4) + 110*y(2)**2*y(3)**2 + 165*y(2)**2*y(3)*y(4) &
                                & + 260*y(2)**2*y(4)**2 + 120*y(2)*y(3)**3 + 270*y(2)*y(3)**2*y(4) + 800*y(2)*y(3)*y(4)**2 &
                                & + 450*y(2)*y(4)**3 + 45*y(3)**4 + 135*y(3)**3*y(4) + 600*y(3)**2*y(4)**2 + 675*y(3)*y(4)**3 &
                                & + 996*y(4)**4))/(5*(y(3) + y(4))**2*(y(2) + y(3) + y(4))**2*(y(1) + y(2) + y(3) + y(4))**2) !&
                            beta_coef_${XYZ}$ (i + 1, 3, &
                                ...
                                & **2*y(3)**3 + 165*y(1)**2*y(3)**2*y(4) + 790*y(1)**2*y(3)*y(4)**2 + 415*y(1)**2*y(4)**3 &
                                & + 60*y(1)*y(2)**3*y(3) + 30*y(1)*y(2)**3*y(4) + 270*y(1)*y(2)**2*y(3)**2 + 315*y(1)*y(2)**2*y(3) &
                                & *y(4) + 975*y(1)*y(2)**2*y(4)**2 + 360*y(1)*y(2)*y(3)**3 + 645*y(1)*y(2)*y(3)**2*y(4) &
                                & + 2850*y(1)*y(2)*y(3)*y(4)**2 + 1460*y(1)*y(2)*y(4)**3 + 150*y(1)*y(3)**4 + 360*y(1)*y(3) &
                                & **3*y(4) + 2000*y(1)*y(3)**2*y(4)**2 + 2005*y(1)*y(3)*y(4)**3 + 2077*y(1)*y(4)**4 + 30*y(2) &
                                & **4*y(3) + 15*y(2)**4*y(4) + 180*y(2)**3*y(3)**2 + 210*y(2)**3*y(3)*y(4) + 650*y(2)**3*y(4)**2 &
                                & + 360*y(2)**2*y(3)**3 + 645*y(2)**2*y(3)**2*y(4) + 2850*y(2)**2*y(3)*y(4)**2 + 1460*y(2)**2*y(4) &
                                & **3 + 300*y(2)*y(3)**4 + 720*y(2)*y(3)**3*y(4) + 4000*y(2)*y(3)**2*y(4)**2 + 4010*y(2)*y(3)*y(4) &
                                & **3 + 4154*y(2)*y(4)**4 + 90*y(3)**5 + 270*y(3)**4*y(4) + 1800*y(3)**3*y(4)**2 + 2655*y(3) &
                                & **2*y(4)**3 + 4464*y(3)*y(4)**4 + 1767*y(4)**5))/(5*(y(2) + y(3))*(y(3) + y(4))*(y(1) + y(2) &
                                & + y(3))*(y(2) + y(3) + y(4))**2*(y(1) + y(2) + y(3) + y(4))**2) !&
                            beta_coef_${XYZ}$ (i + 1, 3, &
                            ...
                            beta_coef_${XYZ}$ (i + 1, 3, &
                                ...
                                & + 900*y(1)*y(2)**3*y(4)**2 + 270*y(1)*y(2)**2*y(3)**3 + 405*y(1)*y(2)**2*y(3)**2*y(4) &
                                & + 3735*y(1)*y(2)**2*y(3)*y(4)**2 + 1800*y(1)*y(2)**2*y(4)**3 + 270*y(1)*y(2)*y(3)**4 + 540*y(1) &
                                & *y(2)*y(3)**3*y(4) + 5025*y(1)*y(2)*y(3)**2*y(4)**2 + 4755*y(1)*y(2)*y(3)*y(4)**3 + 4224*y(1) &
                                & *y(2)*y(4)**4 + 90*y(1)*y(3)**5 + 225*y(1)*y(3)**4*y(4) + 2190*y(1)*y(3)**3*y(4)**2 + 3060*y(1) &
                                & *y(3)**2*y(4)**3 + 4529*y(1)*y(3)*y(4)**4 + 1762*y(1)*y(4)**5 + 45*y(2)**4*y(3)**2 + 45*y(2) &
                                & **4*y(3)*y(4) + 450*y(2)**4*y(4)**2 + 180*y(2)**3*y(3)**3 + 270*y(2)**3*y(3)**2*y(4) + 2490*y(2) &
                                & **3*y(3)*y(4)**2 + 1200*y(2)**3*y(4)**3 + 270*y(2)**2*y(3)**4 + 540*y(2)**2*y(3)**3*y(4) &
                                & + 5025*y(2)**2*y(3)**2*y(4)**2 + 4755*y(2)**2*y(3)*y(4)**3 + 4224*y(2)**2*y(4)**4 + 180*y(2) &
                                & *y(3)**5 + 450*y(2)*y(3)**4*y(4) + 4380*y(2)*y(3)**3*y(4)**2 + 6120*y(2)*y(3)**2*y(4)**3 &
                                & + 9058*y(2)*y(3)*y(4)**4 + 3524*y(2)*y(4)**5 + 45*y(3)**6 + 135*y(3)**5*y(4) + 1395*y(3)**4*y(4) &
                                & **2 + 2565*y(3)**3*y(4)**3 + 4884*y(3)**2*y(4)**4 + 3624*y(3)*y(4)**5 + 831*y(4)**6))/(5*(y(2) &
                                & + y(3))**2*(y(1) + y(2) + y(3))**2*(y(2) + y(3) + y(4))**2*(y(1) + y(2) + y(3) + y(4))**2) !&
                            beta_coef_${XYZ}$ (i + 1, 3, &
                                & 4) = -(4*y(4)**2*(10*y(1)**2*y(2)*y(3)**2 + 10*y(1)**2*y(2)*y(3)*y(4) + 100*y(1)**2*y(2)*y(4) &
                                & **2 + 10*y(1)**2*y(3)**3 + 15*y(1)**2*y(3)**2*y(4) + 205*y(1)**2*y(3)*y(4)**2 + 100*y(1)**2*y(4) &
                                & **3 + 30*y(1)*y(2)**2*y(3)**2 + 30*y(1)*y(2)**2*y(3)*y(4) + 300*y(1)*y(2)**2*y(4)**2 + 60*y(1) &
                                & *y(2)*y(3)**3 + 90*y(1)*y(2)*y(3)**2*y(4) + 1030*y(1)*y(2)*y(3)*y(4)**2 + 500*y(1)*y(2)*y(4)**3 &
                                ...
                                & + 3624*y(2)*y(4)**4 + 30*y(3)**5 + 75*y(3)**4*y(4) + 1060*y(3)**3*y(4)**2 + 1515*y(3)**2*y(4) &
                                & **3 + 3824*y(3)*y(4)**4 + 1662*y(4)**5))/(5*(y(1) + y(2))*(y(2) + y(3))*(y(1) + y(2) + y(3)) &
                                & **2*(y(2) + y(3) + y(4))*(y(1) + y(2) + y(3) + y(4))**2) !&
                            beta_coef_${XYZ}$ (i + 1, 3, &
                            ...
                            beta_coef_${XYZ}$ (i + 1, 2, &
                                ...
                                & )**2*(y(1) + y(2) + y(3) + y(4))**2) !&
                            beta_coef_${XYZ}$ (i + 1, 2, &
                                & 1) = (4*y(3)**2*(5*y(1)**3*y(2)*y(3) + 10*y(1)**3*y(2)*y(4) - 95*y(1)**3*y(3)**2 + 5*y(1) &
                                & **3*y(3)*y(4) + 20*y(1)**2*y(2)**2*y(3) + 40*y(1)**2*y(2)**2*y(4) - 465*y(1)**2*y(2)*y(3)**2 &
                                & + 55*y(1)**2*y(2)*y(3)*y(4) + 10*y(1)**2*y(2)*y(4)**2 - 285*y(1)**2*y(3)**3 + 20*y(1)**2*y(3) &
                                & **2*y(4) + 5*y(1)**2*y(3)*y(4)**2 + 30*y(1)*y(2)**3*y(3) + 60*y(1)*y(2)**3*y(4) - 825*y(1)*y(2) &
                                & **2*y(3)**2 + 135*y(1)*y(2)**2*y(3)*y(4) + 30*y(1)*y(2)**2*y(4)**2 - 1040*y(1)*y(2)*y(3)**3 &
                                & + 100*y(1)*y(2)*y(3)**2*y(4) + 35*y(1)*y(2)*y(3)*y(4)**2 - 1847*y(1)*y(3)**4 + 125*y(1)*y(3) &
                                & **3*y(4) + 110*y(1)*y(3)**2*y(4)**2 + 15*y(2)**4*y(3) + 30*y(2)**4*y(4) - 550*y(2)**3*y(3)**2 &
                                & + 90*y(2)**3*y(3)*y(4) + 20*y(2)**3*y(4)**2 - 1040*y(2)**2*y(3)**3 + 100*y(2)**2*y(3)**2*y(4) &
                                & + 35*y(2)**2*y(3)*y(4)**2 - 3694*y(2)*y(3)**4 + 250*y(2)*y(3)**3*y(4) + 220*y(2)*y(3)**2*y(4) &
                                & **2 - 3219*y(3)**5 - 1452*y(3)**4*y(4) + 105*y(3)**3*y(4)**2))/(5*(y(2) + y(3))*(y(3) + y(4)) &
                            ...
                            beta_coef_${XYZ}$ (i + 1, 2, &
                                ...
                                & + 4224*y(2)**2*y(3)**4 - 120*y(2)**2*y(3)**3*y(4) + 25*y(2)**2*y(3)**2*y(4)**2 + 165*y(2) &
                                & **2*y(3)*y(4)**3 + 20*y(2)**2*y(4)**4 + 6648*y(2)*y(3)**5 + 2814*y(2)*y(3)**4*y(4) - 200*y(2) &
                                & *y(3)**3*y(4)**2 + 140*y(2)*y(3)**2*y(4)**3 + 30*y(2)*y(3)*y(4)**4 + 3174*y(3)**6 + 3039*y(3) &
                                & **5*y(4) + 771*y(3)**4*y(4)**2 + 135*y(3)**3*y(4)**3 + 60*y(3)**2*y(4)**4))/(5*(y(2) + y(3)) &
                                & **2*(y(1) + y(2) + y(3))**2*(y(2) + y(3) + y(4))**2*(y(1) + y(2) + y(3) + y(4))**2) !&
                            beta_coef_${XYZ}$ (i + 1, 2, &
                                & 4) = -(4*y(3)**2*(100*y(1)**2*y(2)*y(3)**2 + 10*y(1)**2*y(2)*y(3)*y(4) + 10*y(1)**2*y(2)*y(4) &
                                & **2 - 95*y(1)**2*y(3)**2*y(4) + 5*y(1)**2*y(3)*y(4)**2 + 300*y(1)*y(2)**2*y(3)**2 + 30*y(1)*y(2) &
                            ...
                            beta_coef_${XYZ}$ (i + 1, 1, &
                                ...
                                & + 3174*y(2)**6 + 6648*y(2)**5*y(3) + 3324*y(2)**5*y(4) + 4224*y(2)**4*y(3)**2 + 4224*y(2) &
                                & **4*y(3)*y(4) + 1081*y(2)**4*y(4)**2 + 1200*y(2)**3*y(3)**3 + 1800*y(2)**3*y(3)**2*y(4) &
                                & + 1000*y(2)**3*y(3)*y(4)**2 + 200*y(2)**3*y(4)**3 + 450*y(2)**2*y(3)**4 + 900*y(2)**2*y(3) &
                                & **3*y(4) + 750*y(2)**2*y(3)**2*y(4)**2 + 300*y(2)**2*y(3)*y(4)**3 + 50*y(2)**2*y(4)**4)) &
                                & /(5*(y(2) + y(3))**2*(y(1) + y(2) + y(3))**2*(y(2) + y(3) + y(4))**2*(y(1) + y(2) + y(3) + y(4)) &
                                & **2) !&
                            beta_coef_${XYZ}$ (i + 1, 1, &
                                ...
                                & + 20*y(1)*y(2)**2*y(4)**2 + 90*y(1)*y(2)*y(3)**3 + 135*y(1)*y(2)*y(3)**2*y(4) + 55*y(1)*y(2) &
                                & *y(3)*y(4)**2 + 5*y(1)*y(2)*y(4)**3 + 30*y(1)*y(3)**4 + 60*y(1)*y(3)**3*y(4) + 40*y(1)*y(3) &
                                & **2*y(4)**2 + 10*y(1)*y(3)*y(4)**3 - 3219*y(2)**5 - 3694*y(2)**4*y(3) - 1847*y(2)**4*y(4) &
                                & - 1040*y(2)**3*y(3)**2 - 1040*y(2)**3*y(3)*y(4) - 285*y(2)**3*y(4)**2 - 550*y(2)**2*y(3)**3 &
                                & - 825*y(2)**2*y(3)**2*y(4) - 465*y(2)**2*y(3)*y(4)**2 - 95*y(2)**2*y(4)**3 + 15*y(2)*y(3)**4 &
                                & + 30*y(2)*y(3)**3*y(4) + 20*y(2)*y(3)**2*y(4)**2 + 5*y(2)*y(3)*y(4)**3))/(5*(y(1) + y(2))*(y(2) &
                                & + y(3))*(y(1) + y(2) + y(3))**2*(y(2) + y(3) + y(4))*(y(1) + y(2) + y(3) + y(4))**2) !&
                            beta_coef_${XYZ}$ (i + 1, 1, &
                            ...
                            beta_coef_${XYZ}$ (i + 1, 0, &
                                ...
                                & + y(3) + y(4))**2*(y(1) + y(2) + y(3) + y(4))**2) !&
                            beta_coef_${XYZ}$ (i + 1, 0, &
                                & 2) = (4*y(1)**2*(1767*y(1)**4 + 725*y(1)**3*y(2) + 415*y(1)**3*y(3) + 105*y(4)*y(1)**3 &
                                & + 665*y(1)**2*y(2)**2 + 775*y(1)**2*y(2)*y(3) + 220*y(4)*y(1)**2*y(2) + 215*y(1)**2*y(3)**2 &
                                & + 110*y(4)*y(1)**2*y(3) + 75*y(1)*y(2)**3 + 130*y(1)*y(2)**2*y(3) + 35*y(4)*y(1)*y(2)**2 &
                                & + 60*y(1)*y(2)*y(3)**2 + 35*y(4)*y(1)*y(2)*y(3) + 5*y(1)*y(3)**3 + 5*y(4)*y(1)*y(3)**2 + 30*y(2) &
                                & **4 + 70*y(2)**3*y(3) + 20*y(4)*y(2)**3 + 50*y(2)**2*y(3)**2 + 30*y(4)*y(2)**2*y(3) + 10*y(2) &
                                & *y(3)**3 + 10*y(4)*y(2)*y(3)**2))/(5*(y(1) + y(2))*(y(3) + y(4))*(y(1) + y(2) + y(3))*(y(2) &
                            ...
                            beta_coef_${XYZ}$ (i + 1, 0, &
                                ...
                                & + 1460*y(1)**3*y(3)*y(4) + 415*y(1)**3*y(4)**2 + 1800*y(1)**2*y(2)**3 + 4000*y(1)**2*y(2) &
                                & **2*y(3) + 2000*y(1)**2*y(2)**2*y(4) + 2850*y(1)**2*y(2)*y(3)**2 + 2850*y(1)**2*y(2)*y(3)*y(4) &
                                & + 790*y(1)**2*y(2)*y(4)**2 + 650*y(1)**2*y(3)**3 + 975*y(1)**2*y(3)**2*y(4) + 535*y(1)**2*y(3) &
                                & *y(4)**2 + 105*y(1)**2*y(4)**3 + 270*y(1)*y(2)**4 + 720*y(1)*y(2)**3*y(3) + 360*y(1)*y(2) &
                                & **3*y(4) + 645*y(1)*y(2)**2*y(3)**2 + 645*y(1)*y(2)**2*y(3)*y(4) + 165*y(1)*y(2)**2*y(4)**2 &
                                & + 210*y(1)*y(2)*y(3)**3 + 315*y(1)*y(2)*y(3)**2*y(4) + 155*y(1)*y(2)*y(3)*y(4)**2 + 25*y(1)*y(2) &
                                & *y(4)**3 + 15*y(1)*y(3)**4 + 30*y(1)*y(3)**3*y(4) + 20*y(1)*y(3)**2*y(4)**2 + 5*y(1)*y(3)*y(4) &
                                & **3 + 90*y(2)**5 + 300*y(2)**4*y(3) + 150*y(2)**4*y(4) + 360*y(2)**3*y(3)**2 + 360*y(2)**3*y(3) &
                                & *y(4) + 90*y(2)**3*y(4)**2 + 180*y(2)**2*y(3)**3 + 270*y(2)**2*y(3)**2*y(4) + 130*y(2)**2*y(3) &
                                & *y(4)**2 + 20*y(2)**2*y(4)**3 + 30*y(2)*y(3)**4 + 60*y(2)*y(3)**3*y(4) + 40*y(2)*y(3)**2*y(4) &
                                & **2 + 10*y(2)*y(3)*y(4)**3))/(5*(y(1) + y(2))*(y(2) + y(3))*(y(1) + y(2) + y(3))**2*(y(2) + y(3) &
                                    ...
                                    do i = 1, v_size
                                        ...
                                        if (.not. teno) then
                                            ...
                                            poly(1) = v_rs_ws_${XYZ}$ (j, k, l, i) + poly_coef_cbL_${XYZ}$ (j, 2, &
                                                & 0)*dvd(0) + poly_coef_cbL_${XYZ}$ (j, 2, 1)*dvd(-1) + poly_coef_cbL_${XYZ}$ (j, &
                                                & 2, 2)*dvd(-2)
                                            ...
                                            #:if not MFC_CASE_OPTIMIZATION or weno_num_stencils > 3
                                                ...
                                                ! Here we need the left-sided flux, so we flip the weights with respect to the x=i
                                                ! point
                                        ...
                                        if (.not. teno) then
                                            ...
                                            poly(1) = v_rs_ws_${XYZ}$ (j, k, l, i) + poly_coef_cbR_${XYZ}$ (j, 2, &
                                                & 0)*dvd(0) + poly_coef_cbR_${XYZ}$ (j, 2, 1)*dvd(-1) + poly_coef_cbR_${XYZ}$ (j, &
                                                & 2, 2)*dvd(-2)
