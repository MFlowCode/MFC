...
contains
    ...
    subroutine s_compute_rhs(q_cons_vf, q_prim_vf, rhs_vf, pb, rhs_pb, mv, rhs_mv, t_step, time_avg) ! -------
        ...
        if (t_step >= 4) then
            ! `time_avg` is moving avg of seconds/gridpt/eqn/rhs
            ! `sys_size` = number of equations
            time_avg = ((abs(t_finish - t_start)/((ix%end - ix%beg)*(iy%end - iy%beg)*(iz%end - iz%beg)))/sys_size + (t_step - 4)*time_avg) &
                       /(t_step - 3)
        else
            time_avg = 0d0
        end if
