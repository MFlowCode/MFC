...
contains
    ...
    impure subroutine s_initialize_rhs_module
        ...
        if (hypo_nc_interface .or. (hypo_nc_dual_pass .and. grid_geometry == 2) &
            ...
            do i = 1, num_dims
                ...
                do l = 1, num_dims
                    @:ALLOCATE(nc_iface_vel_n(i)%vf(l)%sf( &
                        idwbuff(1)%beg:idwbuff(1)%end, &
                        idwbuff(2)%beg:idwbuff(2)%end, &
                        idwbuff(3)%beg:idwbuff(3)%end))
                end do
    ...
    impure subroutine s_compute_rhs(q_cons_vf, q_T_sf, q_prim_vf, bc_type, rhs_vf, pb_in, rhs_pb, mv_in, rhs_mv, t_step, time_avg, stage, is_hat_L)
                        ...
                        do k = 0, n
                            do q = 0, m
                                ...
                                rhs_vf(j)%sf(q, k, l) = rhs_vf(j)%sf(q, k, l) - &
                                                        geom_fac/y_cc(k)*(flux_face1 + flux_face2)
                            end do
                        end do
                    end do
                end do
                    ...
                    if (riemann_solver == 1 .and. alt_soundspeed .and. .not. bubbles_euler) then
                        ...
                        do q_idx = 0, p; do l_idx = 0, n; do k_idx = 0, m
                                    ...
                                    rhs_vf_arg(advxb)%sf(k_idx, l_idx, q_idx) = rhs_vf_arg(advxb)%sf(k_idx, l_idx, q_idx) + &
                                                                                local_k_term_val*local_inv_ds*(local_flux1 - local_flux2)
            ...
            case (2)
                if ((riemann_solver == 1 .and. .not. hll_u_interface) .or. &
                    ...
                    if (riemann_solver == 1 .and. alt_soundspeed .and. .not. bubbles_euler) then
                        ...
                        if (cyl_coord) then
                            ...
                            do l_idx = 0, p; do k_idx = 0, n; do q_idx = 0, m
                                        local_k_term_val = Kterm_arg(q_idx, k_idx, l_idx)
                                        rhs_vf_arg(advxb)%sf(q_idx, k_idx, l_idx) = rhs_vf_arg(advxb)%sf(q_idx, k_idx, l_idx) + &
                                                                                    local_k_term_val*q_prim_vf_arg%vf(momxb + 1)%sf(q_idx, k_idx, l_idx)/y_cc(k_idx)
                                        rhs_vf_arg(advxe)%sf(q_idx, k_idx, l_idx) = rhs_vf_arg(advxe)%sf(q_idx, k_idx, l_idx) - &
                                                                                    local_k_term_val*q_prim_vf_arg%vf(momxb + 1)%sf(q_idx, k_idx, l_idx)/y_cc(k_idx)
                                    end do; end do; end do
                            $:END_GPU_PARALLEL_LOOP()
                    ...
                    if (alt_soundspeed .and. .not. bubbles_euler) then
                        ...
                        $:END_GPU_PARALLEL_LOOP()
                        if (cyl_coord) then
                            $:GPU_PARALLEL_LOOP(collapse=3,private='[k_idx,l_idx,q_idx,local_k_term_val,local_flux1,local_flux2]')
                            do l_idx = 0, p; do k_idx = 0, n; do q_idx = 0, m
                                        local_k_term_val = Kterm_arg(q_idx, k_idx, l_idx)
                                        local_flux1 = flux_src_n_vf_arg%vf(advxb)%sf(q_idx, k_idx, l_idx)
                                        local_flux2 = flux_src_n_vf_arg%vf(advxb)%sf(q_idx, k_idx - 1, l_idx)
                    ...
                    if (riemann_solver == 1) then
                        ...
                        if (alt_soundspeed) then
                            if (bubbles_euler .neqv. .true.) then
                                $:GPU_PARALLEL_LOOP(collapse=3,private='[k_idx,l_idx,q_idx,local_inv_ds,local_k_term_val,local_term_coeff,local_flux1,local_flux2]')
                                do k_idx = 0, p; do q_idx = 0, n; do l_idx = 0, m
                                            local_inv_ds = 1._wp/dz(k_idx)
                                            local_k_term_val = Kterm_arg(l_idx, q_idx, k_idx)
                                            local_term_coeff = q_cons_vf_arg%vf(advxe)%sf(l_idx, q_idx, k_idx) - local_k_term_val
                                ...
                                do k_idx = 0, p; do q_idx = 0, n; do l_idx = 0, m
                                            local_inv_ds = 1._wp/dz(k_idx)
                                            local_k_term_val = Kterm_arg(l_idx, q_idx, k_idx)
                                            local_term_coeff = q_cons_vf_arg%vf(advxb)%sf(l_idx, q_idx, k_idx) + local_k_term_val
