            ...
            do l = startz, p - startz
                do k = starty, n - starty
                    do j = startx, m - startx
                        do d = 1, num_dims
                            flux_gsrc_n(d)%vf(i)%sf(j, k, l) = 0._wp;
                        end do
                    end do
                end do
            end do
...
!$acc parallel loop collapse(3) gang vector default(present)
                do l = 0, p
                    do k = 0, n
                        do j = 0, m
                            blkmod1(j, k, l) = ((gammas(1) + 1._wp)*q_prim_qp%vf(E_idx)%sf(j, k, l) + &
                                                pi_infs(1))/gammas(1)
                            blkmod2(j, k, l) = ((gammas(2) + 1._wp)*q_prim_qp%vf(E_idx)%sf(j, k, l) + &
                                                pi_infs(2))/gammas(2)
                        ...
                        do l = 0, p
                            do k = 0, n
                                do j = 0, m
                                    divu%sf(j, k, l) = 0._wp
                                    divu%sf(j, k, l) = &
                                        (5._wp * (10._wp ** -(1)))/dx(j)*(q_prim_qp%vf(contxe + id)%sf(j + 1, k, l) - &
                        ...
                        do k = 0, n
                            do j = 0, m
                                divu%sf(j, k, l) = divu%sf(j, k, l) + &
                                                   (5._wp * (10._wp ** -(1)))/dy(k)*(q_prim_qp%vf(contxe + id)%sf(j, k + 1, l) - &
                        ...
                        do l = 0, p
                            do k = 0, n
                                do j = 0, m
                                    do i = 1, num_fluids
                                        rhs_vf(i + intxb - 1)%sf(j, k, l) = &
                                            rhs_vf(i + intxb - 1)%sf(j, k, l) - (5._wp * (10._wp ** -(1)))/y_cc(k)* &
                                            q_cons_qp%vf(i + advxb - 1)%sf(j, k, l)* &
                                            q_prim_qp%vf(E_idx)%sf(j, k, l)* &
                                            (flux_src_n(2)%vf(advxb)%sf(j, k, l) + &
                                             flux_src_n(2)%vf(advxb)%sf(j, k - 1, l))
                    ...
                    do j = 1, sys_size
                        do l = 0, p
                            do k = 0, n
                                do q = 0, m
                                    rhs_vf(j)%sf(q, k, l) = &
                                        rhs_vf(j)%sf(q, k, l) - (5._wp * (10._wp ** -(1)))/y_cc(k)* &
                            ...
                            do l = 0, p
                                do k = 1, n
                                    do j = 0, m
                                        ...
                                        do i = momxb, E_idx
                                            rhs_vf(i)%sf(j, k, l) = &
                                                rhs_vf(i)%sf(j, k, l) - (5._wp * (10._wp ** -(1)))/y_cc(k)* &
                                                (flux_src_n(2)%vf(i)%sf(j, k - 1, l) &
                                                 + flux_src_n(2)%vf(i)%sf(j, k, l))
                                        end do
                                    end do
                    ...
                    do j = 1, sys_size
                        do k = 0, p
                            do q = 0, n
                                do l = 0, m
                                    rhs_vf(j)%sf(l, q, k) = &
                                        rhs_vf(j)%sf(l, q, k) - (5._wp * (10._wp ** -(1)))/y_cc(q)* &
                ...
                if (bubbles .and. (.not. qbmm)) then
                    ...
                    do l = 0, p
                        do k = 0, n
                            do j = 0, m
                                divu%sf(j, k, l) = divu%sf(j, k, l) + &
                                                   (5._wp * (10._wp ** -(1)))/dz(l)*(q_prim_qp%vf(contxe + id)%sf(j, k, l + 1) - &
        ...
        do l = 0, p
            do k = 0, n
                do j = 0, m
                    ...
                    if (relax == 1) then
                        ...
                        do i = 1, num_fluids
                            if (q_cons_vf(i + advxb - 1)%sf(j, k, l) > sgm_eps) then
                                ...
                                if (pres_K_init(i) <= -(1._wp - (1._wp * (10._wp ** -(8))))*pres_inf(i) + (1._wp * (10._wp ** -(8)))) &
                                    pres_K_init(i) = -(1._wp - (1._wp * (10._wp ** -(8))))*pres_inf(i) + (1._wp * (10._wp ** -(8)))
                            else
                                pres_K_init(i) = 0._wp
                            end if
                        ...
                        f_pres = (1._wp * (10._wp ** -(9)))
                        df_pres = (1._wp * (10._wp ** 9))
                        ...
                        do iter = 0, 49
                            ...
                            if (abs(f_pres) > (1._wp * (10._wp ** -(10)))) then
                                pres_relax = pres_relax - f_pres/df_pres

                                ! Physical pressure
                                do i = 1, num_fluids
                                    if (pres_relax <= -(1._wp - (1._wp * (10._wp ** -(8))))*pres_inf(i) + (1._wp * (10._wp ** -(8)))) &
                                        pres_relax = -(1._wp - (1._wp * (10._wp ** -(8))))*pres_inf(i) + 1._wp
                                end do
                    ...
                    !$acc loop seq
                    do i = momxb, momxe
                        dyn_pres = dyn_pres + (5._wp * (10._wp ** -(1)))*q_cons_vf(i)%sf(j, k, l)* &
