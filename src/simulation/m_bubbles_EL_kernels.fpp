...
contains
                ...
                do j = 1, smearGrid
                    do k = 1, smearGridz
                        ...
                        if (.not. celloutside) then
                            ...
                            if (any((/bc%x%beg, bc%x%end, bc%y%beg, bc%y%end, bc%z%beg, bc%z%end/) == BC_REFLECTIVE)) then
    ...
    subroutine s_shift_cell_symmetric_bc(cellaux, cell)
        ...
        ! x-dir
        if (bc%x%beg == BC_REFLECTIVE .and. (cell(1) <= mapCells - 1)) then
            cellaux(1) = abs(cellaux(1)) - 1
        end if
        if (bc%x%end == BC_REFLECTIVE .and. (cell(1) >= m + 1 - mapCells)) then
            cellaux(1) = cellaux(1) - (2*(cellaux(1) - m) - 1)
        end if

        ! y-dir
        if (bc%y%beg == BC_REFLECTIVE .and. (cell(2) <= mapCells - 1)) then
            cellaux(2) = abs(cellaux(2)) - 1
        end if
        if (bc%y%end == BC_REFLECTIVE .and. (cell(2) >= n + 1 - mapCells)) then
            cellaux(2) = cellaux(2) - (2*(cellaux(2) - n) - 1)
        end if

        if (p > 0) then
            ! z-dir
            if (bc%z%beg == BC_REFLECTIVE .and. (cell(3) <= mapCells - 1)) then
                cellaux(3) = abs(cellaux(3)) - 1
            end if
            if (bc%z%end == BC_REFLECTIVE .and. (cell(3) >= p + 1 - mapCells)) then
