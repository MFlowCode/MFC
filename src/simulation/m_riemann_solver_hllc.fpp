                ...
                if (norm_dir == ${NORM_DIR}$) then
                    ...
                    if (model_eqns == model_eqns_6eq) then
                        ...
                        $:GPU_PARALLEL_LOOP(collapse=3, private='[i, j, k, l, vel_L, vel_R, Re_L, Re_R, alpha_L, alpha_R, &
                                            ...
                                            & Yi_avg, Phi_avg, h_iL, h_iR, h_avg_2, pcorr, zcoef, rho_L, rho_R, pres_L, pres_R, &
                                            & E_L, E_R, H_L, H_R, Cp_avg, Cv_avg, T_avg, eps, c_sum_Yi_Phi, T_L, T_R, Y_L, Y_R, &
                                            & MW_L, MW_R, R_gas_L, R_gas_R, Cp_L, Cp_R, Cv_L, Cv_R, Gamm_L, Gamm_R, gamma_L, &
                                            & gamma_R, pi_inf_L, pi_inf_R, qv_L, qv_R, qv_avg, c_L, c_R, rho_avg, H_avg, c_avg, &
                                            & gamma_avg, ptilde_L, ptilde_R, vel_L_rms, vel_R_rms, vel_avg_rms, vel_L_tmp, &
                                            & vel_R_tmp, Ms_L, Ms_R, pres_SL, pres_SR, alpha_L_sum, alpha_R_sum, rho_Star, &
                        ...
                        $:GPU_PARALLEL_LOOP(collapse=3, private='[i, q, R0_L, R0_R, V0_L, V0_R, P0_L, P0_R, pbw_L, pbw_R, vel_L, &
                                            ...
                                            & Re_R, pcorr, zcoef, rho_L, rho_R, pres_L, pres_R, E_L, E_R, H_L, H_R, gamma_L, &
                                            & gamma_R, pi_inf_L, pi_inf_R, qv_L, qv_R, qv_avg, c_L, c_R, c_avg, vel_L_rms, &
                                            & vel_R_rms, vel_avg_rms, vel_L_tmp, vel_R_tmp, Ms_L, Ms_R, pres_SL, pres_SR, &
                                            & alpha_L_sum, alpha_R_sum, s_L, s_R, s_M, s_P, s_S, xi_M, xi_P, xi_L, xi_R, xi_L_m1, &
                                            & xi_R_m1, xi_MP, xi_PP, nbub_L, nbub_R, PbwR3Lbar, PbwR3Rbar, R3Lbar, R3Rbar, &
                                            & R3V2Lbar, R3V2Rbar, Ys_L, Ys_R, Cp_iL, Cp_iR, Xs_L, Xs_R, Gamma_iL, Gamma_iR, &
                                            & Yi_avg, Phi_avg, h_iL, h_iR, h_avg_2]', firstprivate='[Re_size_loc1, Re_size_loc2]')
                        do l = ${Z_BND}$%beg, ${Z_BND}$%end
                            do k = ${Y_BND}$%beg, ${Y_BND}$%end
                                do j = ${X_BND}$%beg, ${X_BND}$%end
                                    vel_L_rms = 0._wp; vel_R_rms = 0._wp
                            ...
                            do l = ${Z_BND}$%beg, ${Z_BND}$%end
                                do k = ${Y_BND}$%beg, ${Y_BND}$%end
                                    do j = ${X_BND}$%beg, ${X_BND}$%end
                                        ...
                                        call s_accumulate_mixture_properties(num_fluids, alpha_rho_L, alpha_lim_L, rho_L, &
                                                                             & gamma_L, pi_inf_L, qv_L)
                                        call s_accumulate_mixture_properties(num_fluids, alpha_rho_R, alpha_lim_R, rho_R, &
                                                                             & gamma_R, pi_inf_R, qv_R)
