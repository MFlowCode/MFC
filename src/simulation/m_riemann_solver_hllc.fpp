            ...
            if (norm_dir == ${NORM_DIR}$) then
                ...
                if (model_eqns == model_eqns_6eq) then
                    ...
                    $:GPU_PARALLEL_LOOP(collapse=3, private='[i, j, k, l, vel_L, vel_R, Re_L, Re_R, alpha_L, alpha_R, &
                                        ...
                                        & Yi_avg, Phi_avg, h_iL, h_iR, h_avg_2, tau_e_L, tau_e_R, flux_ene_e, mass_flux, &
                                        & xi_field_L, xi_field_R, pcorr, zcoef, rho_L, rho_R, pres_L, pres_R, E_L, E_R, H_L, H_R, &
                                        & Cp_avg, Cv_avg, T_avg, eps, c_sum_Yi_Phi, T_L, T_R, Y_L, Y_R, MW_L, MW_R, R_gas_L, &
                                        & R_gas_R, Cp_L, Cp_R, Cv_L, Cv_R, Gamm_L, Gamm_R, gamma_L, gamma_R, pi_inf_L, pi_inf_R, &
                    ...
                    do l = ${Z_BND}$%beg, ${Z_BND}$%end
                        do k = ${Y_BND}$%beg, ${Y_BND}$%end
                            do j = ${X_BND}$%beg, ${X_BND}$%end
                                ...
                                $:GPU_LOOP(parallelism='[seq]')
                                do i = 1, eqn_idx%cont%end
                                    mass_flux = mass_flux + flux_rsx_vf(${SF('')}$, i)
                                end do
                                ...
                                if (elasticity) then
                                    ...
                                    do i = 1, num_dims
                                        ...
                                        flux_ene_e = flux_ene_e - xi_M*(vel_L(dir_idx(i))*tau_e_L(dir_idx_tau(i)) &
                                                                        & + s_M*(xi_L*((s_S - vel_L(dir_idx(i))) &
                                                                        & *(tau_e_L(dir_idx_tau(i))/(s_L - vel_L(dir_idx(i))))))) &
                                                                        & - xi_P*(vel_R(dir_idx(i))*tau_e_R(dir_idx_tau(i)) &
                                                                        & + s_P*(xi_R*((s_S - vel_R(dir_idx(i))) &
                                                                        & *(tau_e_R(dir_idx_tau(i))/(s_R - vel_R(dir_idx(i)))))))
                                    end do
                                ...
                                if (hyperelasticity) then
                                    ...
                                    do i = 1, num_dims
                                        flux_rsx_vf(${SF('')}$, &
                                                    & eqn_idx%xi%beg - 1 + i) = mass_flux*(xi_M*xi_field_L(i) + xi_P*xi_field_R(i))
                                    end do
                                    $:GPU_LOOP(parallelism='[seq]')
                                    do i = 1, b_size - 1
                                        flux_rsx_vf(${SF('')}$, &
                                                    & eqn_idx%stress%beg - 1 + i) = mass_flux*(xi_M*tau_e_L(i) + xi_P*tau_e_R(i))
                                    end do
                                    flux_rsx_vf(${SF('')}$, eqn_idx%xi%end + 1) = mass_flux*(xi_M*qL_prim_rsx_vf(${SF('')}$, &
                                                & eqn_idx%xi%end + 1) + xi_P*qR_prim_rsx_vf(${SF(' + 1')}$, eqn_idx%xi%end + 1))
                                end if
                    ...
                    $:GPU_PARALLEL_LOOP(collapse=3, private='[i, T_L, T_R, vel_L_rms, vel_R_rms, pres_L, pres_R, rho_L, gamma_L, &
                                        ...
                                        & Gamma_iR, Cp_iL, Cp_iR, tau_e_L, tau_e_R, flux_ene_e, mass_flux, xi_field_L, &
