                ...
                if (model_eqns == model_eqns_6eq) then
                    ...
                    #:for HYPO in [True, False]
                        ...
                        #:if HYPO
                            ...
                            #:set _hllc_p1 = '[i, j, k, l, q, T_L, T_R, vel_L_rms, vel_R_rms, pres_L, pres_R, rho_L, gamma_L, pi_inf_L, qv_L, rho_R, gamma_R, pi_inf_R, qv_R, alpha_L_sum, alpha_R_sum, E_L, E_R, MW_L, MW_R, R_gas_L, R_gas_R, Cp_L, Cp_R, Cv_L, Cv_R, Cp_avg, Cv_avg, T_avg, eps, c_sum_Yi_Phi, Gamm_L, Gamm_R, Y_L, Y_R, H_L, H_R, qv_avg, rho_avg, gamma_avg, H_avg, c_L, c_R, c_avg, s_P, s_M, xi_P, xi_M, xi_L, xi_R, xi_L_m1, xi_R_m1, Ms_L, Ms_R, pres_SL, pres_SR, vel_L, vel_R, Re_L, Re_R, alpha_L, alpha_R, alpha_rho_L, alpha_rho_R, alpha_lim_L, alpha_lim_R, s_L, s_R, s_S, vel_avg_rms, pcorr, zcoef, ptilde_L, ptilde_R, vel_L_tmp, vel_R_tmp, Ys_L, Ys_R, Xs_L, Xs_R, Gamma_iL, Gamma_iR, Cp_iL, Cp_iR, tau_e_L, tau_e_R, Yi_avg, Phi_avg, h_iL, h_iR, h_avg_2, G_L, G_R, damage_L, damage_R, msol_L, msol_R,'
                            ...
                            do k = ${Y_BND}$%beg, ${Y_BND}$%end
                                do j = ${X_BND}$%beg, ${X_BND}$%end
                                    ...
                                    #:if HYPO
                                        ...
                                        if (cont_damage) then
                                            msol_L = 0._wp; msol_R = 0._wp
                                            $:GPU_LOOP(parallelism='[seq]')
                                            do i = 1, num_fluids
                                                if (Gs_rs(i) > verysmall) then
                                                    msol_L = msol_L + alpha_rho_L(i)
                                                    msol_R = msol_R + alpha_rho_R(i)
                                                end if
                                            end do
                                            flux_rsx_vf(${SF('')}$, &
                                                        & eqn_idx%damage) = xi_M*msol_L*damage_L*(vel_L(dir_idx(1)) + s_M*(xi_L &
                                                        & - 1._wp)) + xi_P*msol_R*damage_R*(vel_R(dir_idx(1)) + s_P*(xi_R - 1._wp))
                                        end if
