    ...
    #:if not MFC_CASE_OPTIMIZATION or riemann_solver in (-1, 2)
        #:for NORM_DIR, XYZ, STENCIL_VAR, COORDS, X_BND, Y_BND, Z_BND in &
            ...
            #:for HYPO in [True, False]
                #:set SUF = '_hypo' if HYPO else ''
                !> One face of the HLLC solve, direction ${XYZ}$${SUF}$: the body of the s_hllc_riemann_solver kernel, moved into a
                !! device routine so its ~22 work arrays (vel_*, alpha_*, Ys/Xs/Gamma_i/Cp_i/h_i*, Re_*, R_species, tau_e_*, U_*,
                !! F_*) are routine LOCALS. As `private` entities of the target region each array costs ~31 us per launch on
                ...
                subroutine s_hllc_face_${XYZ}$${SUF}$ (j, k, l, qL_prim_rsx_vf, qR_prim_rsx_vf, Re_size_loc1, Re_size_loc2, &
                    ...
                    !! copies.
                    integer, intent(in) :: j, k, l, Re_size_loc1, Re_size_loc2, wave_speeds_loc
                    ...
                    real(wp) :: H_L, H_R
                    #:if not MFC_CASE_OPTIMIZATION and USING_AMD
                        real(wp), dimension(${AMD_NUM_SPECIES_MAX}$) :: Ys_L, Ys_R, Xs_L, Xs_R, Gamma_iL, Gamma_iR, Cp_iL, Cp_iR, &
                             & R_species, h_iL, h_iR
                    #:else
                        real(wp), dimension(num_species) :: Ys_L, Ys_R, Xs_L, Xs_R, Gamma_iL, Gamma_iR, Cp_iL, Cp_iR, R_species, &
                             & h_iL, h_iR
                    #:endif
                    ...
                    integer                :: i, q   !< Generic loop iterators
                    ...
                    if (mpp_lim) then
                        ...
                        do i = 1, num_fluids
                            ...
                            qR_prim_rsx_vf(${SF(' + 1')}$, eqn_idx%E + i) = min(max(0._wp, qR_prim_rsx_vf(${SF(' + 1')}$, &
                                           & eqn_idx%E + i)), 1._wp)
                    ...
                    ! Post-limiter loads for the mixture properties; alpha_L/R keep the pre-limiter loads used downstream
                    $:GPU_LOOP(parallelism='[seq]')
                    do i = 1, num_fluids
                    ...
                    #:if HYPO
                        ...
                        call s_compute_hypoelastic_interface_energy(num_fluids, alpha_L, alpha_R, damage_L, damage_R, tau_e_L, &
                            & tau_e_R, G_L, G_R, E_L, E_R)
                    ...
                    if (wave_speeds_loc == wave_speeds_pressure) then
                        call s_compute_average_state(rho_L, rho_R, vel_L, vel_R, H_L, H_R, gamma_L, gamma_R, qv_L, qv_R, rho_avg, &
                                                     & vel_avg_rms, H_avg, gamma_avg, qv_avg)
                        if (chemistry .and. avg_state == avg_state_roe) then
                            ...
                            call s_compute_chemistry_average_state(rho_L, rho_R, T_L, T_R, Ys_L, Ys_R, R_species, h_iL, h_iR, &
                                                                   & Cp_iL, Cp_iR, vel_avg_rms, gamma_avg, c_sum_Yi_Phi)
                        end if
                    end if
                    ...
                    if (wave_speeds_loc == wave_speeds_pressure) then
                        call s_compute_speed_of_sound_avg(pres_R, rho_avg, gamma_avg, pi_inf_R, qv_avg, vel_avg_rms, H_avg, &
                                                          & c_sum_Yi_Phi, alpha_R, c_avg, alpha_rho_R)
                    end if
                    ...
                    ! Low Mach correction
                    if (low_Mach == 2) then
                        call s_apply_low_Mach_velocity(vel_L_rms, vel_R_rms, c_L, c_R, vel_L(dir_idx(1)), vel_R(dir_idx(1)))
                    end if

                    if (wave_speeds_loc == wave_speeds_direct) then
                        #:if HYPO
                            ! Elastic wave speed, Rodriguez et al. JCP (2019)
                            s_L = min(vel_L(dir_idx(1)) - f_elastic_signal_speed(c_L, G_L, tau_e_L(dir_idx_tau(1)), rho_L), &
                                      & vel_R(dir_idx(1)) - f_elastic_signal_speed(c_R, G_R, tau_e_R(dir_idx_tau(1)), rho_R))
                            s_R = max(vel_R(dir_idx(1)) + f_elastic_signal_speed(c_R, G_R, tau_e_R(dir_idx_tau(1)), rho_R), &
                                      & vel_L(dir_idx(1)) + f_elastic_signal_speed(c_L, G_L, tau_e_L(dir_idx_tau(1)), rho_L))
                            s_S = (pres_R - tau_e_R(dir_idx_tau(1)) - pres_L + tau_e_L(dir_idx_tau(1)) + rho_L*vel_L(dir_idx(1)) &
                                   & *(s_L - vel_L(dir_idx(1))) - rho_R*vel_R(dir_idx(1))*(s_R - vel_R(dir_idx(1))))/(rho_L*(s_L &
                                   & - vel_L(dir_idx(1))) - rho_R*(s_R - vel_R(dir_idx(1))))
                        #:else
                            s_L = min(vel_L(dir_idx(1)) - c_L, vel_R(dir_idx(1)) - c_R)
                            s_R = max(vel_R(dir_idx(1)) + c_R, vel_L(dir_idx(1)) + c_L)
                            s_S = (pres_R - pres_L + rho_L*vel_L(dir_idx(1))*(s_L - vel_L(dir_idx(1))) - rho_R*vel_R(dir_idx(1)) &
                                   & *(s_R - vel_R(dir_idx(1))))/(rho_L*(s_L - vel_L(dir_idx(1))) - rho_R*(s_R - vel_R(dir_idx(1))))
                        #:endif
                    else if (wave_speeds_loc == wave_speeds_pressure) then
                        ...
                        Ms_L = max(1._wp, &
                                   & sqrt(1._wp + 5.e-1_wp*(f_isentrope_exponent(gamma_L) + 1._wp)/f_isentrope_exponent(gamma_L) &
                                   & *(pres_SL - pres_L)/(pres_L + f_isentrope_pressure(pi_inf_L, gamma_L))))
                        Ms_R = max(1._wp, &
                                   & sqrt(1._wp + 5.e-1_wp*(f_isentrope_exponent(gamma_R) + 1._wp)/f_isentrope_exponent(gamma_R) &
                                   & *(pres_SR - pres_R)/(pres_R + f_isentrope_pressure(pi_inf_R, gamma_R))))
                    ...
                    ! Low Mach correction
                    pcorr = f_low_Mach_pcorr_hllc(vel_L_rms, vel_R_rms, c_L, c_R, rho_L, rho_R, s_L, s_R, vel_L(dir_idx(1)), &
                                                  & vel_R(dir_idx(1)))
                    ...
                    #:if HYPO
                        flux_rsx_vf(${SF('')}$, &
                                    ...
                                    & + s_M*(xi_L*s_S - vel_L(dir_idx(1)))) + pres_tot_L) + xi_P*(rho_R*(vel_R(dir_idx(1)) &
                                    & *vel_R(dir_idx(1)) + s_P*(xi_R*s_S - vel_R(dir_idx(1)))) + pres_tot_R) + (s_M/s_L)*(s_P/s_R) &
                                    & *pcorr
                        if (n > 0) then
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%cont%end + dir_idx(2)) = xi_M*(rho_L*(vel_L(dir_idx(1))*u_t_L &
                                        & + s_M*(xi_L*u_t_star - u_t_L)) - tau_nt_L) + xi_P*(rho_R*(vel_R(dir_idx(1))*u_t_R &
                                        & + s_P*(xi_R*u_t_star - u_t_R)) - tau_nt_R)
                        end if
                        if (p > 0) then
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%cont%end + dir_idx(3)) = xi_M*(rho_L*(vel_L(dir_idx(1))*u_t2_L &
                                        & + s_M*(xi_L*u_t2_star - u_t2_L)) - tau_nt2_L) + xi_P*(rho_R*(vel_R(dir_idx(1))*u_t2_R &
                                        & + s_P*(xi_R*u_t2_star - u_t2_R)) - tau_nt2_R)
                        end if

                        flux_rsx_vf(${SF('')}$, &
                                    & eqn_idx%E) = xi_M*((E_L + pres_tot_L)*vel_L(dir_idx(1)) - u_t_L*tau_nt_L - u_t2_L*tau_nt2_L &
                                    & + s_M*(xi_L*(E_L + (s_S - vel_L(dir_idx(1)))*(rho_L*s_S + pres_tot_L/(s_L - vel_L(dir_idx(1) &
                                    & ))) + (u_t_L*tau_nt_L - u_t_star*tau_nt_star)/(s_L - vel_L(dir_idx(1))) + (u_t2_L*tau_nt2_L &
                                    & - u_t2_star*tau_nt2_star)/(s_L - vel_L(dir_idx(1)))) - E_L)) + xi_P*((E_R + pres_tot_R) &
                                    & *vel_R(dir_idx(1)) - u_t_R*tau_nt_R - u_t2_R*tau_nt2_R + s_P*(xi_R*(E_R + (s_S &
                                    & - vel_R(dir_idx(1)))*(rho_R*s_S + pres_tot_R/(s_R - vel_R(dir_idx(1)))) + (u_t_R*tau_nt_R &
                                    & - u_t_star*tau_nt_star)/(s_R - vel_R(dir_idx(1))) + (u_t2_R*tau_nt2_R &
                                    & - u_t2_star*tau_nt2_star)/(s_R - vel_R(dir_idx(1)))) - E_R)) + (s_M/s_L)*(s_P/s_R)*pcorr*s_S

                        if (n == 0) then
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%stress%beg) = xi_M*rho_L*tau_nn_L*(vel_L(dir_idx(1)) + s_M*(xi_L - 1._wp)) &
                            ...
                            if (dir_idx(1) == 1) then
                                ...
                                flux_rsx_vf(${SF('')}$, &
                                            ...
                                            & + s_M*(rho_L*xi_L*tau_nt_star - rho_L*tau_nt_L)) + xi_P*(rho_R*vel_R(dir_idx(1)) &
                                            & *tau_nt_R + s_P*(rho_R*xi_R*tau_nt_star - rho_R*tau_nt_R))
                                flux_rsx_vf(${SF('')}$, &
                                            & eqn_idx%stress%beg + 2) = xi_M*rho_L*tau_tt_L*(vel_L(dir_idx(1)) + s_M*(xi_L &
                                            & - 1._wp)) + xi_P*rho_R*tau_tt_R*(vel_R(dir_idx(1)) + s_P*(xi_R - 1._wp))
                            else
                                flux_rsx_vf(${SF('')}$, &
                                            & eqn_idx%stress%beg + 2) = xi_M*rho_L*tau_nn_L*(vel_L(dir_idx(1)) + s_M*(xi_L &
                                ...
                                flux_rsx_vf(${SF('')}$, &
                                            ...
                                            & *tau_nt_R + s_P*(rho_R*xi_R*tau_nt_star - rho_R*tau_nt_R))
                                flux_rsx_vf(${SF('')}$, &
                                            & eqn_idx%stress%beg) = xi_M*rho_L*tau_tt_L*(vel_L(dir_idx(1)) + s_M*(xi_L - 1._wp)) &
                                            & + xi_P*rho_R*tau_tt_R*(vel_R(dir_idx(1)) + s_P*(xi_R - 1._wp))
                            end if
                        else
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%stress%beg - 1 + stress_perm(1)) = xi_M*rho_L*tau_nn_L*(vel_L(dir_idx(1)) &
                                        & + s_M*(xi_L - 1._wp)) + xi_P*rho_R*tau_nn_R*(vel_R(dir_idx(1)) + s_P*(xi_R - 1._wp))
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%stress%beg - 1 + stress_perm(2)) = xi_M*(rho_L*vel_L(dir_idx(1))*tau_nt_L &
                                        & + s_M*(rho_L*xi_L*tau_nt_star - rho_L*tau_nt_L)) + xi_P*(rho_R*vel_R(dir_idx(1)) &
                                        & *tau_nt_R + s_P*(rho_R*xi_R*tau_nt_star - rho_R*tau_nt_R))
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%stress%beg - 1 + stress_perm(4)) = xi_M*(rho_L*vel_L(dir_idx(1))*tau_nt2_L &
                                        & + s_M*(rho_L*xi_L*tau_nt2_star - rho_L*tau_nt2_L)) + xi_P*(rho_R*vel_R(dir_idx(1)) &
                                        & *tau_nt2_R + s_P*(rho_R*xi_R*tau_nt2_star - rho_R*tau_nt2_R))
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%stress%beg - 1 + stress_perm(3)) = xi_M*rho_L*tau_tt_L*(vel_L(dir_idx(1)) &
                                        & + s_M*(xi_L - 1._wp)) + xi_P*rho_R*tau_tt_R*(vel_R(dir_idx(1)) + s_P*(xi_R - 1._wp))
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%stress%beg - 1 + stress_perm(6)) = xi_M*rho_L*tau_t2t2_L*(vel_L(dir_idx(1)) &
                                        & + s_M*(xi_L - 1._wp)) + xi_P*rho_R*tau_t2t2_R*(vel_R(dir_idx(1)) + s_P*(xi_R - 1._wp))
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%stress%beg - 1 + stress_perm(5)) = xi_M*rho_L*tau_t1t2_L*(vel_L(dir_idx(1)) &
                                        & + s_M*(xi_L - 1._wp)) + xi_P*rho_R*tau_t1t2_R*(vel_R(dir_idx(1)) + s_P*(xi_R - 1._wp))
                        end if
                        if (cyl_coord) then
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%stress%end) = xi_M*rho_L*tau_qq_L*(vel_L(dir_idx(1)) + s_M*(xi_L - 1._wp)) &
                                        & + xi_P*rho_R*tau_qq_R*(vel_R(dir_idx(1)) + s_P*(xi_R - 1._wp))
                        end if
                        ...
                        if (cont_damage) then
                            ...
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%damage) = xi_M*solid_partial_density_L*damage_L*(vel_L(dir_idx(1)) + s_M*(xi_L &
                                        & - 1._wp)) + xi_P*solid_partial_density_R*damage_R*(vel_R(dir_idx(1)) + s_P*(xi_R - 1._wp))
                        end if
                    ...
                    #:else
                        $:GPU_LOOP(parallelism='[seq]')
                        do i = 1, num_dims
                            ! MOMENTUM FLUX. identity: xi*(dir_flg*s_S+(1-dir_flg)*u_i)-u_i = (dir_flg*s_L/R+(1-dir_flg)*u_i)*xi_m1
                            flux_rsx_vf(${SF('')}$, &
                                        & eqn_idx%cont%end + dir_idx(i)) = xi_M*(rho_L*(vel_L(dir_idx(1))*vel_L(dir_idx(i)) &
                                        & + s_M*(dir_flg(dir_idx(i))*s_L + (1._wp - dir_flg(dir_idx(i)))*vel_L(dir_idx(i))) &
                                        & *xi_L_m1) + dir_flg(dir_idx(i))*(pres_L)) + xi_P*(rho_R*(vel_R(dir_idx(1)) &
                                        & *vel_R(dir_idx(i)) + s_P*(dir_flg(dir_idx(i))*s_R + (1._wp - dir_flg(dir_idx(i))) &
                                        & *vel_R(dir_idx(i)))*xi_R_m1) + dir_flg(dir_idx(i))*(pres_R)) + (s_M/s_L)*(s_P/s_R) &
                        ...
                        flux_rsx_vf(${SF('')}$, &
                                    ...
                                    & + xi_P*(vel_R(dir_idx(1))*(E_R + pres_R) + s_P*(E_R*xi_R_m1 + xi_R*(s_S - vel_R(dir_idx(1))) &
                                    & *(rho_R*s_S + pres_R/(s_R - vel_R(dir_idx(1)))))) + (s_M/s_L)*(s_P/s_R)*pcorr*s_S
                    #:endif
                    ...
                    if (chemistry) then
                        ...
                        do i = eqn_idx%species%beg, eqn_idx%species%end
                            ...
                            flux_rsx_vf(${SF('')}$, &
                                        & i) = xi_M*rho_L*Y_L*(vel_L(dir_idx(1)) + s_M*xi_L_m1) + xi_P*rho_R*Y_R*(vel_R(dir_idx(1) &
                                        & ) + s_P*xi_R_m1)
                    ...
                    #:if (NORM_DIR == 2)
                        ...
                        #:else
                            if (cyl_coord) then
                                ...
                                flux_gsrc_rsx_vf(${SF('')}$, &
                                                 & eqn_idx%cont%end + dir_idx(1)) = f_compute_hllc_star_momentum_flux(rho_L, &
                                                 & rho_R, vel_L(dir_idx(1)), vel_R(dir_idx(1)), s_M, s_P, s_S, xi_L, xi_R, xi_M, &
                                                 & xi_P, dir_flg(dir_idx(1)))
                    ...
                    #:if (NORM_DIR == 3)
                        if (grid_geometry == 3) then
                            ...
                            flux_gsrc_rsx_vf(${SF('')}$, eqn_idx%mom%beg + 1) = -f_compute_hllc_star_momentum_flux(rho_L, rho_R, &
                                             & vel_L(dir_idx(1)), vel_R(dir_idx(1)), s_M, s_P, s_S, xi_L, xi_R, xi_M, xi_P, &
                                             & dir_flg(dir_idx(1)))
                            flux_gsrc_rsx_vf(${SF('')}$, eqn_idx%mom%end) = flux_rsx_vf(${SF('')}$, eqn_idx%mom%beg + 1)
                        end if
                    #:endif

                end subroutine s_hllc_face_${XYZ}$${SUF}$
            #:endfor
        #:endfor
    #:endif
