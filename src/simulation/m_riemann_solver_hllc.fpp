...
#! This is safe because the arrays a kernel names inside physics branches are always allocated:
...
#! always exist. Without it every launch re-maps the descriptor of each named allocatable. A kernel
#! naming an unallocated array aborts under present, so keep that invariant when adding kernels.
                ...
                if (norm_dir == ${NORM_DIR}$) then
                    ...
                    if (model_eqns == model_eqns_6eq) then
                        ...
                        #:for HYPO in [True, False]
                            ...
                            #:set _hllc_s1 = ['i', 'j', 'k', 'l', 'q', 'T_L', 'T_R', 'vel_L_rms', 'vel_R_rms', 'pres_L', 'pres_R', 'rho_L', &
                                & 'gamma_L', 'pi_inf_L', 'qv_L', 'rho_R', 'gamma_R']
                            #:set _hllc_s2 = ['pi_inf_R', 'qv_R', 'alpha_L_sum', 'alpha_R_sum', 'E_L', 'E_R', 'MW_L', 'MW_R', 'R_gas_L', &
                                & 'R_gas_R', 'Cp_L', 'Cp_R', 'Cv_L', 'Cv_R', 'c_sum_Yi_Phi']
                            #:set _hllc_s3 = ['Gamm_L', 'Gamm_R', 'Y_L', 'Y_R', 'H_L', 'H_R', 'qv_avg', 'rho_avg', 'gamma_avg', 'H_avg', 'c_L', &
                                & 'c_R', 'c_avg', 's_P', 's_M', 'xi_P', 'xi_M', 'xi_L']
                            #:set _hllc_s4 = ['xi_R', 'xi_L_m1', 'xi_R_m1', 'Ms_L', 'Ms_R', 'pres_SL', 'pres_SR']
                            #:set _hllc_s5 = ['s_L', 's_R', 's_S', 'vel_avg_rms', 'pcorr']
                            #:set _hllc_s6 = []
                            #:set _hllc_e1 = ['ptilde_L', 'ptilde_R', 'G_L', 'G_R', 'damage_L', 'damage_R', 'solid_partial_density_L', &
                                & 'solid_partial_density_R']
                            #:set _hllc_e2 = ['u_n_HLLC', 'u_t_HLLC', 'u_t2_HLLC', 'pres_tot_L', 'pres_tot_R', 'u_n_L', 'u_n_R', 'u_t_L', 'u_t_R', &
                                & 'u_t2_L', 'u_t2_R', 'tau_nn_L', 'tau_nn_R']
                            ...
                            #:set _hllc_e4 = ['p_face', 'tau_qq_face', 'A_L', 'A_R', 'denom_A', 'u_t_star', 'tau_nt_star', 'u_t2_star', &
                                & 'tau_nt2_star', 'pres_tot_star', 'u_n_HLL_trace']
                            #:set _hllc_e5 = ['u_t_HLL_trace', 'u_t2_HLL_trace', 'p_face_HLL', 'tau_qq_face_HLL', 'tau_nn_HLL', 'phi', 'Sigma_L', &
                                & 'Sigma_R', 'dSigma', 'Sigma_ref', 'a_L_ref']
                                ...
                                do k = ${Y_BND}$%beg, ${Y_BND}$%end
                                    do j = ${X_BND}$%beg, ${X_BND}$%end
                                        block
                                            ...
                                            #:if not MFC_CASE_OPTIMIZATION and USING_AMD
                                                real(wp), dimension(3) :: alpha_rho_L, alpha_rho_R, alpha_L, alpha_R, &
                                                     & alpha_lim_L, alpha_lim_R, vel_L, vel_R
                                                real(wp), dimension(${AMD_NUM_SPECIES_MAX}$) :: Ys_L, Ys_R, Xs_L, Xs_R, Gamma_iL, &
                                                     & Gamma_iR, Cp_iL, Cp_iR, R_species, h_iL, h_iR
                                            #:else
                                                real(wp), dimension(num_fluids) :: alpha_rho_L, alpha_rho_R, alpha_L, alpha_R, &
                                                     & alpha_lim_L, alpha_lim_R
                                                real(wp), dimension(num_dims)    :: vel_L, vel_R
                                                real(wp), dimension(num_species) :: Ys_L, Ys_R, Xs_L, Xs_R, Gamma_iL, Gamma_iR, &
                                                     & Cp_iL, Cp_iR, R_species, h_iL, h_iR
                                            #:endif
                                            real(wp), dimension(2) :: Re_L, Re_R
                                            #:if HYPO
                                                real(wp), dimension(6) :: tau_e_L, tau_e_R
                                                #:if not MFC_CASE_OPTIMIZATION and USING_AMD
                                                    real(wp), dimension(${AMD_SYS_SIZE_MAX}$) :: U_L, U_R, F_L, F_R, F_star_L, &
                                                         & F_star_R, F_HLLC, F_HLL
                                                #:else
                                                    real(wp), dimension(sys_size) :: U_L, U_R, F_L, F_R, F_star_L, F_star_R, &
                                                         & F_HLLC, F_HLL
                                                #:endif
                                            #:endif
                                            ...
                                            if (mpp_lim) then
                                                ...
                                                do i = 1, num_fluids
                                                    ...
                                                    qR_prim_rsx_vf(${SF(' + 1')}$, i) = max(0._wp, qR_prim_rsx_vf(${SF(' + 1')}$, &
                                                                   & i))
                                                    qR_prim_rsx_vf(${SF(' + 1')}$, eqn_idx%E + i) = min(max(0._wp, &
                                                                   & qR_prim_rsx_vf(${SF(' + 1')}$, eqn_idx%E + i)), 1._wp)
                                                    alpha_L_sum = alpha_L_sum + qL_prim_rsx_vf(${SF('')}$, eqn_idx%E + i)
                                                ...
                                                do i = 1, num_fluids
                                                    qL_prim_rsx_vf(${SF('')}$, eqn_idx%E + i) = qL_prim_rsx_vf(${SF('')}$, &
                                                                   & eqn_idx%E + i)/max(alpha_L_sum, sgm_eps)
                                                    qR_prim_rsx_vf(${SF(' + 1')}$, &
                                                                   & eqn_idx%E + i) = qR_prim_rsx_vf(${SF(' + 1')}$, &
                                                                   & eqn_idx%E + i)/max(alpha_R_sum, sgm_eps)
                                                end do
                                            end if

                                            ! Post-limiter loads for the mixture properties; alpha_L/R keep the pre-limiter loads
                                            ! used downstream
                                            $:GPU_LOOP(parallelism='[seq]')
                                            do i = 1, num_fluids
                                                alpha_rho_L(i) = qL_prim_rsx_vf(${SF('')}$, i)
                                            ...
                                            call s_compute_mixture_coefficients(alpha_rho_L, alpha_lim_L, rho_L, gamma_L, &
                                                                                & pi_inf_L, qv_L)
                                            call s_compute_mixture_coefficients(alpha_rho_R, alpha_lim_R, rho_R, gamma_R, &
                                                                                & pi_inf_R, qv_R)
                                            ...
                                            #:if HYPO
                                                ...
                                                call s_compute_hypoelastic_interface_energy(num_fluids, alpha_L, alpha_R, &
                                                    & damage_L, damage_R, tau_e_L, tau_e_R, G_L, G_R, E_L, E_R)
                                            ...
                                            ! average costs eight square roots per face.
                                            if (wave_speeds == wave_speeds_pressure) then
                                                call s_compute_average_state(rho_L, rho_R, vel_L, vel_R, H_L, H_R, gamma_L, &
                                                                             & gamma_R, qv_L, qv_R, rho_avg, vel_avg_rms, H_avg, &
                                                                             & gamma_avg, qv_avg)
                                            ...
                                            if (wave_speeds == wave_speeds_pressure) then
                                                call s_compute_speed_of_sound_avg(pres_R, rho_avg, gamma_avg, pi_inf_R, qv_avg, &
                                                                                  & vel_avg_rms, H_avg, c_sum_Yi_Phi, alpha_R, &
                                                                                  & c_avg, alpha_rho_R)
                                            end if
                                            ...
                                            ! goes with q_star_L/R = xi_L/R * (variable) xi_L/R = ( ( s_L/R - u_L/R )/(s_L/R -
                                            ! s_star) )
                                            xi_L = (s_L - vel_L(dir_idx(1)))/min(s_L - s_S, -sgm_eps)
                                            xi_R = (s_R - vel_R(dir_idx(1)))/max(s_R - s_S, sgm_eps)
                                            ...
                                            #:if HYPO
                                                flux_rsx_vf(${SF('')}$, &
                                                            ...
                                                            & *vel_L(dir_idx(1)) + s_M*(xi_L*s_S - vel_L(dir_idx(1)))) &
                                                            & + pres_tot_L) + xi_P*(rho_R*(vel_R(dir_idx(1))*vel_R(dir_idx(1)) &
                                                            & + s_P*(xi_R*s_S - vel_R(dir_idx(1)))) + pres_tot_R) + (s_M/s_L) &
                                                            & *(s_P/s_R)*pcorr
                                                if (n > 0) then
                                                    flux_rsx_vf(${SF('')}$, &
                                                                & eqn_idx%cont%end + dir_idx(2)) = xi_M*(rho_L*(vel_L(dir_idx(1)) &
                                                                & *u_t_L + s_M*(xi_L*u_t_star - u_t_L)) - tau_nt_L) &
                                                                & + xi_P*(rho_R*(vel_R(dir_idx(1))*u_t_R + s_P*(xi_R*u_t_star &
                                                                & - u_t_R)) - tau_nt_R)
                                                end if
                                                if (p > 0) then
                                                    flux_rsx_vf(${SF('')}$, &
                                                ...
                                                flux_rsx_vf(${SF('')}$, &
                                                            ...
                                                            & ))) + (u_t_L*tau_nt_L - u_t_star*tau_nt_star)/(s_L &
                                                            & - vel_L(dir_idx(1))) + (u_t2_L*tau_nt2_L - u_t2_star*tau_nt2_star) &
                                                            & /(s_L - vel_L(dir_idx(1)))) - E_L)) + xi_P*((E_R + pres_tot_R) &
                                                            & *vel_R(dir_idx(1)) - u_t_R*tau_nt_R - u_t2_R*tau_nt2_R &
                                                            & + s_P*(xi_R*(E_R + (s_S - vel_R(dir_idx(1)))*(rho_R*s_S &
                                                            & + pres_tot_R/(s_R - vel_R(dir_idx(1)))) + (u_t_R*tau_nt_R &
                                                            & - u_t_star*tau_nt_star)/(s_R - vel_R(dir_idx(1))) &
                                                            & + (u_t2_R*tau_nt2_R - u_t2_star*tau_nt2_star)/(s_R &
                                                ...
                                                if (n == 0) then
                                                    ...
                                                    if (dir_idx(1) == 1) then
                                                        ...
                                                        flux_rsx_vf(${SF('')}$, &
                                                                    & eqn_idx%stress%beg + 2) &
                                                                    & = xi_M*rho_L*tau_tt_L*(vel_L(dir_idx(1)) + s_M*(xi_L &
                                                                    & - 1._wp)) + xi_P*rho_R*tau_tt_R*(vel_R(dir_idx(1)) &
                                                                    & + s_P*(xi_R - 1._wp))
                                                    else
                                                        flux_rsx_vf(${SF('')}$, &
                                                                    & eqn_idx%stress%beg + 2) &
                                                                    & = xi_M*rho_L*tau_nn_L*(vel_L(dir_idx(1)) + s_M*(xi_L &
                                                                    & - 1._wp)) + xi_P*rho_R*tau_nn_R*(vel_R(dir_idx(1)) &
                                                                    & + s_P*(xi_R - 1._wp))
                                                        flux_rsx_vf(${SF('')}$, &
                                                                    & eqn_idx%stress%beg + 1) = xi_M*(rho_L*vel_L(dir_idx(1)) &
                                                                    & *tau_nt_L + s_M*(rho_L*xi_L*tau_nt_star - rho_L*tau_nt_L)) &
                                                ...
                                                if (cont_damage) then
                                                    ...
                                                    flux_rsx_vf(${SF('')}$, &
                                                                & eqn_idx%damage) &
                                                                & = xi_M*solid_partial_density_L*damage_L*(vel_L(dir_idx(1)) &
                                                                & + s_M*(xi_L - 1._wp)) &
                                                                & + xi_P*solid_partial_density_R*damage_R*(vel_R(dir_idx(1)) &
                                                                & + s_P*(xi_R - 1._wp))
                                                end if
                                                ...
                                                if (s_L >= 0._wp) then
                                                    ...
                                                    u_n_HLLC = s_S*(xi_M*xi_L + xi_P*xi_R) &
                                                                    & ; u_t_HLLC = u_t_star; u_t2_HLLC = u_t2_star
                                                end if
                                            ...
                                            #:else
                                                ...
                                                ! \sigma/(s-u)) xi*(E+expr)-E = E*xi_m1 + xi*expr avoids E*(xi-1) cancellation
                                                flux_rsx_vf(${SF('')}$, &
                                                            & eqn_idx%E) = xi_M*(vel_L(dir_idx(1))*(E_L + pres_L) &
                                                            & + s_M*(E_L*xi_L_m1 + xi_L*(s_S - vel_L(dir_idx(1)))*(rho_L*s_S &
                                                            & + pres_L/(s_L - vel_L(dir_idx(1)))))) + xi_P*(vel_R(dir_idx(1)) &
                                                            & *(E_R + pres_R) + s_P*(E_R*xi_R_m1 + xi_R*(s_S - vel_R(dir_idx(1))) &
                                                            & *(rho_R*s_S + pres_R/(s_R - vel_R(dir_idx(1)))))) + (s_M/s_L) &
                                                            & *(s_P/s_R)*pcorr*s_S
                                            #:endif
                                            ...
                                            #:if HYPO
                                                ...
                                                if (riemann_hypo_ADC) then
                                                    ...
                                                    if (s_L >= 0._wp) then
                                                        ...
                                                        if (p > 0) u_t2_HLL_trace = (s_R*u_t2_L - s_L*u_t2_R)/(s_R - s_L &
                                                            & + verysmall)
                                                    end if
                                                ...
                                                #:else
                                                    if (cyl_coord) then
                                                        ...
                                                        flux_gsrc_rsx_vf(${SF('')}$, &
                                                                         & eqn_idx%cont%end + dir_idx(1)) &
                                                                         & = f_compute_hllc_star_momentum_flux(rho_L, rho_R, &
                                                                         & vel_L(dir_idx(1)), vel_R(dir_idx(1)), s_M, s_P, s_S, &
                                                                         & xi_L, xi_R, xi_M, xi_P, dir_flg(dir_idx(1)))
                                                        ! Geometrical source of the void fraction(s) is zero
                                                        $:GPU_LOOP(parallelism='[seq]')
                                                        do i = eqn_idx%adv%beg, eqn_idx%adv%end
                                                            flux_gsrc_rsx_vf(${SF('')}$, i) = 0._wp
