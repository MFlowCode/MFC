...
contains
    ...
    subroutine s_check_inputs_common
        ...
        character(len=5) :: iStr, jStr, numStr !< for int to string conversion
        ...
        ! Constraints on qbmm and polydisperse parameters
        ...
        ! Constraints on directly solving for the number density
        if (adv_n) then
            if (.not. bubbles) then
        ...
        ! Constraints on phase change parameters
        if (relax) then
