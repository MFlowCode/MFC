        ...
        do z = bounds(3)%beg, bounds(3)%end
            do y = bounds(2)%beg, bounds(2)%end
                do x = bounds(1)%beg, bounds(1)%end
                    ...
                    ! e = E - 1/2*|u|^2 cons. sys_idx%E = \rho E cons. contxb = \rho (1-fluid model) cons. momxb + i = \rho u_i
                    energy = q_cons_vf(sys_idx%E)%sf(x, y, z)/q_cons_vf(contxb)%sf(x, y, z)
                    do eqn = momxb, momxe
        ...
        do z = bounds(3)%beg, bounds(3)%end
            do y = bounds(2)%beg, bounds(2)%end
                do x = bounds(1)%beg, bounds(1)%end
                    ...
                    q_T_sf%sf(x, y, z) = q_prim_vf(sys_idx%E)%sf(x, y, z)*mix_mol_weight/(gas_constant*q_prim_vf(1)%sf(x, y, z))
                end do
            end do
        end do
                ...
                do z = isc3%beg, isc3%end
                    do y = isc2%beg, isc2%end
                        do x = isc1%beg, isc1%end
                            ...
                            P_L = q_prim_qp(sys_idx%E)%sf(x, y, z)
                            P_R = q_prim_qp(sys_idx%E)%sf(x + offsets(1), y + offsets(2), z + offsets(3))
                            ...
                            ! Update flux arrays
                            flux_src_vf(sys_idx%E)%sf(x, y, z) = flux_src_vf(sys_idx%E)%sf(x, y, z) - Mass_Diffu_Energy
