...
contains
    ...
    subroutine s_convert_conservative_to_primitive_variables(qK_cons_vf, q_T_sf, qK_prim_vf, ibounds)
        ...
        & rhoYks, B, pres, vftmp, nbub_sc, G_K, T, pres_mag, Ga, B2, m2, S, W, dW, E, D, f, dGa_dW, dp_dW, df_dW, iter]')
            ...
            do k = 0, n
                do j = 0, m
                    ...
                    if (relativity) then
                        ...
                        q_cons_vf(E_idx)%sf(j, k, l) = rho*h*Ga**2 - q_prim_vf(E_idx)%sf(j, k, l) + 0.5_wp*(B2 + v2*B2 - vdotB**2)
                        ! Remove rest energy
                        do i = 1, contxe
                    ...
                    if (chemistry) then
                        ...
                        q_cons_vf(E_idx)%sf(j, k, l) = dyn_pres + rho*e_mix
                    else
