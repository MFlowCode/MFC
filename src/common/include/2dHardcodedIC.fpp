...
#:def Hardcoded2D()
    ...
    case (200)  ! Two-fluid cubic interface
        if (y_cc(j) <= (-x_cc(i)**3 + 1)**(1._wp/3._wp)) then
            ...
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = 1000._wp
        end if
    case (202)  ! Gresho vortex (Gouasmi et al 2022 JCP)
        ...
        if (r < rmax) then
            ...
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = p0 + umax**2*((r/rmax)**2._wp/2._wp)
            ...
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = p0 + umax**2*((r/rmax)**2/2._wp + 4*(1 - (r/rmax) + log(r/rmax)))
        else
            q_prim_vf(momxb)%sf(i, j, 0) = 0._wp
            q_prim_vf(momxe)%sf(i, j, 0) = 0._wp
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = p0 + umax**2*(-2 + 4*log(2._wp))
        end if
    case (203)  ! Gresho vortex (Gouasmi et al 2022 JCP) with density correction
        ...
        if (r < rmax) then
            ...
            q_prim_vf(momxb)%sf(i, j, 0) = -((y_cc(j) - 0.5_wp)/r)*umax*(2._wp - r/rmax)
            q_prim_vf(momxe)%sf(i, j, 0) = ((x_cc(i) - 0.5_wp)/r)*umax*(2._wp - r/rmax)
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = p0 + umax**2*((r/rmax)**2/2._wp + 4._wp*(1._wp - (r/rmax) + log(r/rmax)))
        else
            q_prim_vf(momxb)%sf(i, j, 0) = 0._wp
            q_prim_vf(momxe)%sf(i, j, 0) = 0._wp
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = p0 + umax**2._wp*(-2._wp + 4*log(2._wp))
        end if

        q_prim_vf(contxb)%sf(i, j, 0) = q_prim_vf(sys_idx%E)%sf(i, j, 0)**(1._wp/gam)
    case (204)  ! Rayleigh-Taylor instability
        ...
        if (y_cc(j) > intH) then
            ...
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = pref + rhoH*9.81_wp*(1.2_wp - y_cc(j))
        else
            q_prim_vf(advxb)%sf(i, j, 0) = alph
            q_prim_vf(advxe)%sf(i, j, 0) = 1._wp - alph
            q_prim_vf(contxb)%sf(i, j, 0) = alph*rhoH
            ...
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = pInt + rhoL*9.81_wp*(intH - y_cc(j))
        end if
    case (205)  ! 2D lung wave interaction problem
        ...
        if (y_cc(j) > intH) then
            ...
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = patch_icpp(1)%pres
    ...
    case (250)  ! MHD Orszag-Tang vortex
        ...
        q_prim_vf(sys_idx%B%beg)%sf(i, j, 0) = -sin(2._wp*pi*y_cc(j))/sqrt(4._wp*pi)
        q_prim_vf(sys_idx%B%beg + 1)%sf(i, j, 0) = sin(4._wp*pi*x_cc(i))/sqrt(4._wp*pi)
    case (251)  ! RMHD Cylindrical Blast Wave [Mignone, 2006: Section 4.3.1]
        if (x_cc(i)**2 + y_cc(j)**2 < 0.08_wp**2) then
            ...
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = 1.0
            ...
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = 1.0_wp*factor + 3.e-5_wp*(1.0_wp - factor)
        else
            q_prim_vf(contxb)%sf(i, j, 0) = 1.e-4_wp
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = 3.e-5_wp
        end if
    ...
    case (253)  ! MHD Smooth Magnetic Vortex
        ...
        ! magnetic field
        q_prim_vf(sys_idx%B%beg)%sf(i, j, 0) = -y_cc(j)*exp(1 - (x_cc(i)**2 + y_cc(j)**2))/(2.*pi)
        q_prim_vf(sys_idx%B%beg + 1)%sf(i, j, 0) = x_cc(i)*exp(1 - (x_cc(i)**2 + y_cc(j)**2))/(2.*pi)
    ...
    case (260)  ! Gaussian Divergence Pulse
        ...
        ! B-field
        q_prim_vf(sys_idx%B%beg)%sf(i, j, 0) = 1._wp + C_mhd*erf((x_cc(i) - 0.5_wp)/sigma)
    case (261)  ! Blob
        r0 = 1._wp/sqrt(8._wp)
        ...
        alpha = r/r0
        if (alpha < 1) then
            q_prim_vf(sys_idx%B%beg)%sf(i, j, 0) = 1._wp/sqrt(4._wp*pi)*(alpha**8 - 2._wp*alpha**4 + 1._wp)
            ! q_prim_vf(sys_idx%B%beg)%sf(i,j,0) = 1._wp/sqrt(4000._wp*pi) * (4096._wp*r2**4 - 128._wp*r2**2 + 1._wp)
            ! q_prim_vf(sys_idx%B%beg)%sf(i,j,0) = 1._wp/(4._wp*pi) * (alpha**8 - 2._wp*alpha**4 + 1._wp)
            ! q_prim_vf(sys_idx%E)%sf(i,j,0) = 6._wp - q_prim_vf(sys_idx%B%beg)%sf(i,j,0)**2/2._wp
        end if
    case (262)  ! Tilted 2D MHD shock‐tube at α = arctan2 (≈63.4°)
        ...
        if (r <= 0.5_wp) then
            ...
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = 20._wp
            q_prim_vf(sys_idx%B%beg)%sf(i, j, 0) = (5._wp/sqrt(4._wp*pi))*cosA - (5._wp/sqrt(4._wp*pi))*sinA
            q_prim_vf(sys_idx%B%beg + 1)%sf(i, j, 0) = (5._wp/sqrt(4._wp*pi))*sinA + (5._wp/sqrt(4._wp*pi))*cosA
        else
            ! RIGHT state: \rho=1, v\parallel=-10, v\perp=0, p=1, B\parallel=B\perp=5/\sqrt(4\pi)
            q_prim_vf(contxb)%sf(i, j, 0) = 1._wp
            q_prim_vf(momxb)%sf(i, j, 0) = -10._wp*cosA
            q_prim_vf(momxb + 1)%sf(i, j, 0) = -10._wp*sinA
            q_prim_vf(sys_idx%E)%sf(i, j, 0) = 1._wp
            q_prim_vf(sys_idx%B%beg)%sf(i, j, 0) = (5._wp/sqrt(4._wp*pi))*cosA - (5._wp/sqrt(4._wp*pi))*sinA
            q_prim_vf(sys_idx%B%beg + 1)%sf(i, j, 0) = (5._wp/sqrt(4._wp*pi))*sinA + (5._wp/sqrt(4._wp*pi))*cosA
        end if
