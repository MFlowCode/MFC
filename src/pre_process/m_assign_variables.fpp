...
contains
    ...
    pure subroutine s_assign_patch_mixture_primitive_variables(patch_id, j, k, l, &
        ...
        q_prim_vf(eqn_idx%pi_inf)%sf(j, k, l) = &
    ...
    pure subroutine s_perturb_primitive(j, k, l, q_prim_vf)
        ...
        if (qbmm) then
            do i = 1, nb
                q_prim_vf(bubxb + 1 + (i - 1)*nmom)%sf(j, k, l) = q_prim_vf(bubxb + 1 + (i - 1)*nmom)%sf(j, k, l)*((p0 - fluid_pp(1)%pv)/(q_prim_vf(eqn_idx%E)%sf(j, k, l)*p0 - fluid_pp(1)%pv))**(1/3._wp)
            end do
        end if
    ...
    impure subroutine s_assign_patch_species_primitive_variables(patch_id, j, k, l, &
        ...
        if (mpp_lim .and. bubbles_euler) then
            ...
            do i = eqn_idx%adv%beg, eqn_idx%adv%end - 1
                q_prim_vf(i)%sf = q_prim_vf(i)%sf*(1._wp - q_prim_vf(eqn_idx%alf)%sf) &
                                  /alf_sum%sf
            end do
        end if
        ...
        if (bubbles_euler) then
            do i = 1, nb
                ...
                if (qbmm) then
                    ...
                    if (dist_type == 1) then
                        q_prim_vf(eqn_idx%bub%fullmom(i, 0, 0))%sf(j, k, l) = 1._wp
                        q_prim_vf(eqn_idx%bub%fullmom(i, 1, 0))%sf(j, k, l) = muR
                        q_prim_vf(eqn_idx%bub%fullmom(i, 0, 1))%sf(j, k, l) = muV
                        q_prim_vf(eqn_idx%bub%fullmom(i, 2, 0))%sf(j, k, l) = muR**2 + sigR**2
                        q_prim_vf(eqn_idx%bub%fullmom(i, 1, 1))%sf(j, k, l) = muR*muV + rhoRV*sigR*sigV
                        q_prim_vf(eqn_idx%bub%fullmom(i, 0, 2))%sf(j, k, l) = muV**2 + sigV**2
                    else if (dist_type == 2) then
                        q_prim_vf(eqn_idx%bub%fullmom(i, 0, 0))%sf(j, k, l) = 1._wp
                        q_prim_vf(eqn_idx%bub%fullmom(i, 1, 0))%sf(j, k, l) = exp((sigR**2)/2._wp)*muR
                        q_prim_vf(eqn_idx%bub%fullmom(i, 0, 1))%sf(j, k, l) = muV
                        q_prim_vf(eqn_idx%bub%fullmom(i, 2, 0))%sf(j, k, l) = exp((sigR**2)*2._wp)*(muR**2)
                        q_prim_vf(eqn_idx%bub%fullmom(i, 1, 1))%sf(j, k, l) = exp((sigR**2)/2._wp)*muR*muV
                        q_prim_vf(eqn_idx%bub%fullmom(i, 0, 2))%sf(j, k, l) = muV**2 + sigV**2
                    end if
                else
                    q_prim_vf(eqn_idx%bub%rs(i))%sf(j, k, l) = muR
                    q_prim_vf(eqn_idx%bub%vs(i))%sf(j, k, l) = muV
                    if (.not. polytropic) then
                        q_prim_vf(eqn_idx%bub%ps(i))%sf(j, k, l) = patch_icpp(patch_id)%p0
                        q_prim_vf(eqn_idx%bub%ms(i))%sf(j, k, l) = patch_icpp(patch_id)%m0
                    end if
                end if
            end do
            ...
            if (adv_n) then
                ...
                R3bar = 0._wp
                do i = 1, nb
                    R3bar = R3bar + weight(i)*(q_prim_vf(eqn_idx%bub%rs(i))%sf(j, k, l))**3._wp
                end do
        ...
        q_prim_vf(eqn_idx%E)%sf(j, k, l) = &
            (eta*patch_icpp(patch_id)%pres &
             + (1._wp - eta)*orig_prim_vf(eqn_idx%E))

        ! Volume fractions \alpha
        do i = eqn_idx%adv%beg, eqn_idx%adv%end
            q_prim_vf(i)%sf(j, k, l) = &
                eta*patch_icpp(patch_id)%alpha(i - eqn_idx%E) &
        ...
        if (elasticity) then
            do i = 1, (eqn_idx%stress%end - eqn_idx%stress%beg) + 1
                q_prim_vf(i + eqn_idx%stress%beg - 1)%sf(j, k, l) = &
                    (eta*patch_icpp(patch_id)%tau_e(i) &
                     + (1._wp - eta)*orig_prim_vf(i + eqn_idx%stress%beg - 1))
            end do
        end if
        ...
        if (model_eqns /= 4) then
            ...
            q_prim_vf(1)%sf(j, k, l) = &
                (((q_prim_vf(eqn_idx%E)%sf(j, k, l) + pi_inf)/(pref + pi_inf))**(1/lit_gamma))* &
                rhoref*(1 - q_prim_vf(eqn_idx%alf)%sf(j, k, l))
        end if
        ...
        ! Velocity
        do i = 1, eqn_idx%E - eqn_idx%mom%beg
            q_prim_vf(i + eqn_idx%cont%end)%sf(j, k, l) = &
                (eta*patch_icpp(patch_id)%vel(i) &
                 + (1._wp - eta)*orig_prim_vf(i + eqn_idx%cont%end))
        end do
        ...
        if (mixlayer_vel_profile) then
            q_prim_vf(1 + eqn_idx%cont%end)%sf(j, k, l) = &
                (eta*patch_icpp(patch_id)%vel(1)*tanh(y_cc(k)*mixlayer_vel_coef) &
                 + (1._wp - eta)*orig_prim_vf(1 + eqn_idx%cont%end))
        end if
        ...
        if (bubbles_euler) then
            do i = 1, nb
                ...
                if (qbmm) then
                    ...
                    ! q_prim_vf(eqn_idx%bub%rs(i))%sf(j,k,l) = &
                    !     (eta * R0(i)*patch_icpp(patch_id)%r0 &
                    !     + (1._wp-eta)*orig_prim_vf(eqn_idx%bub%rs(i)))
                    ! q_prim_vf(eqn_idx%bub%vs(i))%sf(j,k,l) = &
                    !     (eta * V0(i)*patch_icpp(patch_id)%v0 &
                    !     + (1._wp-eta)*orig_prim_vf(eqn_idx%bub%vs(i)))
                    q_prim_vf(eqn_idx%bub%rs(i))%sf(j, k, l) = muR
                    q_prim_vf(eqn_idx%bub%vs(i))%sf(j, k, l) = muV
        ...
        if (bubbles_euler .and. (.not. polytropic) .and. (.not. qbmm)) then
            do i = 1, nb
                if (f_is_default(q_prim_vf(eqn_idx%bub%ps(i))%sf(j, k, l))) then
                    q_prim_vf(eqn_idx%bub%ps(i))%sf(j, k, l) = pb0(i)
                    ! print *, 'setting to pb0'
                end if
                if (f_is_default(q_prim_vf(eqn_idx%bub%ms(i))%sf(j, k, l))) then
                    q_prim_vf(eqn_idx%bub%ms(i))%sf(j, k, l) = mass_v0(i)
                end if
            end do
        end if

        if (surface_tension) then
            q_prim_vf(eqn_idx%c)%sf(j, k, l) = eta*patch_icpp(patch_id)%cf_val + &
