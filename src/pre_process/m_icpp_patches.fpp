                ...
                if (x_boundary%beg <= x_cc(i) .and. x_boundary%end >= x_cc(i) .and. y_boundary%beg <= y_cc(j) &
                    ...
                    if (patch_icpp(patch_id)%alter_patch(patch_id_fp(i, j, 0))) then
                        ...
                        if ((q_prim_vf(1)%sf(i, j, 0) < 1.e-10) .and. (model_eqns == 4)) then
                            ...
                            q_prim_vf(1)%sf(i, j, 0) = (((q_prim_vf(sys_idx%E)%sf(i, j, &
                                      & 0) + pi_inf)/(pref + pi_inf))**(1._wp/lit_gamma))*rhoref*(1._wp - q_prim_vf(sys_idx%alf) &
                                      & %sf(i, j, 0))
                        end if
        ...
        do j = 0, n
            do i = 0, m
                if (x_boundary%beg <= x_cc(i) .and. x_boundary%end >= x_cc(i) .and. y_boundary%beg <= y_cc(j) &
                    ...
                    ! Assign Parameters
                    q_prim_vf(sys_idx%mom%beg)%sf(i, j, 0) = U0*sin(x_cc(i)/L0)*cos(y_cc(j)/L0)
                    q_prim_vf(sys_idx%mom%end)%sf(i, j, 0) = -U0*cos(x_cc(i)/L0)*sin(y_cc(j)/L0)
