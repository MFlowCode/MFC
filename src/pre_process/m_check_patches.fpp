...
contains
    ...
    impure subroutine s_check_patches
        ...
        do i = 1, num_patches_max
            if (i <= num_patches) then
                ...
                @:PROHIBIT(patch_icpp(i)%geometry == 6, "Invalid patch geometry number. "// "patch_icpp("//trim(iStr)//")%geometry is deprecated.")
                @:PROHIBIT(patch_icpp(i)%geometry == 7, "Invalid patch geometry number. "// "patch_icpp("//trim(iStr)//")%geometry is deprecated.")
                @:PROHIBIT(patch_icpp(i)%geometry == 15, "Invalid patch geometry number. "// "patch_icpp("//trim(iStr)//")%geometry is deprecated.")
                @:PROHIBIT(patch_icpp(i)%geometry == dflt_int, "Invalid patch geometry number. "// "patch_icpp("//trim(iStr)//")%geometry must be set.")
                ...
                @:PROHIBIT(patch_icpp(i)%geometry /= dflt_int, "Inactive patch defined. "// "patch_icpp("//trim(iStr)//")%geometry not be set for inactive patches. "// "Patch "//trim(iStr)//" is inactive as the number of patches is "//trim(num_patches_str))
    ...
    impure subroutine s_check_cylinder_patch_geometry(patch_id)
        ...
        @:PROHIBIT(count([ patch_icpp(patch_id)%length_x > 0._wp, patch_icpp(patch_id)%length_y > 0._wp, patch_icpp(patch_id)%length_z > 0._wp ]) /= 1, "Cylinder patch "//trim(iStr)//": Exactly one of length_x, length_y, or length_z must be defined and positive")

        ! Ensure the defined length is positive
        @:PROHIBIT( (.not. f_is_default(patch_icpp(patch_id)%length_x) .and. patch_icpp(patch_id)%length_x <= 0._wp) .or. (.not. f_is_default(patch_icpp(patch_id)%length_y) .and. patch_icpp(patch_id)%length_y <= 0._wp) .or. (.not. f_is_default(patch_icpp(patch_id)%length_z) .and. patch_icpp(patch_id)%length_z <= 0._wp), "Cylinder patch "//trim(iStr)//": The defined length_{} must be greater than zero")
    ...
    impure subroutine s_check_active_patch_alteration_rights(patch_id)
        ...
        @:PROHIBIT(any(patch_icpp(patch_id)%alter_patch(patch_id:)), "Patch "//trim(iStr)// ":alter_patch(i) must be false for i >= "//trim(iStr)//". Only preceding patches can be altered")
    ...
    impure subroutine s_check_supported_patch_smoothing(patch_id)
        ...
        if (patch_icpp(patch_id)%smoothen) then
            @:PROHIBIT(patch_icpp(patch_id)%smooth_patch_id >= patch_id, "Smoothen enabled. Patch "//trim(iStr)//": smooth_patch_id must be less than patch_id")
            @:PROHIBIT(patch_icpp(patch_id)%smooth_patch_id == 0, "Smoothen enabled. Patch "//trim(iStr)//": smooth_patch_id must be greater than zero")
            @:PROHIBIT(patch_icpp(patch_id)%smooth_coeff <= 0._wp, "Smoothen enabled. Patch "//trim(iStr)//": smooth_coeff must be greater than zero")
        else
            @:PROHIBIT(patch_icpp(patch_id)%smooth_patch_id /= patch_id, "Smoothen disabled. Patch "//trim(iStr)//": smooth_patch_id must be equal to patch_id")
            @:PROHIBIT(.not. f_is_default(patch_icpp(patch_id)%smooth_coeff), "Smoothen disabled. Patch "//trim(iStr)//": smooth_coeff must not be set")
        end if
    ...
    impure subroutine s_check_unsupported_patch_smoothing(patch_id)
        ...
        @:PROHIBIT(patch_icpp(patch_id)%smooth_patch_id /= patch_id, "Inactive patch "//trim(iStr)//": smooth_patch_id must be equal to patch_id")
        @:PROHIBIT(.not. f_is_default(patch_icpp(patch_id)%smooth_coeff), "Inactive patch "//trim(iStr)//": smooth_coeff must not be set")
    ...
    impure subroutine s_check_active_patch_primitive_variables(patch_id)
        ...
        @:PROHIBIT(n == 0 .and. (.not. f_is_default(patch_icpp(patch_id)%vel(2))) .and. (.not. f_approx_equal(patch_icpp(patch_id)%vel(2) , 0._wp)) .and. (.not. mhd), "Patch "//trim(iStr)//": vel(2) must not be set when n = 0")
        @:PROHIBIT(n > 0 .and. f_is_default(patch_icpp(patch_id)%vel(2)), "Patch "//trim(iStr)//": vel(2) must be set when n > 0")
        @:PROHIBIT(p == 0 .and. (.not. f_is_default(patch_icpp(patch_id)%vel(3))) .and. (.not. f_approx_equal(patch_icpp(patch_id)%vel(3) , 0._wp)) .and. (.not. mhd), "Patch "//trim(iStr)//": vel(3) must not be set when p = 0")
        @:PROHIBIT(p > 0 .and. f_is_default(patch_icpp(patch_id)%vel(3)), "Patch "//trim(iStr)//": vel(3) must be set when p > 0")
        @:PROHIBIT(mhd .and. (f_is_default(patch_icpp(patch_id)%vel(2)) .or. f_is_default(patch_icpp(patch_id)%vel(3))), "Patch "//trim(iStr)//": All velocities (vel(1:3)) must be set when mhd = true")
        @:PROHIBIT(model_eqns == 1 .and. patch_icpp(patch_id)%rho <= 0._wp, "Patch "//trim(iStr)//": rho must be greater than zero when model_eqns = 1")
        @:PROHIBIT(model_eqns == 1 .and. patch_icpp(patch_id)%gamma <= 0._wp, "Patch "//trim(iStr)//": gamma must be greater than zero when model_eqns = 1")
        @:PROHIBIT(model_eqns == 1 .and. patch_icpp(patch_id)%pi_inf < 0._wp, "Patch "//trim(iStr)//": pi_inf must be greater than or equal to zero when model_eqns = 1")
        @:PROHIBIT(patch_icpp(patch_id)%geometry == 5 .and. patch_icpp(patch_id)%pi_inf > 0, "Patch "//trim(iStr)//": pi_inf must be less than or equal to zero when geometry = 5")
        @:PROHIBIT(model_eqns == 2 .and. any(patch_icpp(patch_id)%alpha_rho(1:num_fluids) < 0._wp), "Patch "//trim(iStr)//": alpha_rho(1:num_fluids) must be greater than or equal to zero when model_eqns = 2")
        ...
        if (model_eqns == 2 .and. num_fluids < num_fluids_max) then
            @:PROHIBIT(.not. f_all_default(patch_icpp(patch_id)%alpha_rho(num_fluids + 1:)), "Patch "//trim(iStr)//": alpha_rho(i) must not be set for i > num_fluids")
            @:PROHIBIT(.not. f_all_default(patch_icpp(patch_id)%alpha(num_fluids + 1:)), "Patch "//trim(iStr)//": alpha(i) must not be set for i > num_fluids")
            @:PROHIBIT(f_is_default(patch_icpp(patch_id)%alpha(num_fluids)), "Patch "//trim(iStr)//": alpha(num_fluids) must be set")
        end if
    ...
    impure subroutine s_check_inactive_patch_primitive_variables(patch_id)
        ...
        @:PROHIBIT(.not. f_all_default(patch_icpp(patch_id)%alpha_rho), "Inactive patch "//trim(iStr)//": alpha_rho must not be set")
    ...
    impure subroutine s_check_model_geometry(patch_id)
        ...
        @:PROHIBIT(.not. file_exists, "Model file "//trim(patch_icpp(patch_id)%model_filepath)// " requested by patch "//trim(iStr)//" does not exist")
