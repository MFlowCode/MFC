...
contains
    ...
    impure subroutine s_apply_initial_condition(start, finish)
        ...
        if (hyper_cleaning) then
            @:ASSERT(sys_idx%psi > 0, "hyper_cleaning requires sys_idx%psi to be set")
            do l = 0, p
                do k = 0, n
                    do j = 0, m
                        r2 = x_cc(j)**2
                        ...
                        q_cons_vf(sys_idx%psi)%sf(j, k, l) = 1.0e-2_wp*exp(-r2/(2.0_wp*0.05_wp**2))
                        q_prim_vf(sys_idx%psi)%sf(j, k, l) = q_cons_vf(sys_idx%psi)%sf(j, k, l)
                    end do
                end do
            end do
        end if
