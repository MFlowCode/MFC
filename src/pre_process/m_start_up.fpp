...
!! user provided inputs, grid and data. This module also allocates
!! the relevant variables sets up the mpi decomposition and
!! procedures.
module m_start_up
    ...
    !! the folder associated with the rank of the local processor

    character(LEN=path_len + 2*name_len), private :: t_step_dir !<
    !! of time-step folder containing preexisting grid and/or
    !! data to be used as starting point for pre-process
...
contains
    ...
    !>  Reads the configuration file pre_process.inp, in order to
        !! parameters in module m_global_parameters.f90
    ...
    impure subroutine s_read_input_file
        ...
        logical :: file_check !<
            !! used for the purpose of asserting whether a file
            !! is not present in the designated location
        ...
        ! for all of the parameters to be inputted by the user
        ...
        ! the status of the pre_process.inp file
        file_loc = 'pre_process.inp'
        inquire (FILE=trim(file_loc), EXIST=file_check)

        ! whether the input file is there. If it is, the input file
        ! read. If not, the program is terminated.
        if (file_check) then
    ...
    !! are compatible with the code's options
    !! the combination of these choices results into a
    !! for the pre-process
    impure subroutine s_check_input_file
        ...
        logical :: dir_check !<
            !! used to test the existence of folders

        ! the existence of the case folder
    ...
    !> The goal of this subroutine is to read in any preexisting
        !! as well as based on the imported grid, complete
        !! global computational domain parameters.
    impure subroutine s_read_serial_grid_data_files

        ! string used to store the address of a particular file
        character(LEN=len_trim(case_dir) + 3*name_len) :: file_loc

        ! variable used to test the existence of folders
        logical :: dir_check

        ! logical used for the purpose of asserting whether a file
        ! or is not present in the designated location
        logical :: file_check

        ! address of the local processor rank and time-step directory
        ...
        ! as to the existence of the time-step directory
        file_loc = trim(t_step_dir)//'/.'
        call my_inquire(file_loc, dir_check)

        ! the time-step directory is missing, the pre-process exits
        if (dir_check .neqv. .true.) then
            call s_mpi_abort('Time-step folder '//trim(t_step_dir)// &
                             ' is missing. Exiting.')
        end if

        ! the Grid Data File for the x-direction

        ! whether x_cb.dat exists
        file_loc = trim(t_step_dir)//'/x_cb.dat'
        inquire (FILE=trim(file_loc), EXIST=file_check)

        ! it exists, x_cb.dat is read
        if (file_check) then
        ...
        ! locations of domain bounds
        x_domain%beg = x_cb(-1)
        x_domain%end = x_cb(m)

        ! the Grid Data File for the y-direction

        if (n > 0) then

            ! whether y_cb.dat exists
            file_loc = trim(t_step_dir)//'/y_cb.dat'
            inquire (FILE=trim(file_loc), EXIST=file_check)

            ! it exists, y_cb.dat is read
            if (file_check) then
                open (1, FILE=trim(file_loc), FORM='unformatted', &
                      STATUS='old', ACTION='read')
                read (1) y_cb(-1:n)
            ...
            ! the Grid Data File for the z-direction
            if (p > 0) then

                ! whether z_cb.dat exists
                file_loc = trim(t_step_dir)//'/z_cb.dat'
                inquire (FILE=trim(file_loc), EXIST=file_check)

                ! it exists, z_cb.dat is read
                if (file_check) then
                    open (1, FILE=trim(file_loc), FORM='unformatted', &
                          STATUS='old', ACTION='read')
                    read (1) z_cb(-1:p)
        ...
        ! only the preexisting grid data files are read in and there will
        ! be any preexisting initial condition data files imported, then
        ! directory associated with the rank of the local processor may
        ! cleaned to make room for the new pre-process data. In addition,
        ! time-step directory that will contain the new grid and initial
    ...
    !> Cell-boundary data are checked for consistency by looking
        !! (non-)uniform cell-width distributions for all the
        !! directions and making sure that all of
        !! are positively valued
    impure subroutine s_check_grid_data_files

        ! Data Consistency Check in x-direction
        ...
        ! Data Consistency Check in y-direction
        ...
        if (n > 0) then
            ...
            ! Data Consistency Check in z-direction
    ...
    !> The goal of this subroutine is to read in any preexisting
        !! data files so that they may be used by
        !! as a starting point in the creation of an
        !! initial condition.
        !! Conservative variables
        !! track if a cell is within the immersed boundary
    impure subroutine s_read_serial_ic_data_files(q_cons_vf_in, ib_markers_in)
        ...
        character(LEN= &
            ...
            !! store the variable position, in character form, of the
            !! conservative variable file
        ...
        ! the Conservative Variables Data Files
        do i = 1, sys_size

            ! whether data file associated with variable position
            ! the currently manipulated conservative variable exists
            write (file_num, '(I0)') i
            file_loc = trim(t_step_dir)//'/q_cons_vf'// &
                       trim(file_num)//'.dat'
            inquire (FILE=trim(file_loc), EXIST=file_check)

            ! it exists, the data file is read
            if (file_check) then
                open (1, FILE=trim(file_loc), FORM='unformatted', &
                      STATUS='old', ACTION='read')
        ...
        if (qbmm .and. .not. polytropic) then
            do i = 1, nb
                do r = 1, nnode
                    ! whether data file associated with variable position
                    ! the currently manipulated bubble variable exists
        ...
        ! the preexisting grid and initial condition data files have
        ! read in, the directory associated with the rank of the local
        ! may be cleaned out to make room for new pre-process data.
        ! addition, the time-step folder that will contain the new grid
        ! initial condition data are also generated.
    ...
    impure subroutine s_read_parallel_ic_data_files(q_cons_vf_in, ib_markers_in)
        ...
        if (file_exist) then
            ...
            ! some integers so MPI can read even the biggest files
    ...
    impure subroutine s_initialize_modules
        ! of parameters, allocation procedures, and/or any other tasks
        ! to properly setup the modules
        ...
        ! the D directory if it doesn't exit, to store
        ! serial data files
        call s_create_directory('D')

        ! pointers for serial or parallel I/O
    ...
    impure subroutine s_apply_initial_condition(start, finish)
        ...
        ! up the grid and the initial condition. If the grid is read in from
        ! grid data files, it is checked for consistency. If the grid is
        ! read in, it is generated from scratch according to the inputs provided
        ! the user. The initial condition may also be read in. It in turn is not
        ! for consistency since it WILL further be edited by the pre-process
        ! also because it may be incomplete at the time it is read in. Finally,
        ! the grid and initial condition are completely setup, they are written
        ! their respective data files.

        ! up grid and initial condition
        call cpu_time(start)
...
&                pTg-equilirium (relax = "T" activated)'
    ...
    impure subroutine s_initialize_mpi_domain
        ! of the MPI environment

        call s_mpi_initialize()

        ! 0 processor assigns default values to user inputs prior to reading
        ! in from the input file. Next, the user inputs are read in and their
        ! is checked. The detection of any inconsistencies automatically
        ! to the termination of the pre-process.
        ...
        ! out if pre-process is in fact not truly executed in parallel.
