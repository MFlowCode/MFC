        ...
        do k = 0, p
            do j = 0, n
                do i = 0, m
                    ...
                    perturb_alpha = q_prim_vf(sys_idx%E + perturb_sph_fluid)%sf(i, j, k)
                    ...
                    if ((.not. f_approx_equal(perturb_alpha, 0._wp)) .and. (.not. f_approx_equal(perturb_alpha, 1._wp))) then
                        do l = 1, num_fluids
                            q_prim_vf(l)%sf(i, j, k) = q_prim_vf(sys_idx%E + l)%sf(i, j, k)*fluid_rho(l)
                        end do
                    end if
                end do
            end do
        ...
        do k = 0, p
            do j = 0, n
                do i = 0, m
                    ...
                    q_prim_vf(sys_idx%mom%end)%sf(i, j, k) = rand_real*q_prim_vf(sys_idx%mom%beg)%sf(i, j, k)
                    q_prim_vf(sys_idx%mom%beg)%sf(i, j, k) = (1._wp + rand_real)*q_prim_vf(sys_idx%mom%beg)%sf(i, j, k)
                    if (bubbles_euler) then
                        q_prim_vf(sys_idx%alf)%sf(i, j, k) = (1._wp + rand_real)*q_prim_vf(sys_idx%alf)%sf(i, j, k)
                    end if
                end do
            end do
        end do
