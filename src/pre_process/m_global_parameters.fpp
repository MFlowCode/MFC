...
module m_global_parameters
    ...
    ! integer :: eqn_idx%E                               !< Index of total energy equation
        ...
        if (model_eqns == 1) then
            ...
            if (bubbles_euler) then
                ...
                if (qbmm) then
                    ...
                    allocate (eqn_idx%bub%fullmom(nb, 0:nmom, 0:nmom))

                    do i = 1, nb
                        do j = 1, nmom
                            eqn_idx%bub%moms(i, j) = eqn_idx%bub%beg + (j - 1) + (i - 1)*nmom
                        end do
                        eqn_idx%bub%fullmom(i, 0, 0) = eqn_idx%bub%moms(i, 1)
                        eqn_idx%bub%fullmom(i, 1, 0) = eqn_idx%bub%moms(i, 2)
                        eqn_idx%bub%fullmom(i, 0, 1) = eqn_idx%bub%moms(i, 3)
                        eqn_idx%bub%fullmom(i, 2, 0) = eqn_idx%bub%moms(i, 4)
                        eqn_idx%bub%fullmom(i, 1, 1) = eqn_idx%bub%moms(i, 5)
                        eqn_idx%bub%fullmom(i, 0, 2) = eqn_idx%bub%moms(i, 6)
                        eqn_idx%bub%rs(i) = eqn_idx%bub%fullmom(i, 1, 0)
                    end do
                else
                    do i = 1, nb
                        if (.not. polytropic) then
            ...
            if (mhd) then
                eqn_idx%B%beg = eqn_idx%sys_size + 1
                if (n == 0) then
                    eqn_idx%B%end = eqn_idx%sys_size + 2 ! 1D: By, Bz
                else
                    eqn_idx%B%end = eqn_idx%sys_size + 3 ! 2D/3D: Bx, By, Bz
                end if
                eqn_idx%sys_size = eqn_idx%B%end
            end if
